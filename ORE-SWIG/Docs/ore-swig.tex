...
\begin{enumerate}
    ...
    \item ORE\_STATIC\_RUNTIME (Windows builds)
\end{enumerate}
...
\subsection*{Recent Expansion Highlights}
The SWIG bindings expansion introduced substantial coverage and infrastructure updates across QuantExt, OREData, and OREAnalytics. Highlights include:
\begin{enumerate}
    \item Shared SWIG infrastructure files and common typemaps for cleaner layering and reduced duplication.
    \item New and extended wrappers for analytics (scenario, engine, aggregation, simulation, SIMM, analytics orchestration), model builders, model data, and portfolio/configuration areas.
    \item Master include files updated so new layer wrappers are compiled into the single \texttt{ORE} Python module.
    \item Documentation enhancements in SWIG entry points:
    \begin{enumerate}
        \item \texttt{\%feature("autodoc", "1")} enabled for Python bindings.
        \item Global \texttt{\%feature("docstring")} support enabled.
        \item Layer-level \texttt{\%pythoncode} module overview blocks added for QuantExt, OREData, and OREAnalytics.
    \end{enumerate}
    \item Build validation flow standardised on Windows with \texttt{SET ORE\_STATIC\_RUNTIME=1} before wrap/build/test.
\end{enumerate}

The historical Windows linker issue remains relevant in some environments:
...
\end{lstlisting}
The current hacky fix for that would be to comment out the function in OREData/utilities/osutils.* in ore. However, the function is used in restore.
