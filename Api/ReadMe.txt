...
docker-compose -f docker-compose-build-sfs.yml -f docker-compose-build-restapi.yml build

Once the images are built to run the two images you can run this command:
docker-compose -f docker-compose-start.yml up

Note for the above command: If you want to specify the Input and Output separately from the docker file through an environment variable you have to use the –env-file=name.env flag with name being the name of the environment variable so for this example it would be: docker-compose -f docker-compose-start.yml --env-file=docker-envfile.env up 

Then once the images are running on postman send a post request in a json format that contains the data for the example. All xml data files should be sent to the simplefileserver image in the URL format that is encoded twice. 

Once the request is sent the docker images should build the results and send the results to the output path specified. 

Last note: apifast.py and apiflask.py are basic scripts that produce the same output as above except they do not build the input parameters using python. Fast Api is very similar to flask and for the above example it should be easy to transition between flask and Fast Api simply by changing the api requests to the syntax of Fast Api which can be seen in the apifast.py script
