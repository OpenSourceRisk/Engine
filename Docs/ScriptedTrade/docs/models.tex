...
dW\,dZ &= \rho\,dt
\end{align*}
for multiple EQ underlyings. Each Heston model is calibrated to a section of the vo\-la\-ti\-lity surface, specified in
terms of option expiries and log-moneyness in smile-direction, i.e. strike prices 
$$
...
We set those following the ``parismonious'' approach in \cite{dimitroff_parsimonious},
...
the original Heston processes in QuantLib, see \cite{innocentis_efficient_heston}.
...
$$
which is one way of computing the matrix square root of the ``internal'' correlation matrix 
$$
C =\left(\begin{array}{cc} 1 & \rho \\ \rho & 1 \end{array} \right).
$$
