...
\begin{minted}[fontsize=\footnotesize]{xml}
   <Product type="ScriptedTrade">
    ...
    <EngineParameters>
      ...
      <!-- Heston specific -->
      <Parameter name="CalibrationExpiries">3M</Parameter>
      <Parameter name="CalibrationVarianceTerms">1M, 2M, 3M, 6M, 9M, 1Y</Parameter>
      <Parameter name="HestonCalibrationMethod">PiecewiseBestFit</Parameter>
      <Parameter name="HestonInitialValues">0.04, 1.0, 0.5, -0.5, 0.04</Parameter> 
      <Parameter name="HestonFixedValues">N, N, N, N, N</Parameter>                
      <Parameter name="HestonRelaxedFellerConstraint">0.25</Parameter>             
      <Parameter name="HestonCalibrationRestarts">0</Parameter>
      <Parameter name="HestonMaximumInitialValues">0.1, 20, 3, 0.9, 0.1</Parameter> 
      <Parameter name="HestonTolerance">0.001</Parameter>                          
      <Parameter name="HestonProcessDiscretization">QuadraticExponential</Parameter>
...
\item Heston-specific parameters [Optional]:
  \begin{itemize}
  \item HestonCalibrationExpiries [Optional]: Comma-separated list of option expiry terms, will be used for Heston model calibration if provided.
    Otherwise we use the effective simulation dates to determine calibration option expiries. \\
    Default: empty list
  \item HestonCalibrationVarianceTerms [Optional]: Comma-separated list of terms where annualised variances are computed from the provided
    ...
    Default: empty list
  \item HestonInitialValues [Optional]: Initial values for the five (constant) model parameters in order $\theta$, $\kappa$, $\sigma$,
    $\rho$, $v_0$. \\
    Default: $0.04, 1.0, 0.5, -0.5, 0.04$
  \item HestonFixedValues [Optional]: Comma-separated list of booleans to indicate which of the five parameters above shall be kept
    fixed in the ``ConstantBestFit'' calibration (see below), same order as the parmeters above. \\
    Default: none fixed, i.e. ``N,N,N,N,N''
  \item HestonRelaxedFellerConstraint [Optional]: Constraint on $2\kappa\theta/\sigma^2$ to be used in the ``ConstantBestFit''
    ...
    Default: 0.25 
  \item HestonMaxCalibrationAttempts [Optional]: If set to zero, we calibrate the Heston
    parameters once (except for fixed parameters, see above), starting from the initial values/guesses; if positive,
    we perform several calibrations as specified by ``HestonMaxCalibrationAttempts'' with randomised initial values, keeping the best result. \\
    Default: 0
  \item HestonMaximumInitialValues [Optional]: When using HestonMaxCalibrationAttempts $>$ 0,  this comma-separated list specifies the range of the
  ...
  \item HestonEarlyExitThreshold [Optional]: When using HestonMaxCalibrationAttempts $>$ 0, exit the search when the calibration error
    (root mean squared error of implied black volatilities) is below this tolerance. \\
    Default: 0.005
  \item HestonMaxAcceptableError [Optional]: Log an error message if the (best) calibration error exceeds this value.\\
    Default: 0.05
  \item HestonCalibrationMethod: Choices are ``None'', ``ConstantBestFit'', ``PiecewiseBestFit'', ``PiecewiseBootstrap''. \\
    ``None'' means no calibration, the initial parameter values are used; \\
    ``ConstantBestFit'' means calibration of the Heston model with constant parameters only, possibly with restarts (see HestonMaxCalibrationAttempts above); \\
  ...
  \item HestonProcessDiscretization [Optional]: Choice of the HestonProcess discretization (PartialTruncation, FullTruncation,
  ...
  \item HestonQuantoProcessDiscretization [Optional]: Choice of the HestonProcess discretization in case the indices contain
    at least one that needs a quanto adjustment \\
    Default: Use the value of HestonProcessDiscretization above, if missing
  \item HestonQuantoTimeStepsPerYear: The number of time steps used in case the indices contain
  ...
  \item Debug [Optional]: If set to true or Y, we generate a report (assetmodel\_paths.csv) that contains the spot price realisations
