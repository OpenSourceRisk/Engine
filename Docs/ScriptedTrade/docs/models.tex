...
\item Heston-specific parameters:
  \begin{itemize}
  \item CalibrationExpiries: Comma-separated list of option expiry terms, to be used in the model calibration. \\
    Default: 3M, 6M
  \item CalibrationVarianceTerms: Comma-separated list of terms where annualised variances are computed from the provided
    ...
    Default: 1M, 2M, 3M, 6M, 9M, 1Y, 15M, 18M, 21M, 2Y, 5Y, 7Y, 10Y
  ...
  \item HestonFixedValues: Comma-separated list of booleans to indicate which of the five parameters above shall be kept
    fixed in the calibration, same order as the parmeters above. \\
    Default: none fixed, i.e. ``N,N,N,N,N''
  \item HestonRelaxedFellerConstraint: Constraint on $2\kappa\theta/\sigma^2$ to be used in the calibration, to be chosen
    in $[0,1]$; 1 means original Feller constraint, 0 means no constraint, anything in between can be chosen representing
    a somewhat relaxed Feller constraint. \\
    Default: 0.25 
  \item HestonCalibrationRestarts: If set to zero, we calibrate all five parameters after an initial guess for $\theta$,
    $\kappa$ and $v_0$. If positive, we perform as many calibrations as specified by ``restarts'' with randomised initial
    values, keeping the best result. \\
    Default: 0
