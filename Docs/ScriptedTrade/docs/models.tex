...
\item Heston-specific parameters:
  \begin{itemize}
  \item CalibrationExpiries: Comma-separated list of option expiry terms, will be used for Heston model calibration if provided.
    Otherwise we use the effective simulation dates to determine calibration option expiries. \\
    Default: empty list
  \item CalibrationVarianceTerms: Comma-separated list of terms where annualised variances are computed from the provided
  ...
  \item HestonInitialValues: Initial values for the five (constant) model parameters in order $\theta$, $\kappa$, $\sigma$,
    $\rho$, $v_0$. \\
    Default: $0.04, 1.0, 0.5, -0.5, 0.04$
  \item HestonFixedValues: Comma-separated list of booleans to indicate which of the five parameters above shall be kept
    fixed in the ``ConstantBestFit'' calibration (see below), same order as the parmeters above. \\
    Default: none fixed, i.e. ``N,N,N,N,N''
  \item HestonRelaxedFellerConstraint: Constraint on $2\kappa\theta/\sigma^2$ to be used in the ``ConstantBestFit''
    ...
    Default: 0.25 
  \item HestonCalibrationRestarts: Applies to the ``ConstantBestFit'' method below; if set to zero, we calibrate the Heston
    ...
    Default: 0
  \item HestonMaximumInitialValues: When using ``restarts'' above, this comma-separated list specifies the range of the
  ...
  \item HestonTolerance: Error tolerance of the calibration (root mean squared error of implied black volatilities) where
  ...
  \item HestonProcessDiscretization: Choice of the QuantLib::HestonProcess discretization (PartialTruncation, FullTruncation,
  ...
  \item Debug: If set to true or Y, we generate a report (assetmodel\_paths.csv) that contains the spot price realisations
