...
\item Heston-specific parameters:
  ...
  \item HestonRelaxedFellerConstraint [Optional]: Constraint on $2\kappa\theta/\sigma^2$ to be used in the ``ConstantBestFit''
    ...
    Default: 0.25 
  \item HestonCalibrationRestarts [Optional]: Applies to the ``ConstantBestFit'' method below; if set to zero, we calibrate the Heston
    parameters once (except for fixed parameters, see above), starting from the initial values/guesses; if positive,
    we perform as many calibrations as specified by ``restarts'' with randomised initial values, keeping the best result. \\
    Default: 0
  \item HestonMaximumInitialValues [Optional]: When using ``restarts'' above, this comma-separated list specifies the range of the
