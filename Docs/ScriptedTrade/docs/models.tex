...
\item Heston-specific parameters:
  ...
  \item HestonEarlyExitTolerance [Optional]: When using HestonMaxCalibrationAttempts > 0, exit the search when the calibration error
  ...
  \item HestonMaxAcceptableError [Optional]: Log an error message if the (best) calibration error exceeds this value.\\
    Default: 0.05
  \item HestonTolerance [Optional]: Error tolerance of the calibration (root mean squared error of implied black volatilities) where
    we stop the search for best parameters when calibrating with HestonCalibrationRestarts $>0$. \\
    Default: 0.001
  \item HestonCalibrationMethod: Choices are ``ConstantBestFit'', ``PiecewiseBestFit'', ``PiecewiseBootstrap''. \\
    ``ConstantBestFit'' means calibration of the Heston model with constant parameters only, possibly with restarts (see above); \\
...
\medskip
Limitation: The quanto adustment is currently implemented using a simple Euler scheme which is only consistent with
the Heston process discretizations {\em FullTruncation, PartialTruncation, Reflection} which should be used with
a sufficiently large number of time steps per year. 
