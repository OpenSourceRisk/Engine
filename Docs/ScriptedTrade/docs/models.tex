...
\item Heston-specific parameters:
  ...
  \item HestonCalibrationExpiries [Optional]: Comma-separated list of option expiry terms, will be used for Heston model calibration if provided.
    ...
    Default: empty list
  \item CalibrationVarianceTerms [Optional]: Comma-separated list of terms where annualised variances are computed from the provided
  ...
  \item HestonCalibrationMethod: Choices are ``ConstantBestFit'', ``PiecewiseBestFit'', ``PiecewiseBootstrap''. \\
    ``ConstantBestFit'' means calibaration of the Heston model with constant parameters only, possibly with restarts (see above); \\
