...
\item an optional NewSchedules node specifying new schedules should be created before the script execution, this node
  ...
  \end{itemize}
\item an optional StickyCloseOutStates node specifying variables that should be held constant during an AMC / AMC-CG
  exposure run with sticky close-out mpor mode, usually a list of exercise / barrier hit indicators, see
  \ref{scripting_amc} for more details on this
\item an optional AmcCg node specifying the components of a trade for the calculation of the conditional expectation of
  the npv and aad-based derivatives, see \ref{scripting_amc} for more details on this
