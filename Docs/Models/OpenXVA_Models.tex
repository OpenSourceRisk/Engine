...
If parameters are constrained (e.g. by positivity) this can be specified by QL's \verb+Constraint+ and this constraint will be ensured during model calibration. However this is done by simply rejecting non-admissable points in an otherwise unconstrained optimizer (usually Levenberg Marquardt or Simplex).

A better way of doing the optimization under constraints with unconstrained optimizers is to perform the optimization in transformed variables with unconstrained domain. For example, positivity can be ensured by optimizing $x\in(-\infty,\infty)$, but interpreting $x^2\in(0,\infty)$ as the actual parameter value.
...
\scriptsize
\begin{minted}[bgcolor=mintedBg]{c++}
    XAssetModel(const std::vector<ext::make_shared<Parametrization> >
                    &parametrizations,
...
\begin{minted}[bgcolor=mintedBg]{c++}
    ...
    const ext::make_shared<StochasticProcess>
...
\begin{minted}[bgcolor=mintedBg]{c++}
    ...
    void calibrateIrLgm1fVolatilitiesIterative(
        const Size ccy,
        const std::vector<ext::make_shared<CalibrationHelper> > &helpers,
...
\scriptsize
\begin{minted}[bgcolor=mintedBg]{c++}
    Lgm(const ext::make_shared<IrLgm1fParametrization> &parametrization);
    Lgm(const ext::make_shared<XAssetModel> &model, const Size ccy);
\end{minted}
\normalsize
\medskip
...
\begin{minted}[bgcolor=mintedBg]{c++}
    ...
    AnalyticLgmSwaptionEngine(
        const ext::make_shared<Lgm> &model,
    ...
    AnalyticLgmSwaptionEngine(
        const ext::make_shared<XAssetModel> &model, const Size ccy,
        const Handle<YieldTermStructure> &discountCurve =
            Handle<YieldTermStructure>(),
        const FloatSpreadMapping floatSpreadMapping = proRata);
    ...
    AnalyticLgmSwaptionEngine(
        const ext::make_shared<IrLgm1fParametrization> irlgm1f,
        const Handle<YieldTermStructure> &discountCurve =
            Handle<YieldTermStructure>(),
        const FloatSpreadMapping floatSpreadMapping = proRata);
\end{minted}
...
\scriptsize
\begin{minted}[bgcolor=mintedBg]{c++}
    AnalyticCcLgmFxOptionEngine(const ext::make_shared<XAssetModel> &model,
