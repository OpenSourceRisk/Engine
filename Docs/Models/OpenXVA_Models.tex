...
\scriptsize
\begin{minted}[bgcolor=mintedBg]{c++}
    XAssetModel(const std::vector<ext::shared_ptr<Parametrization> >
                    &parametrizations,
...
\scriptsize
\begin{minted}[bgcolor=mintedBg]{c++}
    Lgm(const ext::shared_ptr<IrLgm1fParametrization> &parametrization);
    Lgm(const ext::shared_ptr<XAssetModel> &model, const Size ccy);
\end{minted}
\normalsize
\medskip
...
\begin{minted}[bgcolor=mintedBg]{c++}
    ...
    AnalyticLgmSwaptionEngine(
        const ext::shared_ptr<Lgm> &model,
    ...
    AnalyticLgmSwaptionEngine(
        const ext::shared_ptr<XAssetModel> &model, const Size ccy,
        const Handle<YieldTermStructure> &discountCurve =
            Handle<YieldTermStructure>(),
        const FloatSpreadMapping floatSpreadMapping = proRata);
    ...
    AnalyticLgmSwaptionEngine(
        const ext::shared_ptr<IrLgm1fParametrization> irlgm1f,
        const Handle<YieldTermStructure> &discountCurve =
            Handle<YieldTermStructure>(),
        const FloatSpreadMapping floatSpreadMapping = proRata);
\end{minted}
...
\scriptsize
\begin{minted}[bgcolor=mintedBg]{c++}
    AnalyticCcLgmFxOptionEngine(const ext::shared_ptr<XAssetModel> &model,
