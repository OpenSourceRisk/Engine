...
\item Mingone2022Essvi: Extended Surface SVI (see \ref{sec:essvi}) with global calibration \cite{Mingone_2022}, arbitrage-free,
\end{itemize}
...
Applicable to model variant Mingone2022Essvi.
...
f_i &=& \min \left( \frac{4}{1+|\rho_i|}, \sqrt{f_{GJ}(\theta_i, |\rho_i|)}\right) \quad \text{for } i \ge 1 \\
A_{\psi_1} &=& 0 \\
...
\item ``Implied'': if $\theta$ is implied from ATM market volatility
