...
na & 23 June 2025 & Alex Chow & add parameters CashflowGeneration.OnCpnMaxSimTimes,
  CashflowGeneration.OnCpnAddSimTimesCutoff, Regression.MaxSimTimesIR, Regression.MaxSimTimesFX,
  Regression.MaxSimTimesEQ, Regression.VarGroupMode\\
\hline
\end{supertabular}
\end{center}
...
AMC provide the required functionality to be processed by the AMC engine, for technical details cf. \ref{overview}.
...
<Product type="BermudanSwaption">
  ...
  <EngineParameters>
    ...
    <Parameter name="CashflowGeneration.OnCpnMaxSimTimes">1</Parameter>
    <Parameter name="CashflowGeneration.OnCpnAddSimTimesCutoff">0D</Parameter>
    <Parameter name="Regression.MaxSimTimesIR">0</Parameter>
    <Parameter name="Regression.MaxSimTimesFX">0</Parameter>
    <Parameter name="Regression.MaxSimTimesEQ">0</Parameter>
  </EngineParameters>
</Product>
\end{minted}
...
\item \verb+MinObsDate+: if true the conditional expectation of each cashflow is taken from the minimum possible
  ...
  is \verb+true+
\item \verb+RegressorModel+: \verb+Simple+, \verb+Lagged+, \verb+LaggedIR+, \verb+LaggedFX+, \verb+LaggedEQ+.
  If not given, it defaults to \verb+Simple+. For an observation date the full model state observed on this date is
  included in the regressor. In any case only past states are included to the extend that these past states were also
  used in the cashflow generation phase. Depending on the choice the regressor is built as follows:
  \begin{itemize}
    \item \verb+Simple+: No past states are included though.
    \item \verb+Lagged+: In addition, past IR, FX and EQ states that are relevant for future cashflows are included.
    \item \verb+LaggedIR+: In addition, past IR states that are relevant for future cashflows are included. For example,
      for an Overnight Indexed cashflow the IR states observed on fixing dates are included.
    \item \verb+LaggedFX+: In addition, past FX states that are relevant for future cashflows are included. For example,
      for a FX resettable cashflow the FX state observed on the FX reset date is included.
    \item \verb+LaggedEQ+: In addition, past EQ states that are relevant for future cashflows are included. For example,
      for an EQ indexed cashflow the EQ state observed on the EQ fixing date is included.
  \end{itemize}
...
\item \verb+ReevaluateExerciseInStickyRun+: Optional, defaults to false. If true, the exercise decision is updated for
  ...
  date.
\item \verb+CashflowGeneration.OnCpnMaxSimTimes+ [optional]: The maximum number of IR states used in calculation of
...
\item \verb+CashflowGeneration.OnCpnAddSimTimesCutoff+ [optional]: The cutoff time for IR states being included in
...
\item \verb+Regression.MaxSimTimesIR+ [optional]: The maximum number of regressors per IR model for the regression analysis.
  If not given, it defaults to 0. If set to 0, there is no limit to number of regressors per model. This setting is only
  relevant when \verb+RegressorModel+ is \verb+Lagged+ or \verb+LaggedIR+.
\item \verb+Regression.MaxSimTimesFX+ [optional]: The maximum number of regressors per FX model for the regression analysis.
  If not given, it defaults to 0. If set to 0, there is no limit to number of regressors per model. This setting is only
  relevant when \verb+RegressorModel+ is \verb+Lagged+ or \verb+LaggedFX+.
\item \verb+Regression.MaxSimTimesEQ+ [optional]: The maximum number of regressors per EQ model for the regression analysis.
  If not given, it defaults to 0. If set to 0, there is no limit to number of regressors per model. This setting is only
  relevant when \verb+RegressorModel+ is \verb+Lagged+ or \verb+LaggedEQ+
\item \verb+Regression.VarGroupMode+ [optional]: \verb+Global+, \verb+Trivial+. If not given, it defaults to \verb+Global+.
  Depending on the choice the regressors are assigned to var groups as follows:
    \begin{itemize}
    \item \verb+Global+: All regressors are assigned to a sinlge var group
    \item \verb+Trivial+: Every regressor is assigned to a group that consists of a single element
  \end{itemize}
\end{enumerate}
...
\begin{table}[hbt]
  \begin{tabular}{l|l|l}
    ...
    Bond & CrossAssetModel & AMC \\
    ForwardBond & CrossAssetModel & AMC \\
  \end{tabular}
  \caption{AMC enabled products with engine and model types}
  \label{tbl:amcconfig}
\end{table}

A sample configuration file can be found in Examples / AmericanMonteCarlo / Input / pricingengine\_amc.xml. Currently it
...
\begin{enumerate}
\item \verb+CapFlooredIborLeg+
\item \verb+CapFlooredInterpolatedIborLeg+
\item \verb+CMS+  
\end{enumerate}
...
The folder \verb+Examples/AmericanMonteCarlo+ contains an example using the AMC valuation engine to produce exposure
...
\item FX Option
\item FX Swap
\item Composite Trade
\item Overnight Indexed Swap
\item Average Overnight Indexed Swap
\end{enumerate}
...
\verb+generateAmcEngineBuilders()+ in \verb+xvaanalytic.cpp+ and are special in that unlike usual engine builders they
...
For technical reasons, the configuration also contains configurations for \\ \verb+CapFlooredIborLeg+,
\verb+CapFlooredInterpolatedIborLeg+ and \verb+CMS+
...
the override of the method \verb+XvaAnalyticImpl::runAnalytic()+.
