...
\subsection{Testsuites}\label{example:testsuites}

The build includes a set of testsuites for each of QuantLib, QuantExt, OREData, and OREAnalytics libraries. You can run
all tests by navigating into the Build folder and running the following command:
{\tt ctest -C Release -j4 --timeout 3600}
...
\item riskClassBreakdown (optional, default Y): if true (the default), VaR is computed separately for
  each risk-class / risk-type combination in addition to the aggregate portfolio row. Set to false to
  compute only the aggregate {\tt [All, All]} row, which significantly reduces runtime when the
  per-class breakdown is not required.
\item quantiles: comma separated list of quantiles to be reported
\item includeExpectedShortfall (optional): boolean, if {\tt Y} the output report contains additional columns {\tt ExpectedShortfall\_<quantile>} reporting the Expected Shortfall (average P\&L in the tail at or below the VaR level) for each quantile; defaults to {\tt N}
...
fixings to {\tt Input/DimValidation/fixings.csv}. The script then loops over simulation dates of that single path
...
\item Swap with normal yield curves in HW2F: {\tt python run\_swap\_hw2f.py}
...
\item FX Forward Exposure with LocalVol FX process and deterministic interest rate
\end{itemize}
...
\item Simulation in the two-factor Hull-White model, calibrated to historical and risk neutral volatilities: {\tt python run\_hw2f.py}
...
% TODO add description for 2F simulation
...
\subsubsection{FX Forward Exposure with LocalVol FX process and deterministic interest rate}\label{example:fx_localvol_detrates}

Calling

\medskip
\centerline{\tt python run\_fx\_localvol.py }
\medskip

runs pricing of FX Options with smile and exosure simulation with local vol and deterministic rates. The EPE / ENE match
the T0 pricing with smile volas. Using a Black Scholes fx process underestimates the EPE / ENE on the other hand. You
can run with Black Scholes by using the outcommented sections in the simulation configuration. You can also switch to
stochastic rates there. If the local vol process is not corrected for stochastic rates, the resulting EPE / ENE will be
too high compared to the T0 pricing.
...
% TODO add description for risk neutral calibration
...
Scripted equity products can also be priced using the Heston model. To demonstrate this, we provide a separate portfolio and
configuration in {\tt Examples/ScriptedTrade} that can be run with

\medskip
\centerline{\tt python run\_heston.py}

\medskip
or by running the Jupyter notebook

\medskip
\centerline{\tt python -m jupyterlab heston.ipynb \&}

\medskip
covering a vanilla Equity Option, Barrier Option and Rainbow Option with three underlyings. The Jupyter notebook prices
the portfolio twice, using a Black and Heston model setup for comparison, and highlights the new asset model calibration reports
and the MC paths report so that we can validate the simulated price distribution against the semi-analytical benchmark.

The switch from Black to Haston is achieved by modifying the ScriptedTrade's pricing engine configuration, see
{\tt pricingengine\_heston.xml}, as shown in the following listing.
...
\begin{minted}[fontsize=\footnotesize]{xml}
  <Product type="ScriptedTrade">
    ...
    <ModelParameters>
      ...
      <Parameter name="Model">Heston</Parameter>
      ...
    </ModelParameters>
    ...
    <EngineParameters>
      ...
      ...
      <Parameter name="CalibrationMoneyness">-2.0,-1.5,-1.0,-0.5,0.0,0.5,1.0,1.5,2.0</Parameter>
...
\end{minted}
%\hrule
\caption{Pricing engine parametrisation for using Heston}
\end{listing}

Note:
\begin{itemize}
\item FullDynamicFx needs to be set to {\tt false} for now, because the Heston process might need an
  approproate drift adjustment for equities with natural payoff in a foreign currency;
  this adjustment is not implemented yet
\item Engine needs to be set to {\tt MC}, because the finite difference version of the engine
  is not implemented yet
\end{itemize}
See the scripted trade documentation for a discussion of the Heston-specific parameters above.
...
\item FX Local Vol exposure generation, run with {\tt python run\_fx\_localvol.py}, this is the amc-variant of example \ref{example:fx_localvol_detrates}
\end{itemize}
...
As a side product the AMC module provides plain MC pricing engines for Bermudan Swaptions.
...
\subsubsection{FX LocalVol exposure generation}
...
runs exposure simulation under a local vol model, this is the amc-variant of example \ref{example:fx_localvol_detrates}
