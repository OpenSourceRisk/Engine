...
This paper described how to implement the ORE compute framework Interface. An implementation of this interface can be
...
\verb+QuantExt/qle/math/openclenvironment.?pp+, which can can also be used as a template for new frameworks.
...
\item \verb+ComputeEnvironment+: This is a singleton exposing the compute frameworks ORE code that wishes to leverge an
  external compute environment. See \ref{ComputeEnvironment} for more details.
\end{itemize}

To give more context the typical usage of these interfaces from ORE engines is described in \ref{usageFromORE}.
...
The \verb+ComputeEnvironment+ is a thread local singleton exposes external compute frameworks to ORE code. A new compute
framework has to be registered to this singleton by instantiating a raw pointer the framework and adding it to the
...
\item \verb+getAvailableDevices()+: returns a set of descriptions of available devices for all registered frameworks. A
  description is a triplet \verb+OpenCL/Apple/AMD Radeon Pro 5500M Compute Engine+ where the first component identifies
  the framework, and the second and third component the concrete compute device exposed by that framework. Here, the
  second component is the name of the platform and the third the name of the device itself.
...
The fact that \verb+ComputeEnvironment+ is thread local means that each thread will access different instances of a
...
\item \verb+getAvailableDevices()+ should return the set of names of supported devices in that framework. By convention
  ...
  \end{itemize}
  Notice that the same device can appear under several frameworks, e.g. a GPU can be accessed both via an OpenCL driver
  or via a CUDA driver.
...
return an empty set. A call to \verb+getContext()+ should always throw an error.
...
\item \verb+init()+ initialize the context. This method is called every time \verb+selectContext()+ is called in
  \verb+ComputeEnvironment+ and should be used to do one-off set up work. For example \verb+OpenClContext+ will create
...
\item \verb+initiateCalculation()+: start a new calculation. The input parameter are:
  \begin{itemize}
  ...
  \item \verb+id+: The id of the calculation. If \verb+id+ is not \verb+0+ the steps from a previous calculation
    ...
    retrieve the results of the calculation straight away calling \verb+finalizeCalculation+. If \verb+id+ is \verb+0+
    on the other hand, a new calculation \verb+id+ will be generated and returned.
  \item \verb+version+: The version of a calculation.
  \item \verb+debug+: A flag indicating whether debug information should be collected.
  \end{itemize}
...
\begin{table}[ht]
  \begin{flushleft}
    \begin{tabular}{|l|l|l|}
      ...
      NormalCdf & 1 & normal cumulative distribut ion \\ \hline
...
where \verb+externalComputeDevice_+ is a string identifying the device. Whenever a new computation i.e. the (re)pricing
of a trade is requested a calculation is initiated
