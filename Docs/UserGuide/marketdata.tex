...
\centering
  ...
  \hline
    ...
    Forward start & An integer followed by D, W, M or Y, or an IMM date convention  & Period from today to start, or number of IMM dates from today to start  \\ \hline
    Term & An integer followed by D, W, M or Y, or an IMM date convention & Period from start to maturity, or number of IMM dates from today to maturity\\ \hline
  \end{tabular}
  \caption{FRA Rate}
  \label{tab:fra_quote}
\end{table}
The IMM date convention for ORE is `IMx' where x represents a number of IMM dates from today. 
If the number of dates is 9 or less we take x to be the number itself, otherwise we take x to be A for 10, B for 11, C for 12 or D for 13.
