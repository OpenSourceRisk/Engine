...
%\begin{table}
{\scriptsize
\begin{tabular}{|p{5.5cm}|p{6cm}|p{1.5cm}|p{1.5cm}|}
  \hline
  Option & Description & Default & Recommendation \\
  \hline
  \hline
  ORE\_BUILD\_DOC & Build PDF documentation & ON & ON \\
  \hline
  ORE\_BUILD\_TESTS & Build test suites  & ON & ON \\
  \hline
  ORE\_BUILD\_EXAMPLES & Create a testsuite of examples in ORE/Examples. Requires ORE\_BUILD\_TESTS to be ON. & ON & ON\\
  \hline
  ORE\_BUILD\_APP & Build the ORE commandline application {\tt ore[.exe]} & ON & ON \\
  \hline
  ORE\_BUILD\_SWIG & Build the ORE Python module & ON & ON \\
  \hline
  ORE\_BUILD\_QL\_SEPARATELY & Import separate pre-built QuantLib & OFF & OFF \\
  \hline
  ...
  \hline
  ORE\_USE\_ZLIB & Use compression for boost::iostreams, e.g. to write compressed cube files & OFF & ON \\
  \hline
  ...
  \hline
  MSVC\_LINK\_DYNAMIC\_RUNTIME & Link against dynamic runtime & ON & ON \\
  \hline
  MSVC\_PARALLELBUILD & Use flag /MP & ON & ON\\
  \hline
...
%    \lstinline!Roll QuantLib Flags! & %\begin{table}
{\scriptsize
\begin{tabular}{|p{5.5cm}|p{6cm}|p{3cm}|}
  ...
  \hline
  QL\_BUILD\_TEST_SUITE & Build test suites  & ON \\
  \hline
  QL\_BUILD\_FUZZ\_TEST\_SUITE & Build fuzz test suite & OFF \\
  \hline
  QL\_ENABLE\_OPEN\_MP & Detect and use OpenMP & OFF \\
  \hline
  QL\_ENABLE\_ENABLE\_PARALLEL\_UNIT\_TEST\_RUNNER & Enable the parallel unit test runner & OFF \\
  \hline
...
\end{table}
\caption{The value indicated should not be changed.}
