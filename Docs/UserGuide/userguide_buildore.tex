...
ORE uses the CMake build system with the following options and default setting (see ore/CMakeLists.txt):
...
\begin{tabular}{|p{5cm}|l||l|}
  ...
  \hline
  ORE\_BUILD\_APP & Build commandline application & ON \\
  \hline
  ORE\_BUILD\_SWIG & Build ORE Python & ON \\
  \hline
  ...
  \hline
  ORE\_PYTHON\_INTEGRATION & Build ORE with Python Integration& OFF \\
  \hline
...
So the default CMake build includes the ORE Python module already which can be used to run ORE Python locally as shown in
...
Note that there is a longer list of options named QL\_... and default settings in QuantLib's ore/QuantLib/CMakeLists.txt.
...
\medskip
    {\tt cmake ..  -D ORE\_USE\_ZLIB=ON  -D QL\_USE\_PCH=ON} \\
\medskip
