...
\item 32-bit: \\
  {\footnotesize\tt .{\bs}b2 -{}-stagedir=.{\bs}lib{\bs}Win32{\bs}lib -{}-build-type=complete toolset=msvc-14.3 \bs \\
    address-model=32 -{}-with-test -{}-with-system  \bs \\
...
\item 64-bit: \\
  {\footnotesize\tt .{\bs}b2 -{}-stagedir=.{\bs}lib{\bs}x64{\bs}lib -{}-build-type=complete toolset=msvc-14.3 \bs \\
    address-model=64 -{}-with-test -{}-with-system \bs \\
    -{}-with-serialization -{}-with-regex -{}-with-date\_time stage}
\end{itemize}
\end{enumerate}

There are two options for pointing at your Boost installation:
\begin{enumerate}
\item With CMake 3.30.0+ and Boost 1.70.0+, set the environment variable {\tt ORE\_BOOST\_DIR} or {\tt BOOST\_DIR} to the directory containing the 
Boost CMake package configuration file i.e. the file BoostConfig.cmake. {\tt ORE\_BOOST\_DIR} is provided to allow for a selected version of
Boost for building ORE.
\item If you are still using the old FindBoost module for a particular reason (like building ORE-SWIG with setup.py), set the 
environment variables BOOST and {\tt BOOST\_LIB64} to point to the Boost root directory and Boost lib directory respectively, e.g:
    \begin{itemize}
...
ORE uses the CMake build system with the following essential parameters (see ore/cmake/commonSettings.cmake and
...
\begin{tabular}{|p{5.5cm}|p{6cm}|p{3cm}|}
  ...
  \hline
  ORE\_BUILD\_EXAMPLES & Create a testsuite of examples in ORE/Examples. Requires ORE\_BUILD\_TESTS to be ON. & ON \\
  \hline
  ORE\_BUILD\_APP & Build the ORE commandline application {\tt ore[.exe]} & ON \\
  \hline
  ORE\_BUILD\_SWIG & Build the ORE Python module & ON \\
  \hline
  ORE\_BUILD\_QL\_SEPARATELY & Import separate pre-built QuantLib & OFF \\
  \hline
  ...
  \hline
  ORE\_USE\_ZLIB & Use compression for boost::iostreams, e.g. to write compressed cube files & ON\textsuperscript{*} \\
  \hline
...
{\scriptsize \textsuperscript{*}If ON and using Windows, ensure your boost is built with zlib.}
...
\begin{tabular}{|p{5.5cm}|p{6cm}|p{3cm}|}
    ...
    \hline
    \hline
    QL\_BUILD\_Examples & Build examples & ON \\
    \hline
    QL\_BUILD\_TEST\_SUITE & Build test suites  & ON \\
    \hline
    QL\_USE\_PCH & Use precompiled headers & OFF \\
    \hline
    QL\_ENABLE\_SESSIONS & Singletons return different instances for different sessions; turn ON for ORE's multi-threading & ON\textsuperscript{*} \\
    \hline
    QL\_USE\_STD\_ANY & Use std::any instead of boost::any & ON\textsuperscript{*} \\
    \hline
    QL\_USE\_STD\_OPTIONAL & Use std::optional instead of boost::optional & ON\textsuperscript{*} \\
    \hline
    QL\_USE\_STD\_SHARED\_PTR & Use standard smart pointers intead of Boost ones & OFF\textsuperscript{*} \\
    \hline
    QL\_USE\_STD\_CLASSES & Enable all QL\_USE\_STD\_ options & OFF\textsuperscript{*} \\
    \hline
    QL\_USE\_INDEXED\_COUPON & Use indexed coupons instead of par coupons & OFF \\
    \hline
    QL\_ENABLE\_OPEN\_MP & Detect and use OpenMP & OFF \\
    \hline
    QL\_ENABLE\_PARALLEL \_UNIT\_TEST\_RUNNER & Enable the parallel unit test runner & OFF \\
    \hline
    QL\_ENABLE\_THREAD \_SAFE\_OBSERVER\_PATTERN & Enable the thread-safe observer pattern & OFF \\
    \hline
    QL\_ENABLE\_TRACING & Allow tracing messages & OFF \\
    \hline
    QL\_COMPILE\_WARNING \_AS\_ERROR & Specify whether to treat warnings on compile as errors. & OFF \\
    \hline
    QL\_ERROR\_FUNCTIONS & Error messages should include current function information & OFF \\
    \hline
    QL\_FASTER\_LAZY\_OBJECTS & Cause lazy objects to forward just the first notification instead of every one & ON \\
    \hline
    QL\_HIGH\_RESOLUTION\_DATE & Enable date resolution down to microseconds & OFF \\
    \hline

\end{tabular}
}
\scriptsize{ \textsuperscript{*}Default values indicated should not be modified.}
\scriptsize{ Boost::shared\_ptrs are preferred due to our definition of BOOST\_ENABLE\_ASSERT\_HANDLER. }
\scriptsize{ Changing QL\_USE\_STD\_OPTIONAL would require code modifications of all occurences of member function has\_value.}
...
\hspace{2cm} {\tt [ -D BOOST=<path/to/boost/> \bs ~] } \\
\hspace{2cm} {\tt [ -D BOOST\_LIB64=<path/to/boost/libraries> \bs ~] } \\
...
\medskip
Typical usage of the Python module is shown in ORE's {\tt Examples/ORE-Python} directory and described in user guide section
\ref{example:orepython}.

\subsection{ORE with Python Integration}\label{sec:orepythonintegration}

This section describes a new (still experimental) feature of ORE that supports calls from ORE into Python in order to utilise
some functionality that is readily available in Python so that we can avoid its implementation in C++ in ``core ORE'', e.g.
for testing purposes. An example is the use of regression methods provided in the {\tt scikit-learn} and {\tt py-earth2}
Python modules, as demonstrated in Example/InitialMargin where we have provided an option to utilise such methods
embedded into American Monte Carlo as alternatives for the usual polynomial regression provided by QuantLib.

Prerequisites for using this functionality are
\begin{itemize}
\item building ORE with the {\tt ORE\_PYTHON\_INTEGRATION} switch set to {\tt ON}, see section \ref{sec:build};
\item specifying the {\tt Python\_ROOT} and {\tt Python\_LIBRARY} variables when calling {\tt cmake}
  so that cmake finds the Python installation on your system, as in the case of building Python wheels \ref{sec:oreswig};
\item extending the {\tt PYTHONPATH} environment variable to contain directory {\tt ore/PythonIntegration}
  where ORE will be looking for the script {\tt ore\_python\_integration.py} that contains the Python functions that
  can be called from the C++ side of ORE.
\end{itemize}

See the Jupyter notebook Examples/InitialMargin/ore\_dynamicsimm.ipynb for a demonstration of this new feature.
