...
If sensitivity input is supplied, the same historical simulation VaR statistic can alternatively be computed from a sensitivity-based
historical PL vector. In this mode, ORE combines the supplied delta, gamma and cross-gamma sensitivities with the
historical scenario shifts, using the shift definitions from the sensitivity scenario configuration, and evaluates the
second order Taylor PL approximation described in \eqref{taylorPl2} for each historical observation period. The VaR
and Expected Shortfall are then computed from the empirical distribution of these sensitivity-based PL observations in
the same way as in the full revaluation case.

The \textbf{Expected Shortfall} (ES), also known as Conditional Value-at-Risk (CVaR), at confidence level $p$ is
defined as the expected loss given that the loss exceeds the VaR level. For the empirical distribution described above, this is
$$
\text{ES}(p) = \frac{1}{|\mathcal{L}(p)|}\sum_{j \in \mathcal{L}(p)} \pi_F(j) ,
$$
where $\mathcal{L}(p) = \bigl\{ j : \pi_F(j) \leq \text{VaR}(p) \bigr\}$ is the set of scenarios in the lower
tail at or below the VaR level. In other words, ES is the average of the P\&L realisations that fall at or below
the VaR at confidence level $p$.
...
The sensitivity-based Historical Simulation VaR path uses this Taylor PL vector with the historical scenario moves as
the risk factor returns. Trade-level and risk-factor-level PL reports, if requested, are decompositions of the same
Taylor PL observations used in the VaR calculation.
