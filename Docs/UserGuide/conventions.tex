...
A node with name \lstinline!CommodityFuture! is used to store conventions for commodity future contracts. These conventions are used in commodity derivative trades to calculate contract expiry dates. The structure of this node is shown in Listing \ref{lst:commodity_future_conventions}.
...
<CommodityFuture>
  ...
  <Calendar>...</Calendar>
  <ExpiryInPreviousMonth>...</ExpiryInPreviousMonth>
...
\item \lstinline!AnchorDay!: This node is used to give a date in the future contract month to use as a base date for calculating the expiry date. It can contain a \lstinline!DayOfMonth! node or an \lstinline!NthWeekday! node:
\begin{itemize}
...
\item \lstinline!Calendar!: The business day calendar(s) applicable for the commodity future contract.
\item \lstinline!ExpiryInPreviousMonth! [Optional]: The allowable values are \lstinline!true! and \lstinline!false!. If \lstinline!true!, the commodity future contract expires in the month prior to the contract month. If \lstinline!false!, the commodity future contract expiry date is in the contract month. If omitted, it defaults to \lstinline!false!.
...
<CommodityFuture>
  ...
  <ExpiryInPreviousMonth>true</ExpiryInPreviousMonth>
