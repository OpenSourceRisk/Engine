...
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Commodity Curve Conventions}
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
A node with name \emph{Commodity} is used to store conventions for commodity price quotes. The
structure of this node is shown in Listing \ref{lst:commodity_conventions}.
...
<Commodity>
  ...
  <AdvanceCalendar>USD</AdvanceCalendar>
  <SpotRelative>true</SpotRelative>
  <BusinessDayConvention>ModifiedFollowing</BusinessDayConvention>
  <Outright>false</Outright>
</Commodity>
\end{minted}
\caption{Commodity price conventions}
\label{lst:commodity_conventions}
\end{listing}
...
\emph{False} if the forward tenor is to be interpreted as being relative to the valuation date. If not provided, it
defaults to \emph{True}.
\item BusinessDayConvention: The Business Day Conventions.
\item Outright: \emph{True} if the forward quotes are to be interpreted as outright prices (e.g. Futures prices),
\emph{False} if they are infact forward points which must be added to the spot price to get the outright.
\end{itemize}
