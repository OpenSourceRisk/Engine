...
<CrossCurrencyBasis>
  ...
  <EOM> </EOM>
  <IsMtmResettable> </IsMtmResettable>
  <FlatIsResettableLeg> </FlatIsResettableLeg>>
</CrossCurrencyBasis>
\end{minted}
...
\item IsMtmResettable [Optional]: \emph{True} if the swap is a mark-to-market resetting, and \emph{False} otherwise. If not provided, it defaults to \emph{False}.
\item FlatIsResettableLeg [Optional]: \emph{True} if it is the flat leg that resets, and \emph{False} otherwise. If not provided, it defaults to \emph{True}.
\end{itemize}
