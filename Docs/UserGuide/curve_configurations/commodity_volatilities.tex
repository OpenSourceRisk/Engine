...
\item A calendar spread option volatility structure. This supports constant, one-dimensional (expiry dependent) and two-dimensional (expiry and absolute strike) normal volatility structures for commodity calendar spread options.
\end{itemize}
...
\item \lstinline!VolatilityType! [Optional]: The allowable values in general for \lstinline!VolatilityType! are \lstinline!Lognormal!, \lstinline!ShiftedLognormal! and \lstinline!Normal!.
\lstinline!Normal!, \lstinline!ShiftedLognormal! and \lstinline!Lognormal! are supported for constant and absolute strike surfaces. For all other types only \lstinline!Lognormal! is supported. \lstinline!Lognormal! is the default for \lstinline!VolatilityType! so this node may be omitted.
...
Indicates the interpolation in the time direction. The allowable values for \lstinline!TimeInterpolation! are \lstinline!Linear!, \lstinline!Cubic! and \lstinline!BackwardFlat!. If neither \lstinline!Strikes! nor \lstinline!Expiries! use the single wildcard value \lstinline!*!, \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! must have the same value. If it does not, then \lstinline!Linear! is used for both. In other words, if neither \lstinline!Strikes! nor \lstinline!Expiries! use the single wildcard value \lstinline!*!, we can configure bilinear or bicubic interpolation. If either \lstinline!Strikes! or \lstinline!Expiries! use the single wildcard value \lstinline!*!, \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! can be different. For backward compatibility, if either \lstinline!Strikes! or \lstinline!Expiries! use the single wildcard value \lstinline!*!, \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! will use Linear as default value or when input values are not in allowable list. Again, in all cases, the interpolation is carried out on the variance.
Alternatively, both \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! may be set to the same SVI model variant name (requiring \lstinline!PriceCurveId! and \lstinline!YieldCurveId!) to fit a parametric SVI surface instead of piecewise interpolation. Supported SVI variant names: \lstinline!Gatheral2004SviRaw!, \lstinline!Gatheral2004SviNatural!, \lstinline!Gatheral2004SviJw!, \lstinline!Gatheral2012SsviHeston!, \lstinline!Gatheral2012SsviPowerLaw!, \lstinline!CorbettaEtAl2019Essvi!, \lstinline!HendriksMartini2017EssviFirstPowerLaw!, \lstinline!HendriksMartini2017EssviSecondPowerLaw!, \lstinline!Mingone2022EssviGJ!, \lstinline!Mingone2022EssviMM!. See the SVI model documentation for details on parameters and calibration.

\item \lstinline!StrikeInterpolation!:
Indicates the interpolation in the strike direction. The requirements are exactly as outlined for the \lstinline!TimeInterpolation! node, including support for SVI model variant names.

\item \lstinline!ParametricSmileConfiguration! [Optional]:
Only relevant when \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are set to an SVI model variant. Allows the user to pin or warm-start individual model parameters and to control calibration tolerances. The sub-elements are: a \lstinline!Parameters! block listing \lstinline!Name!, \lstinline!InitialValue!, and \lstinline!Calibration! (\lstinline!Fixed!, \lstinline!Implied!, or \lstinline!Calibrated!) per parameter, and a \lstinline!Calibration! block with \lstinline!MaxCalibrationAttempts!, \lstinline!ExitEarlyErrorThreshold!, and \lstinline!MaxAcceptableError!.
...
\item \lstinline!TimeInterpolation!:
Allowable values are \lstinline!Linear!, \lstinline!Cubic!, or an SVI model variant name. When both \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are set to the same SVI variant, a parametric SVI surface is fitted (requiring \lstinline!PriceCurveId! and \lstinline!YieldCurveId!). See the \lstinline!StrikeSurface! section above for the list of supported SVI variant names.

\item \lstinline!StrikeInterpolation!:
Allowable values are \lstinline!Linear!, \lstinline!NaturalCubic!, \lstinline!FinancialCubic!, \lstinline!CubicSpline!, or an SVI model variant name. When set to an SVI variant, \lstinline!TimeInterpolation! must be set to the same variant.
...
\item \lstinline!TimeInterpolation!:
Allowable values are \lstinline!Linear!, or an SVI model variant name. When both \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are set to the same SVI variant, a parametric SVI surface is fitted (requiring \lstinline!PriceCurveId! and \lstinline!YieldCurveId!). See the \lstinline!StrikeSurface! section above for the list of supported SVI variant names.

\item \lstinline!StrikeInterpolation!:
Allowable values are \lstinline!Linear!, \lstinline!NaturalCubic!, \lstinline!FinancialCubic!, \lstinline!CubicSpline!, or an SVI model variant name (see \lstinline!StrikeSurface! for supported names). When set to an SVI variant, \lstinline!TimeInterpolation! must be set to the same variant.
...
Listing \ref{lst:comm_vol_calspread_config} outlines the configuration for a commodity calendar spread option volatility structure. Calendar spread options are options on the price difference between two consecutive futures contracts for the same commodity. The volatility structures used here are \emph{normal} (Bachelier) volatilities since the calendar spread can take negative values. The supported inner volatility configurations are \lstinline!Constant!, \lstinline!Curve! and \lstinline!StrikeSurface! --- moneyness, delta and APO configurations are not supported for calendar spread option volatilities.

\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<CommodityVolatility>
  <CurveId>ICE:B_CALENDAR_SPREAD_1</CurveId>
  <CurveDescription>...</CurveDescription>
  <Currency>USD</Currency>
  <InstrumentType>COMMODITY_CALENDAR_SPREAD_OPTION</InstrumentType>
  <CalendarSpreadOffset>1</CalendarSpreadOffset>
  <CalendarSpreadUnderlyingName>ICE:B</CalendarSpreadUnderlyingName>
  <StrikeSurface>
    <QuoteType>ImpliedVolatility</QuoteType>
...
\end{minted}
\caption{Calendar spread option volatility configuration}
\label{lst:comm_vol_calspread_config}
\end{longlisting}

The meaning of the calendar spread specific elements is given below. All other common elements (\lstinline!DayCounter!, \lstinline!Calendar!, \lstinline!FutureConventions!, etc.) have the same meaning as described for the previous configurations.
\begin{itemize}

\item \lstinline!CurveId!:
The curve identifier must follow the naming convention \lstinline!{underlying}_CALENDAR_SPREAD_{offset}! where \lstinline!{underlying}! is the name of the underlying commodity (matching \lstinline!CalendarSpreadUnderlyingName!) and \lstinline!{offset}! is a positive integer (matching \lstinline!CalendarSpreadOffset!). For example, \lstinline!ICE:B_CALENDAR_SPREAD_1! represents the first-nearby calendar spread for ICE Brent.

\item \lstinline!InstrumentType!:
Must be set to \lstinline!COMMODITY_CALENDAR_SPREAD_OPTION! to indicate that this volatility configuration is for calendar spread options. The default value is \lstinline!COMMODITY_OPTION! (i.e.\ standard commodity options), so this node must be explicitly provided for calendar spread configurations.

\item \lstinline!CalendarSpreadOffset!:
A positive integer indicating the futures contract offset for the calendar spread. A value of \lstinline!1! means the spread is between the $N$-th and $(N+1)$-th futures contracts. This value must match the offset embedded in the \lstinline!CurveId!.

\item \lstinline!CalendarSpreadUnderlyingName!:
The name of the underlying commodity for which the calendar spread is defined. This must match the underlying portion of the \lstinline!CurveId!. For example, if \lstinline!CurveId! is \lstinline!ICE:B_CALENDAR_SPREAD_1!, then \lstinline!CalendarSpreadUnderlyingName! must be \lstinline!ICE:B!.

\item \lstinline!QuoteType! / \lstinline!VolatilityType!:
Only \lstinline!Normal! volatility (\lstinline!RATE_NVOL! quote type) is supported for calendar spread option volatilities. This is enforced during validation.

\end{itemize}

The market data quote strings for calendar spread options follow the pattern:
\begin{center}
\lstinline!COMMODITY_CALENDAR_SPREAD_OPTION/RATE_NVOL/{N}/{O}/{C}/e_n/s_m!
\end{center}
where \lstinline!{N}! is the value in \lstinline!CalendarSpreadUnderlyingName!, \lstinline!{O}! is the \lstinline!CalendarSpreadOffset!, \lstinline!{C}! is the \lstinline!Currency!, and \lstinline!e_n! and \lstinline!s_m! are the expiry and strike respectively. For example, a quote for ICE Brent first-nearby calendar spread with USD currency might look like:
\begin{center}
\lstinline!COMMODITY_CALENDAR_SPREAD_OPTION/RATE_NVOL/ICE:B/1/USD/2026-03-31/-4.84!
\end{center}

Note that the \lstinline!FutureConventions! node is used to build a calendar spread forward price curve automatically from the underlying commodity price curve. This curve is used to compute ATM levels.
