...
\ref{lst:defaultcurve_from_yield}. A typical use case is to back out from a funding spread curve. 
...
on the given pillar times. Her, $R$ is the specified recovery rate.
...
\item RecoveryRate [optional]: The recovery rate for the resulting default curve. Defaults to zero. The recovery rate
  can be a market quote as usual or also a fixed numeric value for this curve type.
\item WrappedYieldCurve: The yield curve to wrap.
\item Extrapolation [Optional]: If set to true, the curve is extrapoalted beyond the last pillar. Defaults to true.
\end{itemize}
...
\begin{minted}[fontsize=\footnotesize]{xml}
    <DefaultCurve>
			<CurveId>FundingCurveCreditCurve</CurveId>
			<Currency>EUR</Currency>
			...
			<Configurations>
				<Configuration priority="0">
					...
					<WrappedYieldCurve>Yield/EUR/FundingCurve</WrappedYieldCurve>
					<Extrapolation>true</Extrapolation>
				</Configuration>
			</Configurations>
		</DefaultCurve>
