...
\begin{itemize}
  ...
  \item \lstinline!InterpolationVariable=Forward!: Interpolates instantaneous forward rates on pillar dates. Discount
  factors are computed by numerical integration of the forward rates. For $t < t_1$, uses flat forward rate from the
  first pillar.
\end{itemize}

All standard interpolation methods are supported (Linear, LogLinear, NaturalCubic, FinancialCubic, ConvexMonotone,
LogCubicSpline, MonotonicLogCubicSpline, Hermite, CubicSpline, Quadratic, LogQuadratic, LogNaturalCubic,
LogFinancialCubic, mixed interpolation methods, and BackwardFlat).
