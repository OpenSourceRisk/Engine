...
ensuring that $DF(t_0) = 1.0$ by definition at the reference date. This is useful when the first market pillar date 
does not coincide with the as-of date and you want to avoid including a synthetic discount factor point in the 
interpolation. Currently supported only for \lstinline!InterpolationVariable=Discount! with standard interpolation 
methods (Linear, LogLinear, NaturalCubic, FinancialCubic, ConvexMonotone, LogCubicSpline, MonotonicLogCubicSpline, 
Hermite, CubicSpline). If the element is omitted or left blank, then it defaults to \emph{False}, which preserves 
the traditional behavior of including a t0 point with $DF = 1.0$ in the interpolation input.
