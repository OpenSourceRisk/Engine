...
Whether the input market data are term cap volatilities or optionlet volatilities depends on the value of the \lstinline!InputType! node. This node may be set to \lstinline!TermVolatilities! for term cap volatilities or \lstinline!OptionletVolatilities! for optionlet volatilities.
...
\begin{enumerate}
\item If \lstinline!InterpolateOn! is set to \lstinline!TermVolatilities!, \lstinline!InterpolationMethod! is used to interpolate the term volatilities during the bootstrap and to interpolate the optionlet volatilities of the final optionlet surface. The fields \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are ignored in this case.
\item If \lstinline!InterpolateOn! is set to \lstinline!OptionletVolatilities!, \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are used to interpolate optionlet volatilities both during the bootstrap and on the final optionlet surface. \lstinline!InterpolationMethod! is ignored in this case.
\end{enumerate}

% ===========================================================================
% Term vol ATM, interpolation on term vols
% ===========================================================================

Listing \ref{lst:capfloorvol_atm_configuration_term} shows the layout for parameterising an ATM cap volatility curve
with interpolation on term volatilities. Nodes that have no effect for this parameterisation but that are allowed by the
schema are not referenced. The meaning of each of the nodes is as follows:
...
A valid interest rate index name giving the index underlying the cap floor quotes. This has to be an ibor, term or overnight index. Allowable values are given in the Table \ref{tab:indices}.
...
\item \lstinline!RateComputationPeriod! [Optional]:
For overnight indices, the caplet compounding period, e.g. 3M for USD-SOFR. Not relevant for ibor, term indices.

\item \lstinline!FlatFirstPeriod! [Optional]:
If true (default) the volatilities of the caplets that form the first par market cap are set to the same value. If false
and a global bootstrap is used, the caplet volatility at $t=0$ is included in the opimization and the caplet
volatilities of the first par market cap are interpolated between the value at $t=0$ and the next interpolation
node. Since the node at $t=0$ adds a degree of freedom, the parameter SmoothnessLambda should be set to a positive value
in the bootstrap config. See \label{sec:bootstrap_config}.

\item \lstinline!UseEffectiveVolatility! [Optional]:
Only applies to overnight caplets. If false (default), volatilities in the surface will be ``stripped volatilities'',
i.e. they will be placed on the first fixing date of the caplet and a decay adjustment will be applied when pricing the
caplet. If true, volatilities in the surface will be ``effective volatliities'', i.e. they will be placed on the last
fixing date of the caplet and the market black formula will be used to price such a caplet, using the last fixing date
to calculate the usual black variance, without decay adjustment.
...
<CapFloorVolatility>
  ...
  <RateComputationPeriod>...</RateComputationPeriod>
  <FlatFirstPeriod>...</FlatFirstPeriod>
  <UseEffectiveVolatility>...</UseEffectiveVolatility>
</CapFloorVolatility>
\end{minted}
...
% ===========================================================================
% Term vol ATM, interpolation on optionlet vols
% ===========================================================================
...
% ===========================================================================
% Term vol surface, interpolation on term vols
% ===========================================================================
...
% ===========================================================================
% Term vol surface, interpolation on optionlet vols
% ===========================================================================
...
\subsubsection{Proxy Cap Floor Volatility Configuration}
...
Listing \ref{lst:capfloorvol_proxy_configuration} shows the layout for parameterising a proxy cap floor volatility surface. The meaning of each of the nodes is as follows:
...
\item \lstinline!ProxyConfig!:
Contains the proxy configuration settings. This node is required for proxy configurations and should be omitted for direct cap floor volatility configurations.

\item \lstinline!Source! (within ProxyConfig):
Specifies the source cap floor volatility surface to use as the base for the proxy.

\item \lstinline!CurveId! (within Source):
The curve identifier of the source cap floor volatility surface that will be used as the base. This must reference an existing cap floor volatility curve configuration.

\item \lstinline!Index!:
A valid interest rate index name giving the index underlying the cap floor quotes. This has to be an ibor, term or overnight index. Allowable values are given in the Table \ref{tab:indices}.

\item \lstinline!RateComputationPeriod! [Optional] (within Source):
The rate computation period for the source index. This is only required for OIS or BMA/SIFMA indices and should be omitted for standard IBOR indices.

\item \lstinline!Target! (within ProxyConfig):
Specifies the target configuration for the proxy surface.

\item \lstinline!Index! (within Target):
The target interest rate index for which the proxy surface will provide volatilities. This allows you to create a volatility surface for one index based on the volatility surface of another similar index.

\item \lstinline!RateComputationPeriod! [Optional] (within Target):
The rate computation period for the target index. This is only required for OIS or BMA/SIFMA indices and should be omitted for standard IBOR indices.

\item \lstinline!ONCapSettlementDays! [Optional] (within Target):
Settlement days for overnight index cap structures. Only relevant when the target index is an overnight index.

\item \lstinline!ScalingFactor! [Optional] (within ProxyConfig):
...
\begin{enumerate}
\item Taking the base volatility surface from the specified source curve
\item Adjusting the volatilities for differences in forward rate levels between the source and target indices
\item Optionally applying the scaling factor to all volatilities
\item Producing an optionlet volatility structure suitable for pricing instruments on the target index
\end{enumerate}
...
\end{minted}
\caption{Proxy cap floor volatility configuration with scaling factor.}
\label{lst:capfloorvol_proxy_configuration}
\end{longlisting}
