...
date and the payment date. The payment lag applies to the coupons on the following legs:
  \begin{itemize}
  \item Fixed
  \item Equity
  \item Floating with Ibor, OIS, BMA or SIFMA indices
  \item CMS
  \item CMS Spread
  \item CPI
  \item Zero Coupon Fixed
  \end{itemize}

PaymentLag is not currently supported for:
  \begin{itemize}
  \item cap floor floating legs that have Ibor coupons with sub periods i.e.\ 
        where \lstinline!HasSubPeriods! is set to \lstinline!true!
  \item cap floor floating legs with averaged overnight coupons i.e.\ 
        where \lstinline!IsAveraged! is set to \lstinline!true!
  \item range accrual legs
  \item year on year inflation legs
  \end{itemize}
