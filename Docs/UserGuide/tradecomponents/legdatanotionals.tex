...
Allowable values:  \emph{Fixed, Floating, Cashflow, CMS, CMB, DigitalCMS, DurationAdjustedCMS, CMSSpread, DigitalCMSSpread, Equity, CPI, YY, ZeroCouponFixed, FormulaBased, CommodityFloating, CommodityFixed, EquityMargin, RangeAccrual} 
...
If left blank or omitted, it defaults to the calendar in the \lstinline!ScheduleData! node,
unless \lstinline!LegType! is \lstinline!Floating! and \lstinline!Index! is OIS,
in which case this defaults to the index fixing calendar.
If the PaymentCalendar defaults to the calendar in the \lstinline!ScheduleData! node, 
as is the case for non-OIS indices on floating legs, and no calendar is set in ScheduleData 
(for dates-based schedules the calendar field is optional), the \lstinline!NullCalendar! is used.
...
Allowable values: Usually a number of days but any valid period is allowed.
...
Allowable values: \lstinline!BusinessDays! or \lstinline!CalendarDays!. Defaults to \lstinline!BusinessDays! if omitted.
...
\item \lstinline!NotionalPaymentLag! [optional]: The notional payment lag, in days, applied to any notional exchanges.

Allowable values: Any integer that makes economic sense.
In most cases, if non-zero, it is a small positive number of days.
Defaults to zero if left blank or omitted.

\item \lstinline!NotionalPaymentLagUnit! [optional]:
This setting interacts with \lstinline!NotionalPaymentLag! in the same way that \lstinline!PaymentLagUnit! interacts 
with \lstinline!PaymentLag!. See the relevant bullet points above for a description and allowable values.

\item \lstinline!NotionalPaymentLagAnchor! [optional]:
This setting interacts with \lstinline!NotionalPaymentLag! in the same way that \lstinline!PaymentLagAnchor! interacts 
with \lstinline!PaymentLag!. See the relevant bullet points above for a description and allowable values.
...
\item ScheduleData: This is a trade component sub-node outlined in section \ref{ss:schedule_data} Schedule Data and
  Dates.
\item \lstinline!SettlementData! [Optional]: This node is used to specify the settlement of the cash flows in a currency other than the commodity underlying currency. Only used for \emph{CommodityFixed} and \emph{CommodityFloating} leg types. This node will be ignored for other leg types.

A \lstinline!SettlementData! node is shown in Listing \ref{lst:leg_data_settlement_data_node}, and the meanings and allowable values of its elements follow below.

\begin{itemize}
\item \lstinline!FXIndex!: The FX reference index for determining the FX fixing at the value date. The leg NPV will be observed in the commodity underlying currency, and then converted to \lstinline!Currency! with the \lstinline!FXIndex! using an FX fixing on \lstinline!FixingDate!. \\
...
\item \lstinline!FixingDate! [Optional]: The date on which the \lstinline!FXIndex! is observed. Payment date will be used if not given.
...
\begin{listing}[H]
  ...
  \end{minted}
  \caption{Example \lstinline!SettlementData! node with for commodity swap}
  \label{lst:leg_data_settlement_data_node}
\end{listing}
