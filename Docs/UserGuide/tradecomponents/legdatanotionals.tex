...
\lstinline!ScheduleData! trade component sub-node, an optional \lstinline!Indexings! sub-node and a sub-node that depends on
...
\item PaymentLag [optional]: The payment lag given as as number business days, applies to fixed, Ibor and OIS legs. PaymentLag is not supported for the CapFloor trade type.
...
Allowable values for \lstinline!NotionalInitialExchange!,  \lstinline!NotionalFinalExchange! and \lstinline!NotionalAmortizingExchange!:   \emph{true, false}. 
If the  \lstinline!Exchanges! node is omitted then all values default to \emph{false}, but if it is included \lstinline!NotionalInitialExchange! and \lstinline!NotionalFinalExchange! default to \emph{true}.
It is recommended that all values are explicitly set.
...
After the \lstinline!Notional! sub-node the \lstinline!LegData! node includes a \lstinline!ScheduleData! sub-node, an optional
\lstinline!Indexings! sub-node and a sub-node based on the choice of \lstinline!LegType! as per below:

\item ScheduleData: This is a trade component sub-node outlined in section \ref{ss:schedule_data} Schedule Data and
Dates.
\item Indexings [Optional]: This is a trade component sub-node outlined in section \ref{ss:indexings}. It is optional
  and applies only to certain leg types: Fixed, Floating, CMS, CMSSpread, DigitalCMSSpread
