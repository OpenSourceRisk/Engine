...
\item
\lstinline!PowerLoadProfileData! [Optional]: Inline load profile data. If both \lstinline!PowerLoadProfileReference! and \lstinline!PowerLoadProfileData! are provided, the reference takes precedence. If neither is given, it assumes a constant load over 24h everyday.

\lstinline!PowerLoadProfileData! supports two mutually exclusive representations:
\begin{itemize}
\item \lstinline!ExplicitDates!: Explicit load profiles per date. The load factors will be forward flat filled. On a missing date it will use the factors on the last day before the delivery date.
\item \lstinline!BusinessDayRules!: Rule-based load profiles using a calendar, with separate factors for business and non-business days. Rules are forward-flat filled. For a delivery date, the rule with the latest effective date less than or equal to the delivery date is applied.
\end{itemize}

In both cases, load is defined with one or more \lstinline!LoadFactor! elements. Each \lstinline!LoadFactor! has required attributes \lstinline!from! and \lstinline!to!, an optional attribute \lstinline!unit! (defaults to seconds if omitted), and optional attribute \lstinline!dst! to mark the extra hour on a daylight saving backward adjustment date.

\item
\lstinline!LoadFactor! [within \lstinline!PowerLoadProfileData!]: Defines the relative load level on a time interval. The element value is the load factor multiplier and should be non-negative. The attributes \lstinline!from! and \lstinline!to! define the time interval bounds in the given \lstinline!unit!; if \lstinline!unit! is omitted, seconds are assumed. The optional \lstinline!dst! flag can be used to for the factor during the extra hour between 2am and 3am on daylight saving backward-shift dates.
...
<PowerLoadProfileData>
  <ExplicitDates>
    <LoadProfileDatum>
      ...
      <LoadFactors>
        <LoadFactor from="1" to="24" unit='HOUR' dst="N">1.0</LoadFactor>
      </LoadFactors>
    </LoadProfileDatum>
  </ExplicitDates>
</PowerLoadProfileData>
