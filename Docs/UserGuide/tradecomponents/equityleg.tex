...
\begin{itemize}
\item Quantity[Mandatory if FXTerms is used (except if NotionalReset is \emph{false}), Optional otherwise]: 
\\ The number of shares. Either a Notional or the Quantity must be given for the leg, but not both at the same time. Note that when FXTerms is used and NotionalReset is set to  \emph{true}, Quantity must be given, and Notional cannot be used.
...
\item InitialPrice [Mandatory for forward starting, Optional otherwise]: Initial Price of the equity, if not present, the first valuation date is used to
...
Allowable values: Any positive real number. If omitted or left blank it defaults to the equity price of the fixing at the valuation date associated with the start date. Note that when this valuation date is in the future there is no fixing, and in these cases there is no default and the InitialPrice must be set.
