...
\begin{itemize}
\item Quantity[Mandatory if FXTerms is used, Optional otherwise]: The number of shares. Either a Notional or the Quantity must be given for the leg, but not both at the same time. Note that when FXTerms is used, Quantity must be given, and Notional cannot be used.
...
Allowable values: Any positive real number. If omitted or left blank it defaults to the price of the fixing at the valuation date associated with the start date. Note that when this valuation date is on the current date or in the future there is no fixing, and in these cases there is no default and the InitialPrice must be set.
