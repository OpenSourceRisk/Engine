...
\begin{itemize}
\item Quantity: The number of shares. Optional. If given, no notional must be specified on the equity leg. If not given,
  a notional must be specified and the quantity is computed from the initial notional of the leg as

  quantity = initial notional / (initial price * initial FX conversion rate)

  where the initial price is the share price on the first valuation date and the fx rate is conversion rate from the equity
  currency to the leg currency on the first valuation date (if applicable).
...
\item NotionalReset [Optional]: Defaults to \emph{true}.  Notional resets only affect the equity leg. If NotionalReset
  ...
  notional is taken to be the notional specified in the leg or - if a quantity is given - to be

  quantity x initial price x initial FX conversion rate

  where initial price and inital FX conversion rate are the share price and FX conversion rate (if applicable) at the
  first valuation date of the leg.
