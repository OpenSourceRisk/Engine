...
in Listing \ref{lst:float_zero_coupon_leg}. Note that the \lstinline!Tenor! in the rules-based schedule is not used when \lstinline!Rule! is set to \emph{Zero}.
\begin{listing}[H]
%\hrule\medskip
\begin{minted}[fontsize=\footnotesize]{xml}
            <LegData>
...
\end{minted}
\caption{Zero Coupon Floating Leg}
\label{lst:float_zero_coupon_leg}
\end{listing}
