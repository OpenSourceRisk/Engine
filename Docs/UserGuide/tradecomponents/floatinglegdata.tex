...
\item FixingDays [Optional]: This is the fixing gap, i.e. the number of business days
  before the period end date an index fixing is observed.   

Allowable values:  Positive integers.  Defaults to the index's fixing days if blank or omitted.

\item Lookback [Optional]: A period by which the fixing schedule of (averaged, compounded) OIS legs is shifted into the
  past.

Allowable values: any valid period, e.g. 2D, 3M, 1Y

\item RateCutoff [Optional]: The number of fixing dates at the end of the fixing period for which the fixing value is
  held constant and set to the previous value. Only applicable to OIS legs, defaults to $0$ for compounded and $2$ for
  averaged OIS legs.
