...
\item \lstinline!StrictResetDates! [Optional]: If given and set to true, the reset dates derived from FixingSchedule and
  FixingDays or given by ResetSchedule are interpreted as strict, meaning that they apply even if falling within a
  calculation (accrual) period. If false, the reset becomes effective in the next calculation period.
