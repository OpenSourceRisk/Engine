...
\item BaseCPIBase [Optional]: Indicates whether the \emph{BaseCPI} value is already adjusted for all rebasing events from the start date to the evaluation date or not.

If \emph{Current} (the default), the \emph{BaseCPI} is already adjusted for all rebasing events up to today and can be used as-is.

If \emph{StartDate}, the \emph{BaseCPI} corresponds to the index level at the \lstinline!StartDate! and will be automatically adjusted for all rebasing events between start date and today.

Allowable values:  \emph{Current} [default], \emph{StartDate}. 
...
values, the cap will be kept at the value of last entered spread for the
