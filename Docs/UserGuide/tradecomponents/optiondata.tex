...
\item SettlementData [Optional]: Only relevant for cash‐settled options. Converts the payoff into a currency different from the option's own currency. It specifies the payment currency, the FX index to perform the conversion, and the date on which the FX rate is fixed. Its XML form is shown in listing \ref{settlement_data_cash_settled_options}.
