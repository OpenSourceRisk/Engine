...
\item PayoffType [Optional, except for trade types detailed below]: Specifies a detailed payoff type for exotic options. Only applicable to specific trade types as
  ...
  Allowable values:
  \begin{itemize}
  \item Accumulator, Decumulator (applies to trade types EquityAccumulator, FxAccumulator, CommodityAccumulator only)
  \item TargetFull, TargetExact, TargetTruncated (applies to trade types EquityTaRF, FxTaRF, CommodityTaRF only)
  \item BestOfAssetOrCash, WorstOfAssetOrCash, MaxRainbow, MinRainbow (applies to trade types EquityRainbowOption,
    FxRainbowOption, CommodityRainbowOption only)
  \item Vanilla, Asian, AverageStrike, LookbackCall, LookbackPut (applies to trade types EquityBasketOption,
