...
\item SettlementData [Optional]: Only relevant for cash settled options. Converts the payoff into a currency different from the option's own currency. It specifies the payment currency, the FX index to perform the conversion, and the date on which the FX rate is fixed. Its XML form is shown in listing \ref{lst:settlement_data_cash_settled_options}.

\begin{itemize}
  \item \lstinline!PayCurrency!: Currency in which the option payoff will be settled.
  \item \lstinline!FXIndex!: FX index used to convert from the option currency into the settlement currency.
  \item \lstinline!FixingDate! [Optional]: Date on which the FX rate is taken. If omitted, the engine will use the index convention to derive the fixing date from the payment date.
  It will subtract **FixingDays** from the payment date and adjust the resulting date for the holidays according to the fx index conventions.
\end{itemize}
...
\end{minted}
\caption{SettlementData node for cash settled options}
\label{lst:settlement_data_cash_settled_options}
\end{listing}
