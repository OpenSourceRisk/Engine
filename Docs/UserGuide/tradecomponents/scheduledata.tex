...
A \lstinline!ScheduleData! object has a \lstinline!name!. The default \lstinline!name! for a \lstinline!ScheduleData! 
object created from a \lstinline!ScheduleData! XML node is the node name itself i.e.\ \lstinline!ScheduleData!. This 
name can be overridden via an optional \lstinline!name! attribute on the \lstinline!ScheduleData! node. The main 
reason to do this would be so that you can refer to it in the \lstinline!BaseSchedule! node of a \lstinline!Derived! 
section using a more appropriate name. Listing \ref{lst:derived_pmt_schedule} shows the use of this in a 
\lstinline!LegData! node.
...
\begin{minted}[fontsize=\footnotesize]{xml}
<LegData>
  <!-- Leg data information omitted for brevity -->
  <ScheduleData name="accrual_schedule">
    <Rules>
      <StartDate>2026-03-05</StartDate>
      <!-- Remaining rules information omitted for brevity -->
    </Rules>
  </ScheduleData>
  <PaymentSchedule>
    <Derived>
      <BaseSchedule>accrual_schedule</BaseSchedule>
      <Shift>-2D</Shift>
      <!-- Remaining derived information omitted for brevity -->
    </Derived>
  </PaymentSchedule>
</LegData>
\end{minted}
\caption{Derived payment schedule in \lstinline!LegData!.}
\label{lst:derived_pmt_schedule}
\end{listing}
...
\item \lstinline!IncludeDuplicateDates! [Optional]: If set to \emph{false} the resulting schedule will have unique set of dates and all duplicates will be removed. Default to \emph{false}.
...
\item \lstinline!Shift! [Optional]:
The period offset to be applied to each date in the base schedule in order to obtain the derived schedule.
The period may be positive, for a forward shift of the base schedule, or negative,
for a backward shift of the base schedule.
...
\item \lstinline!ShiftUnit! [optional]:
Determines how the \lstinline!Shift! is used to calculate the derived date from the base schedule date.
If omitted, or \lstinline!BusinessDays! is given, the \lstinline!Shift! is applied in the usual way.
In other words, the anchor date is shifted by the period defined by the \lstinline!Shift! using the 
\lstinline!Calendar! and \lstinline!Convention!.
If \lstinline!CalendarDays! is given, the \lstinline!Shift! period must have day units i.e.\ must be an integer or an 
integer followed by a \lstinline!D!.
In this case, the anchor date is shifted by \lstinline!Shift! \emph{calendar} days.
If the resulting date is not a good business day with respect to the \lstinline!Calendar!,
it is adjusted using the \lstinline!Calendar! and \lstinline!Convention!.
The anchor date used, off which the shift is based, can be influenced by the \lstinline!ShiftAnchor! below.

Allowable values: \lstinline!BusinessDays! or \lstinline!CalendarDays!. Defaults to \lstinline!BusinessDays! if omitted.

\item \lstinline!ShiftAnchor! [optional]:
Determines the base or anchor date to use when shifting a date using the \lstinline!Shift!.
If omitted, or \lstinline!Adjusted! is given, the anchor date is the underlying base schedule date.
This base schedule date may have had adjustments applied.
If \lstinline!Unadjusted! is given, the anchor date is the underlying base schedule date 
