...
This trade component can be used to define the underlying entity for an Equity, Commodity or FX trade, but also an underlying interest rate index. It can be used for a single underlying, or within a basket with associated weight.
...
a commodity underlying, and in Listing \ref{lst:irunderlying} for an underlying interest rate index.
...
\item \lstinline!Type!: The type of the Underlying asset.
  ...
  Allowable values:  \emph{Equity}, \emph{FX}, \emph{Commodity}, \emph{IR}.
...
\item \lstinline!Name!:
 ...
 \emph{IR}: See Table \ref{tab:indices}
