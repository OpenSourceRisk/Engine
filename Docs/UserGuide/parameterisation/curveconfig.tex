...
\item Intraday power curves
\item Bond future volatility structures % done
\end{itemize}
...
\item Interpolation: Optional. Possible values: Linear, Hagan2002Lognormal, Hagan2002Normal, Hagan2002NormalZeroBeta,
  Antonov2015FreeBoundaryNormal, KienitzLawsonSwaynePde, FlochKennedy\footnote{FlochKennedy shows unstable results and
  is considered experimental. More work is needed.}. If not given, defaults to Linear.
\item ParametricSmileConfiguration: Optional. Applies to SABR only. If not given, default values are used. Allows to
  ...
  parameters. See Example 59 for how to configure single value and termstructures of sabr parameters for swaption and cap curve configs.
\item Extrapolation: Specifies the extrapolation behaviour in all dimensions. \\ Allowable values: {\tt Linear, Flat,
  None}
\item OutputVolatilityType: Optional, defaults to input volatility type. Possible values: Normal, Lognormal (alias for
  ShiftedLognormal, shift is taken from OutputShift if given, or input market data shift. Input market quotes will be
  converted to output volatility type and shift before building the vol surfaces, except for parametric models (SABR),
  where the model is calibrated to the input market data directly (without conversion) and the output volatility type
  and shift is handled by the calibrated parametric model. An early conversion of the input market data would not be an
  advantage in this case, we could only loose information.
\item OutputShift: Optional, defaults to input market data shift. Specifies the shift if OutputVolatilityType is
  Lognormal / ShiftedLognormal
\item ModelShift: Optional, defaults to input market data shift. Specifies the shift used for SABR model if applicable
...
\item An SVI model variant name (e.g.\ {\tt Mingone2022EssviGJ}) in case of SmileType {\tt Delta}, {\tt BFRR}, or
  {\tt Absolute}, to fit a parametric SVI surface. When an SVI variant is selected the intermediate delta/BFRR surface
  uses {\tt Linear} interpolation. See the SVI model documentation for supported variant names.
\end{itemize}
\item TimeInterpolation [Optional]: specifies whether time interpolation is done in volatility ({\tt V}, default) or
  variance ({\tt V2T})
\item ButterflyErrorTolerance [Optional]: max allowed error for broker butterfly match, defaults to 0.01 relative rmse
  of broker butterfly market premiums vs theoretical premiums
\item TimeWeighting [Optional]: if given, a time weighting scheme is applied for time interpolation, see the
  documentation of FxOptionTimeWeighting convention for more details
...
\item
\lstinline!TimeInterpolation!: interpolation in the option expiry direction. If either \lstinline!Strikes! or \lstinline!Expiries! are configured with a wildcard character, \lstinline!Linear! is used. If both \lstinline!Strikes! and \lstinline!Expiries! are configured explicitly, \lstinline!Linear! or \lstinline!Cubic! is allowed, or both may be set to the same SVI model variant name (see below) to fit a parametric SVI surface instead of piecewise interpolation. The value must always agree with the value for \lstinline!StrikeInterpolation!.

\item
\lstinline!StrikeInterpolation!: interpolation in the strike direction. Allowable values mirror those for \lstinline!TimeInterpolation!: \lstinline!Linear!, \lstinline!Cubic!, or an SVI model variant name. The value must agree with the value for \lstinline!TimeInterpolation!. When both fields are set to an SVI variant (e.g.\ \lstinline!CorbettaEtAl2019Essvi!), the surface is fitted using SVI parametric volatility. Supported SVI variant names: \lstinline!Gatheral2004SviRaw!, \lstinline!Gatheral2004SviNatural!, \lstinline!Gatheral2004SviJw!, \lstinline!Gatheral2012SsviHeston!, \lstinline!Gatheral2012SsviPowerLaw!, \lstinline!CorbettaEtAl2019Essvi!, \lstinline!HendriksMartini2017EssviFirstPowerLaw!, \lstinline!HendriksMartini2017EssviSecondPowerLaw!, \lstinline!Mingone2022EssviGJ!, \lstinline!Mingone2022EssviMM!.
...
\paragraph{Legacy Schema}
The legacy schema (still supported) uses a single block for quotes and conventions:
\begin{minted}[fontsize=\footnotesize]{xml}
<InflationCurve>
    <CurveId>USCPI_ZC_Swaps</CurveId>
    <CurveDescription>Estimation Curve for USCPI</CurveDescription>
...
\item {\tt Segments}: Allows multiple instrument/convention blocks.
\item {\tt Quotes}: Legacy (use instead segments). The instruments' market quotes from which to bootstrap the curve.
\item {\tt Conventions}: Legacy (use instead segments). The conventions applicable to the curve instruments.
...
\item {\tt InterpolationVariable}: Optional, the variable on which the interpolation of the zero inflation curve is
  performed. The allowable values \emph{ZeroRate} and \emph{PriceIndex}. If not provided, the default value is \emph{ZeroRate}. Ignored for year on year inflation curves.
\item {\tt InterpolationMethod}: Only relevant if InterpolationVariable is PriceIndex, allowed is \emph{Linear} and \emph{LogLinear}. Defaults to \emph{Linear}.
\end{itemize}

The segment block defines instruments to use for curve building, with their quotes and swap conventions.

\paragraph{Segment Schema}
\begin{itemize}
\item {\tt Conventions}: The conventions applicable to the segment's instruments.
\item {\tt Quotes}: The instruments' market quotes for the segment.
\end{itemize}
...
\paragraph{Notes}
\begin{itemize}
\item All swaps used in segments must reference the same inflation index name for curve building, but can differ in their swap conventions (e.g., lag, interpolation).
\item If zero coupon swap market quotes are given, but the type is set to YY, the zero coupon swap quotes will be
...
Thirdly, the volatility surface configuration layout is given in Listing \ref{lst:cdsvol_surface_config}. 
The nodes have the following meanings and supported values:
\begin{itemize}

\item
\lstinline!QuoteType! [Optional]: If not provided, it defaults to \lstinline!ImpliedVolatility!.
The only other allowed value is \lstinline!Premium!.
If \lstinline!ImpliedVolatility! is specified, the quotes used to build the surface are expected to be 
implied lognormal volatility quotes starting with \lstinline!INDEX_CDS_OPTION/RATE_LNVOL/!. These quotes are 
discussed further below.

If \lstinline!Premium! is specified, the quotes used to build the surface are expected to be 
option premium quotes of the form \lstinline!INDEX_CDS_OPTION/PRICE/[NAME]/[TERM]/[EXPIRY]/[STRIKE]/[TYPE]!,
where \lstinline![TYPE]! is \lstinline!P! for payer and \lstinline!R! for receiver.
Implied volatilities are derived from the option premia. The \lstinline!PriceInfo! node defined below must be provided 
in this case to supply additional information for the stripping process. Also, the \lstinline!Terms! node must contain 
the \lstinline!Maturity! node giving the maturity date of the underlying index CDS series of the given \lstinline!Term!.

If both payer and receiver premiums are quoted for the same expiry and strike, the smaller premium, i.e.\ the 
out-of-the-money option premium, is used in the stripping procedure.
During stripping, ORE calculates the forward price or spread from the market index CDS curve. In general, this forward 
will not exactly coincide with the forward implied by the quoted payer and receiver premiums through put-call parity.
Consequently, the volatility stripped to match the out-of-the-money premium will not, in general, exactly recover the 
premium of the corresponding in-the-money option at the same strike and expiry.
...
\item
\lstinline!TimeInterpolation!: interpolation in the option expiry direction. If either \lstinline!Strikes! or \lstinline!Expiries! are configured with a wildcard character, \lstinline!Linear! is used. If both \lstinline!Strikes! and \lstinline!Expiries! are configured explicitly, \lstinline!Linear! or \lstinline!Cubic! is allowed, or both may be set to the same SVI model variant name to fit a parametric SVI surface instead of piecewise interpolation. The value must always agree with the value for \lstinline!StrikeInterpolation!.

\item
\lstinline!StrikeInterpolation!: interpolation in the strike direction. Allowable values mirror those for \lstinline!TimeInterpolation!: \lstinline!Linear!, \lstinline!Cubic!, or an SVI model variant name. The value must agree with \lstinline!TimeInterpolation!. When both fields are set to an SVI variant, the surface is fitted using SVI parametric volatility. Supported SVI variant names: \lstinline!Gatheral2004SviRaw!, \lstinline!Gatheral2004SviNatural!, \lstinline!Gatheral2004SviJw!, \lstinline!Gatheral2012SsviHeston!, \lstinline!Gatheral2012SsviPowerLaw!, \lstinline!CorbettaEtAl2019Essvi!, \lstinline!HendriksMartini2017EssviFirstPowerLaw!, \lstinline!HendriksMartini2017EssviSecondPowerLaw!, \lstinline!Mingone2022EssviGJ!, \lstinline!Mingone2022EssviMM!.
...
\item
\lstinline!ParametricSmileConfiguration!: optional. Only relevant when \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are set to an SVI model variant. Allows the user to pin or warm-start individual model parameters and to control calibration tolerances. The sub-elements mirror those used for swaption parametric smile configuration (see Section~\ref{sss:swaptionconfig}): a \lstinline!Parameters! block listing \lstinline!Name!, \lstinline!InitialValue!, and \lstinline!Calibration! (\lstinline!Fixed!, \lstinline!Implied!, or \lstinline!Calibrated!) per parameter, and a \lstinline!Calibration! block with \lstinline!MaxCalibrationAttempts!, \lstinline!ExitEarlyErrorThreshold!, and \lstinline!MaxAcceptableError!.

\item
\lstinline!PriceInfo!: this node is optional and only relevant when \lstinline!QuoteType! is set to \lstinline!Premium!.
It is discussed further below.

\end{itemize}

When the CDS volatility surface is configured as in Listing \ref{lst:cdsvol_surface_config}, 
the market is searched for quote strings of the form \lstinline!INDEX_CDS_OPTION/RATE_LNVOL/[NAME]/[EXPIRY]/[STRIKE]! 
or \lstinline!INDEX_CDS_OPTION/RATE_LNVOL/[NAME]/[TERM]/[EXPIRY]/[STRIKE]!.
The population of the \lstinline![NAME]! field, and possibly the \lstinline![TERM]! field, 
and how they depend on the \lstinline!QuoteName! and \lstinline!ParseTerm! nodes has been discussed at length above 
when describing the deprecated CDS volatility curve configuration. 
When both the \lstinline!Strikes! and \lstinline!Expiries! are configured explicitly, 
it is clear that the \lstinline![EXPIRY]! field is populated from the list of expiries in turn 
and the \lstinline![STRIKE]! field is populated from the list of strikes in turn. 
If there are $m$ expiries in the \lstinline!Expiries! list and $n$ strikes in the \lstinline!Strikes! list, 
there will be $m \times n$ quotes created and searched for in the market data. 
If \lstinline!Expiries! are configured via the wildcard character, \lstinline!*!, 
all quotes in the market data matching the pattern \lstinline!INDEX_CDS_OPTION/RATE_LNVOL/[NAME]/*/[STRIKE]! 
will be used if \lstinline![TERM]! has not been populated and all quotes in the market data matching 
the pattern \lstinline!INDEX_CDS_OPTION/RATE_LNVOL/[NAME]/[TERM]/*/[STRIKE]! will be used 
if \lstinline![TERM]! has been populated. 
Similarly for \lstinline!Strikes! configured via the wildcard character, \lstinline!*!.
...
When \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! are both set to an SVI model variant name, the \lstinline!StrikeSurface! block supports an optional \lstinline!ParametricSmileConfiguration! node to warm-start or fix individual model parameters. Listing~\ref{lst:cdsvol_svi_config} shows an example using the Corbetta et~al.\ (2019) eSSVI surface variant for a CDS spread volatility surface.

\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<CDSVolatility>
  <CurveId>CDXIG5Y</CurveId>
  <CurveDescription>CDX IG 5Y CDS option vol surface — eSSVI</CurveDescription>
  <Terms>
    <Term>
      <Label>5Y</Label>
      <Curve>CDXIG/5Y</Curve>
    </Term>
    <Term>
      <Label>7Y</Label>
      <Curve>CDXIG/7Y</Curve>
    </Term>
  </Terms>
  <StrikeSurface>
    <Strikes>*</Strikes>
...
\end{minted}
\caption{CDS volatility SVI surface configuration (Corbetta eSSVI)}
\label{lst:cdsvol_svi_config}
\end{longlisting}

As mentioned above, for premium quotes, the volatility surface configuration must contain a \lstinline!PriceInfo! node 
as given in Listing \ref{lst:cdsvol_price_info}. The nodes have the following meanings and supported values:
\begin{itemize}

\item
\lstinline!CdsConventions!: The ID of a CDS conventions object for the underlying index CDS.

\item
\lstinline!RunningCoupon!: The running coupon of the underlying index CDS.
Generally \lstinline!0.01! for IG and \lstinline!0.05! for HY but any positive value is allowed here.

\item
\lstinline!IndexFactors! [Optional]: This is an optional node but it is needed it you are stripping volatilities for 
an off-the-run version of a particular index series. It is provided so that the correct underlying index CDS option 
can be built during the stripping process. The Listing has example values. The \lstinline!IndexFactor! node gives the 
current index factor and must be a value between 0 and 1. It is 1 minus the sum of the weight of each defaulted entity 
in the index up to the current date. The \lstinline!IndexFactorStrike! is the index factor for the version of the index 
for which we are stripping volatilities. It must be a value between 0 and 1 and will always be greater than or equal to 
the \lstinline!IndexFactor!. The \lstinline!RealisedFep! gives the realised front end protection fraction that has 
accrued up to the current date on the version of the index for which we are stripping volatilities.
If this \lstinline!IndexFactors! node is not provided, this information is derived from the reference data for the 
credit index. The reference data needs to have the optional field \lstinline!IndexVersion! set for this to be possible.

\item
\lstinline!EngineOverride! [Optional]: The engine used to strip the volatilities can be overridden using this node.
If not provided, the engine used depends on the type of the volatility surface being stripped.
For a surface with strikes in terms of spread, the default engine is \lstinline!NumericalIntegrationEngine!.
For a surface with strikes in terms of price, the default engine is \lstinline!BlackIndexCdsOptionEngine!.
Allowable values for this node are \lstinline!NumericalIntegrationEngine! and \lstinline!BlackIndexCdsOptionEngine!.
The methodology for these pricers is described in the ORE Product Catalogue \cite{products}.

\item
\lstinline!QuoteDimension! [Optional]: For premium quotes on new versions of a given series the quote may be given in 
terms of the notional before the index factor is applied or after the index factor is applied.
For example, suppose we are on version 2 of an index and the index factor is 0.99. A premium quote of 50 bps for an 
option with notional 10,000 may mean an upfront payment of 50 or an upfront payment of 50 multiplied by the index 
factor of 0.99, i.e.\ 49.5.
For the former case, the \lstinline!QuoteDimension! should be set to \lstinline!BpsPerOptionNtl! and 
for the latter case, the \lstinline!QuoteDimension! should be set to \lstinline!BpsPerOutstandingNtl!.
If this node is not provided, it defaults to \lstinline!BpsPerOutstandingNtl!.

\item
\lstinline!OneDimSolverConfig! [Optional]: A configuration for the one dimensional solver used in deriving volatilities 
from prices. This node is described in detail in Section \ref{sec:one_dim_solver_config}.
If not provided, a default set of parameters is created in the code for price strike and spread strike surfaces.
...
\begin{minted}[fontsize=\footnotesize]{xml}
<PriceInfo>
  <CdsConventions>...</CdsConventions>
  <RunningCoupon>...</RunningCoupon>
  <IndexFactors>
    <IndexFactor>0.99</IndexFactor>
    <IndexFactorStrike>1.0</IndexFactorStrike>
    <!-- Defaulted entity had weight 0.01 and recovery 0.26625 -->
    <RealisedFep>0.0073375</RealisedFep>
  </IndexFactors>
  <EngineOverride>...</EngineOverride>
  <QuoteDimension>...</QuoteDimension>
  <OneDimSolverConfig>
...
\end{minted}
\caption{CDS volatility \lstinline!PriceInfo! node}
\label{lst:cdsvol_price_info}
\end{longlisting}


\subsubsection{Base Correlations}

Base correlations can be configured using either quoted base correlations directly or by implying them from quoted upfront quotes.
When using quoted base correlations, the market quotes are directly used to construct the base correlation curve.
Alternatively, base correlations can be implied from upfront quotes of tranches, which involves a calibration process to
derive the base correlation values that best fit the observed market prices of the tranches.


Listing \ref{lst:basecorr_configuration} shows the configuration of a Base Correlation curve from quoted correlations.
...
Listing \ref{lst:basecorr_upfront_configuration} shows the configuration of a Base Correlation curve from upfront quotes or base correlations if it fails to imply the base correlations.
...
\end{minted}
\caption{Base Correlation Configuration from Upfront Quotes}
\label{lst:basecorr_upfront_configuration}
\end{longlisting}

The meaning of each of the elements in Listing \ref{lst:basecorr_upfront_configuration} is given below.

\begin{itemize}
\item CurveId: Unique identifier of the base correlation structure.
\item CurveDescription [Optional]: A description of the base correlation structure, may be left blank.
\item Terms: Comma-separated list of tenors, sorted in increasing order, possibly a single term to represent a flat term structure in the time direction. A wildcard (*) can be used to include all terms.
\item DetachmentPoints: Comma-separated list of equity tranche detachment points, sorted in increasing order. A wildcard (*) can be used to include all detachment points.
\item SettlementDays: The floating term structure's settlement days argument used in the reference date calculation.
\item Calendar: The term structure's calendar used in tenor to date conversions.
\item BusinessDayConvention: The term structure's business day convention used in tenor to date conversion.
\item DayCounter: The term structure's day counter used in date to time conversions.
\item Extrapolate: Boolean to indicate whether the correlation curve shall be extrapolated or not.
\item QuoteName: The name of the quote used for the base correlation.
\item StartDate: The start date for the base correlation curve.
\item Rule: The rule used for the base correlation curve, e.g., CDS2015.
\item AdjustForLosses: Boolean to indicate whether to adjust for losses.
\item QuoteTypes: A priority list of quote types to use for the base correlation curve. If it fails to build a base correlation from the first quote type it will try the next one.
\item IndexSpread: The spread of the index.
\item Currency: The currency of the base correlation curve.
\item CalibrateConstituentsToIndexSpread: Relevant only when price quotes are used. Indicates if the constituents should be calibrated to the index spread before deriving the base correlations from the tranche prices.
\item UseAssumedRecovery: Boolean flag to indicate whether to use specialized default curves bootstrapped with assumed recovery rates (mean of the stochastic recovery rate distribution). These curves have following the naming convention
\_\_CDO\_CurveName\_\&\_REC\_0.30\_\&\_ and are used to derive the base correlations.
\item RecoveryGrid: Specifies the recovery rates for different seniorities of the constituent. Use wildcard * to apply one grid for all seniorities.
\item RecoveryProbabilities: Specifies the probabilities of the recovery rates for different seniorities. Use wildcard * to apply one grid for all seniorities.
\end{itemize}
...
\item an optional ConversionFactor. In case of a bond future, a conversion factor might be applied. The application requires
  a clean price settlement. Otherwise the default value of 1.0 for the conversion factor will be used.
\item an optional PriceQuote. If the bond spread imply analytics is available, this is used to imply a SpreadQuote to
  match a given bond price. Notice that if a SpreadQuote is given explicitly in the market data, this will override an
  implied spread.
\end{itemize}
...
Note: in case of a forward bond, the convention for security specification requires the format \emph{securityId\_FWDEXP\_expiryDate}.
...
\input{curve_configurations/bond_future_volatilities}
