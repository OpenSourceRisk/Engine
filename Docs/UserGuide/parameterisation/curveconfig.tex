...
\item
\lstinline!InterpolationModel!: optional. When set to a supported SVI model variant name, the strike surface is fitted using SVI parametric volatility instead of piecewise interpolation. Supported values: \lstinline!Gatheral2004SviRaw!, \lstinline!Gatheral2004SviNatural!, \lstinline!Gatheral2004SviJw!, \lstinline!Gatheral2012SsviHeston!, \lstinline!Gatheral2012SsviPowerLaw!, \lstinline!CorbettaEtAl2019Essvi!, \lstinline!HendriksMartini2017EssviFirstPowerLaw!, \lstinline!HendriksMartini2017EssviSecondPowerLaw!, \lstinline!Mingone2022EssviGJ!, \lstinline!Mingone2022EssviMM!. The per-slice variants (Raw, Natural, JW) calibrate each expiry~$\times$ tenor slice independently. The surface variants (SSVI Heston/PowerLaw, Corbetta, HendriksMartini, Mingone) impose global shape constraints across slices. For CDS spread volatility surfaces the default quote type is normal volatility.

\item
\lstinline!ParametricSmileConfiguration!: optional. Only relevant when \lstinline!InterpolationModel! is set. Allows the user to pin or warm-start individual model parameters and to control calibration tolerances. The sub-elements mirror those used for swaption parametric smile configuration (see Section~\ref{sss:swaptionconfig}): a \lstinline!Parameters! block listing \lstinline!Name!, \lstinline!InitialValue!, and \lstinline!Calibration! (\lstinline!Fixed!, \lstinline!Implied!, or \lstinline!Calibrated!) per parameter, and a \lstinline!Calibration! block with \lstinline!MaxCalibrationAttempts!, \lstinline!ExitEarlyErrorThreshold!, and \lstinline!MaxAcceptableError!.
...
When \lstinline!InterpolationModel! is specified the \lstinline!StrikeSurface! block accepts two additional optional nodes: \lstinline!InterpolationModel! (the SVI variant name) and \lstinline!ParametricSmileConfiguration!. The \lstinline!TimeInterpolation! and \lstinline!StrikeInterpolation! nodes are ignored in this case. Listing~\ref{lst:cdsvol_svi_config} shows an example using the Corbetta et~al.\ (2019) eSSVI surface variant for a CDS spread volatility surface.
...
<CDSVolatility>
  ...
  <StrikeSurface>
    <Strikes>*</Strikes>
    <Expiries>*</Expiries>
    <InterpolationModel>CorbettaEtAl2019Essvi</InterpolationModel>
    <ParametricSmileConfiguration>
      <Calibration>
        <MaxCalibrationAttempts>10</MaxCalibrationAttempts>
        <ExitEarlyErrorThreshold>0.005</ExitEarlyErrorThreshold>
