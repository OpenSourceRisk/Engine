...
In case of fx volatility shifts, the ATM shifts and expiries can be given explicitly similar to the sensitivity config (see \ref{lst:stress_config_fxvol_explict}), or 
a single shift with an weighting schema (see Example listing \ref{stress_config_fxvol_weighting}). In case of the explicit schema, if only one tenor and shift is given a
parallel shift will be performed. There are two weighting schemas supported:
\begin{itemize}
  \item {\\tt Unadjusted}: The shift will be applied only to the given tenor, all other tenors are kept unadjusted.
  \item {\\tt Weighted}: The given shift $s_i$ will be applied to the given tenor $i$, the shifts for all other tenors will be derived 
  from by the shift and the given shifts $s_j = s_i \frac{w_j}{w_i}$.
\end{itemize}
