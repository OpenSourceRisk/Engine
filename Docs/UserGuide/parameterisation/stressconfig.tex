...
configuration, see \ref{sec:sensitivity}. In addition to the ``Absolute`` and ``Relative`` shift types defined in sensitivity 
configuration, stress tests allow an ``EqualTo`` shift type. In this case, the user provides the values that the curve should
be shifted to. In the case of Discount, Yield, Index curves, and Survival Probabilities, an additional field ``ShiftingZeros``
is required to indicate if the given shifted values are zero rates (true) or discount rates (false).

Components that should not be shifted, can just be omitted in the definition
...
FX volatility stress test only works for ATM curves and not with FX surfaces in the simulation market.
If in simulation market the pair is set up to simulate a surface, an error message will be shown and the shifts will be ignored.
...
A date shift or target date can be specified in the optional ``Date'' field. In case of a shift (e.g. 1D ,3M , ...) the
evaluation date is shifted using the given period, no calendar adjustment is applied. In case of a date, the evaluation
date is set to the given target date.
...
\input{parameterisation/stressconfig_intradaypowercurves}
