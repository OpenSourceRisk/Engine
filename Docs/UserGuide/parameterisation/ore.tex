...
For both analytics ORE reuses the parameterisations of the exposure and XVA analtyic. One need to setup those analytics as usual but one can deactivate them. See listings 
see listings \ref{lst:ore_xva} and \ref{lst:ore_simulation}. If set to active, ORE will run the 
regular XVA analytic and the XVA sensitivity or XVA stress analytic respectively.

\medskip The {\tt XVA explain} 'analytic' provide the computation of the market implied changes of the value adjustment 
changes $XVA(t_0, marketdata(t_1)) - XVA(t_0, marketdata(t_0))$ between to evaluation dates. Listing \ref{lst:ore_xva_explain}
...
ORE reuses the parameterisations of the exposure and XVA analtyic. One need to setup those analytics as usual but one can deactivate them. See listings see listings 
