...
\item stress
\item xva stress
\item xva bump and revaluation sensitivities
\item xva explain
\end{itemize}
...
results in place of the initial values for all parametrizations covered so far (IR and FX). In a subsequent run one could replace
the {\tt CrossAssetModel} section in {\tt simulation.xml} with the output in {\tt calibration.xml} to re-run without re-calibration.
Note that the {\tt Calibrate} flags in the IR/FX model sections of {\tt calibration.xml} are set to {\tt false}.
...
ORE reuses the parameterisations of the exposure and XVA analtyic. One need to setup those analytics as usual but one can deactivate them. See listings 
