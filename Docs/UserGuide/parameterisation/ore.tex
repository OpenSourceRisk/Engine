...
<Setup>
  ...
  <Parameter name="gzipCompressionLevel">6</Parameter>
</Setup>
\end{minted}
%\hrule
...
them with pricing engines as specified in {\tt pricingengine.xml}. The counterparty information in {\tt counterparty.xml} covers
minimal counterparty-level information needed in a few capital analytics as shown in example sections \ref{example:marketrisk} and \ref{example:creditrisk}.
...
\medskip Optional parameter {\tt gzipCompressionLevel} specifies the gzip compression level to use when writing zipped cube files. Allowed values are integers between {\tt 0} and {\tt 9}, where {\tt 0} means no compression (fastest write time, largest file size) and {\tt 9} means maximum compression (slowest write time, smallest file size). If not given, the parameter defaults to {\tt 6} (recommended balance of speed and size). An error is raised if the value is outside the range {\tt 0}--{\tt 9}.
...
\item Exposure Simulation, Model Calibration, Scenario Generation
...
240 monthly time steps from today. The optional key {\tt calendar} defines the calendar that is used to roll out the
grid, it defaults to TARGET. The discount factors for all curves with configuration default will be exported on this
monthly grid into the csv file specified by key {\tt outputFileName}. The grid can also be specified explicitly by a
comma separated list of tenor points such as {\tt 1W, 1M, 2M, 3M, \dots}.

The {\tt outputTodaysMarketCalibration} parameter controls whether calibration information for the built market objects
is written to a report. When set to Y, a detailed calibration report is generated. The {\tt todaysMarketCalibrationPrecision}
parameter specifies the number of decimal places to use when formatting floating-point values in the market calibration
report (default: 8). This allows users to control the precision of numerical output in the calibration report, similar to
the {\tt additionalResultsReportPrecision} parameter for the NPV analytic.
...
<Analytics>
  <Analytic type="simulation">
    ...
    <Parameter name="filterRiskKeys">DiscountCurve</Parameter>
...
The 'filter risk keys' is an optional parameter that filters out all but the risk factor key types provided from the outputted scenriodump file.
...
% - xvaCgRegressionOrderDynamicIm
% - xvaCgRegressionVarianceCutoff
% - xvaCgRegressionVarianceCutoffDynamicIm
% - xvaCgPythonIntegration
% - xvaCgPythonIntegrationDynamicIm
% - xvaCgTradeLevelBreakDown
% - xvaCgUseRedBlocks
% - xvaCgRegressionCacheSize
% - xvaCgEnableCgOptimization
...
The {\tt scenarioGeneration} `analytics' generates scenarios and provides the statistics and distribution of the scenarios generated. Listing \ref{lst:ore_scenarioGneration}
...
\end{minted}
\caption{ORE analytic: scenarioGeneration}
\label{lst:ore_scenarioGeneration}
\end{listing}
...
\item {\tt dimScaling:} Optional scaling factor applied to DIM values ({\tt Regression} and
{\tt DeltaVaR}-family models only; no effect for {\tt SimmAnalytic} or {\tt DynamicIM}).
When set, this value is used directly for all netting sets and the collateral balance IM is
ignored. When omitted, the scaling is derived per netting set as
$\text{IM}_{t_0}^{\text{actual}}/\text{DIM}_{t_0}^{\text{model}}$ from
{\tt collateralBalancesFile}; the IM must be strictly positive for each netting set or the
run will abort. Set to {\tt 1.0} to apply no scaling.
...
\item {\tt decomposeIndexSensitivities}: Decompose Credit index and Equity and Commodity index sensitivities into constituent sensitivities
\end{itemize}

The decomposition of index sensitivities is controlled as follows:

\begin{itemize}
    \item \textbf{Credit Index Decomposition:} Enabled via the \texttt{SensitivityDecomposition} flag in the pricing engine settings for \texttt{IndexCreditDefaultSwap}, \texttt{IndexCreditDefaultSwapOption}, and \texttt{SyntheticCDO} (see pricing engine parameterisation, Section~\ref{sec:configuration_pricingengines}).
    \item \textbf{Equity and Commodity Index Decomposition:} Controlled at the trade level by adding the field \texttt{index\_decomposition} to the trade envelope's \texttt{AdditionalFields} section, as shown in Listing~\ref{lst:eq_index_decomposition}.
\end{itemize}
...
\end{minted}
\caption{Trade envelope configuration for equity and commodity index decomposition}
\label{lst:eq_index_decomposition}
\end{listing}

Equity index decomposition is supported for \texttt{EquitySwap} and \texttt{GenericTRS} trades, while commodity index decomposition is available for \texttt{GenericTRS} trades.
...
The parameters have the same interpretation as for the sensitivity analytic. There is one new parameter *sensitivityInputFile* which points to a csv file with the raw (zero)sensitivities. Those raw sensitivities will be converted into par sensitivities, using the same methodology as in the embedded approach above, and the configuration is described in \ref{sec:sensitivity}.
...
\item {\tt SalvagingAlgorithm:} Allowable values are: {\em None}, {\em Spectral}, {\em Hypersphere}, {\em LowerDiagonal}, {\em Highham}, {\em Principal}. If omitted, it defaults to Spectral. Compare \cite{corrSalv}.
...
The \textbf{Historical Simulation VaR} analytic performs full-revaluation P\&L generation over historical market
scenarios and reports VaR quantiles from the resulting empirical distribution. Listing \ref{lst:ore_histsimvar} shows
...
<Analytics>
  <Analytic type="historicalSimulationVar">
    ...
    <!-- Optional sensitivity-based calculation -->
    <!-- <Parameter name="sensitivityInputFile">sensitivity.csv</Parameter> -->
    <!-- <Parameter name="sensitivityConfigFile">sensitivity.xml</Parameter> -->
    <Parameter name="tradePnl">N</Parameter>
    <Parameter name="riskFactorBreakdown">N</Parameter>
    <Parameter name="riskClassBreakdown">Y</Parameter>
    <Parameter name="outputFile">var.csv</Parameter>
  </Analytic>
</Analytics>
\end{minted}
\caption{ORE analytic: Historical Simulation VaR}
\label{lst:ore_histsimvar}
\end{listing}

The parameters have the following interpretation:

\begin{itemize}
\item {\tt historicalScenarioFile}: csv file containing the market scenarios for each date in the observation
  periods; the granularity of the scenarios must match the simulation market definition.
\item {\tt simulationConfigFile}: defines the structure of the simulation market applied in the P\&L calculation,
  e.g.\ discount and index curves, yield curve tenor points, FX pairs, etc.
\item {\tt historicalPeriod}: comma-separated date list; an even number of ordered dates is required
  (d1, d2, d3, d4, \ldots), where each pair defines the start and end of a historical observation period.
\item {\tt mporDays}: number of calendar days between successive historical scenarios used to compute P\&L effects
  (typically 1 or 10).
\item {\tt mporCalendar}: calendar applied in the scenario date calculation.
\item {\tt mporOverlappingPeriods}: if {\tt true}, overlapping holding periods of length \texttt{mporDays} are used
  ($t$ to $t{+}10$, $t{+}1$ to $t{+}11$, \ldots); if {\tt false}, consecutive non-overlapping periods are used.
\item {\tt quantiles}: comma-separated list of quantiles to be reported; each quantile produces one
  {\tt Quantile\_<p>} column in the output file.
\item {\tt includeExpectedShortfall} [Optional, default {\tt N}]: if {\tt Y}, additional
  {\tt ExpectedShortfall\_<p>} columns are appended to the output report for each requested quantile. The
  Expected Shortfall at confidence level $p$ is the average P\&L of all scenarios that fall at or below
  the corresponding VaR level; see section~\ref{histsimvar} for the mathematical definition.
\item {\tt sensitivityInputFile} [Optional]: csv file containing delta, gamma and cross-gamma sensitivities in
  the same format as {\tt parametricVar.sensitivityInputFile}. If this and {\tt sensitivityConfigFile} are both
  provided, historical P\&Ls are calculated from the sensitivities and historical scenario shifts instead of full
  revaluation.
\item {\tt sensitivityConfigFile} [Optional]: sensitivity scenario configuration used to map historical scenario
  moves to the risk-factor keys in {\tt sensitivityInputFile}. If either sensitivity input parameter is omitted,
  the analytic uses the existing full-revaluation path.
\item {\tt tradePnl} [Optional, default {\tt N}]: if {\tt Y}, the second output report provides a P\&L breakdown by
  trade ID, risk class and risk type.
\item {\tt riskFactorBreakdown} [Optional, default {\tt N}]: if {\tt Y}, a third output report provides a
  P\&L breakdown by individual risk factor.
\item {\tt riskClassBreakdown} [Optional, default {\tt Y}]: if {\tt Y} (the default), VaR is computed
  separately for each risk-class / risk-type combination (Interest Rate, FX, Inflation, Equity, Credit,
  Commodity) in addition to the aggregate portfolio row. Set to {\tt N} to compute only the aggregate
  {\tt [All, All]} row, which significantly reduces runtime when the per-class breakdown is not required.
\item {\tt portfolioFilter} [Optional]: only trades whose {\tt portfolioId} matches this filter are processed;
  the entire portfolio is used if omitted.
\item {\tt outputFile}: output file name for the main VaR report.
\end{itemize}
...
<Analytics>
  <Analytic type="correlation">
      ...
      <Parameter name="filterCamCorrelationScenarioTenor">{"DiscountCurve/.*":"2Y", "IndexCurve/USD-FedFunds":"1Y"}</Parameter>
      <Parameter name="mporDays">10</Parameter>
      <Parameter name="mporCalendar">USD</Parameter>
...
\item {\tt filterCamCorrelationScenarioTenor}: JSON object mapping regex patterns to tenors for filtering correlation scenario output. Patterns are matched against risk factor keys (format ``KeyType/Name'') in reverse order (last entry has highest priority). If no pattern matches a risk factor, all tenors are included. Example: \verb+{"DiscountCurve/.*":"5Y", "DiscountCurve/USD/.*":"3Y", "IndexCurve/USD-FedFunds":"10Y"}+.
...
The additional parameters in Listing \ref{lst:ore_pnlexplain} have the following interpretation:
...
\item {\tt riskFactorLevelReporting}:  Enables P\&L explanation with decomposition by risk factor.
\end{itemize}
...
tenor structure for the curves or it uses only the swaption ATM vols if the volatilities aren't simulated.
...
structure for building the curves or uses only the swaption ATM vols if  the volatilities aren't simulated.
...
for building the curves or uses only the swaption ATM vols if the volatilities aren't simulated. To minimize some of
