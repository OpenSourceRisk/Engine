...
By default, par sensitivity analysis sets small diagonal elements in the par conversion matrix to 0.01 silently to avoid matrix inversion issues. However, it is possible to control this behavior by adding an optional block to the sensitivity configuration as shown in listing \ref{lst:par_conversion_matrix_regularisation}. The available options are:

\begin{itemize}
\item {\tt Silent}: Set small diagonal elements to 0.01 without warnings (default behavior)
\item {\tt Warning}: Set small diagonal elements to 0.01 but issue structured warnings, calculation continues  
\item {\tt Fail}: Issue structured errors and fail the calculation when small diagonal elements are encountered
\end{itemize}
