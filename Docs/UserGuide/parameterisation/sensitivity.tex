...
\item Commodity curves (in the flat price domain)
...
\item {\tt IntradayPowerCurve/PJM\_WH\_RT/3/1M}: 1M pillar of intraday power curve ``PJM\_WH\_RT'',
  the pillar index is 3 in this case
...
\item ThetaPeriod: If given and not 0D, the period for theta calculation
...
\input{parameterisation/sensitivity_commoditycurves}

\input{parameterisation/sensitivity_intradaypowercurves}
...
By default, par sensitivity analysis sets small diagonal elements in the par conversion matrix to 0.01 silently to avoid matrix inversion issues. However, it is possible to control this behaviour by adding an optional block to the sensitivity configuration as shown in listing \ref{lst:par_conversion_matrix_regularisation}. The available options are:

\begin{itemize}
\item {\tt Silent}: Set small diagonal elements to 0.01 without warnings (default behaviour)
\item {\tt Warning}: Set small diagonal elements to 0.01 but issue structured warnings
\item {\tt Disable}: Disable regularisation, use original diagonal elements as-is
\end{itemize}
...
\end{minted}
\caption{Par conversion matrix regularisation}
\label{lst:par_conversion_matrix_regularisation}
\end{longlisting}
...
\item Permissible codes for the assumed par instruments:
	\begin{itemize}
	\item DEP, FRA, FUT, IRS, OIS, TBS, FXF, XBS in the case of DiscountCurves
	\item DEP, FRA, FUT, IRS, OIS, TBS in the case of IndexCurves
	\item DEP, FRA, FUT, IRS, OIS, TBS, XBS in the case of YieldCurves
