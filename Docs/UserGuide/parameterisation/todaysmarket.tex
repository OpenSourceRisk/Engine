...
\item Securities
\item Intraday power price curves
\end{itemize}
...
\subsubsection{Intraday Power Price Curves}

Intraday power price curves are configured as follows
\begin{listing}[H]
%\hrule\medskip
\begin{minted}[fontsize=\footnotesize]{xml}
 <IntradayPowerPriceCurves id="default">
   <IntradayPowerPriceCurve name="PJM_WH_RT">IntradayPowerCurve/USD/PJM_WH_RT</IntradayPowerPriceCurve>
 </IntradayPowerPriceCurves>
\end{minted}
\caption{Intraday power price curves block with ID 'default'}
\label{lst:intraday_power_curve_spec}
\end{listing}

This block instructs ORE to build intraday power price curves (and corresponding intraday power indices) from the
curve specifications defined in the curve configuration file. The curve spec string has format
\lstinline!IntradayPowerCurve/<Currency>/<CurveId>!, where \lstinline!<CurveId>! matches an
\lstinline!IntradayPowerCurve! configuration in the curve configuration file.
