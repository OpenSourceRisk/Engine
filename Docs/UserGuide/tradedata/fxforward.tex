...
\begin{itemize}
\item ValueDate: The value date of the FxForward. The FX rate "fixes" on the ValueDate (i.e., the agreed fxforward rate is applied). Before the ValueDate, the FxForward contract's value fluctuates with market FX rates.
At the ValueDate, the settlement becomes deterministic. \\ Allowable values:  See \lstinline!Date! in Table \ref{tab:allow_stand_data}.
