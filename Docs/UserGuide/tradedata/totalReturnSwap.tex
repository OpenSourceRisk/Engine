...
\begin{itemize}
\item The {\tt UnderlyingData} block specifies one or more underlyings, which can be a trade of one of the following
  ...
  \item BondFuture: See \ref{ss:bondfuture}, the trade data is given in a BondFuture sub node.
  \item ForwardBond: See \ref{ss:BondForward_refdata}, the trade data is given in a ForwardBondData sub node. Deprecated way of representing bond future trs, consider using BondFuture as the underlying.
  ...
  \item Derivative: An arbitrary underlying derivative trade (of any type covered by ORE), allowing the set up of a so called Portfolio Swap with multiple underlying derivatives. The Derivative subnode has exactly two subnodes:
    ...
    \end{itemize}
  \item PortfolioIndexTradeData: This is a Portfolio Swap that references an underlying Basket via the \lstinline!BasketName! identifier.
    The underlying basket can have an arbitrary number underlying derivatives of any supported TradeType.
    The PortfolioIndexTradeData subnode has one subnode:
    \begin{itemize}
      ...
      \item \lstinline!PriceIsPerUnit!:
        The allowable values are \lstinline!true! or \lstinline!false!.
        When set to \lstinline!false!, the contribution of the basket on a given date is simply the value of the basket.
        When set to \lstinline!true!, the value of the basket on a given date is interpreted as the unit value of the index.
        The contribution of the basket to the TRS is then given by this unit value multiplied by the \lstinline!IndexQuantity!.
    \end{itemize}
    Historical index fixing levels for the basket should be given under the fixing name \lstinline!GENERIC-<BasketName>!.
  \end{itemize}

  Except for \lstinline!PortfolioIndexTradeData!, each trade is specified by a \lstinline!TradeType! and a trade type 
  dependent data block as listed above.
  \begin{itemize}
    \item Listing \ref{lst:trsdata} shows an example for a convertible bond underlying.
    \item Listing \ref{lst:trsdata2} shows an example for an equity basket underlying.
    \item Listing \ref{lst:trsdata3} shows an example for a bond basket underlying.
    \item Listing \ref{lst:trsdata4} shows an example for a Derivative underlying, with 3 underlying trades in this case.
    \item Listing \ref{lst:trsdata_portfolio} shows an example for a \lstinline!PortfolioIndexTradeData! underlying.
    \item Listing \ref{lst:trsdata_pitd_bond-future-option} shows an example for a \lstinline!PortfolioIndexTradeData! 
      underlying consisting of a cash position, a bond future postion and a bond future option. The associated reference 
      data is shown in Listing \ref{lst:trsdata_pitd_bond-future-option-ref-data}. Note the usage of 
      \lstinline!ApplyConversionFactor! and \lstinline!UseFuturePrice! as outlined in Section~\ref{ss:bondfuture} to 
      indicate that we want the bond future price level contributing to the index rather than the NPV of the bond 
      future trade.
  \end{itemize}
...
\item The {\tt ReturnData} block specifies the details of the return leg.
  ...
  \item \lstinline!PaymentLag! [optional]:
  The lag between the, adjusted or unadjusted (see \lstinline!PaymentLagAnchor! below), reference schedule period end 
  date and the payment date.
  
  Allowable values: Any valid period.
  In other words, an integer followed by a string where the last character must be one of 
  \lstinline!D!, \lstinline!W!, \lstinline!M! or \lstinline!Y!.
  Compound period definitions also work, for example \lstinline!1Y6M! and \lstinline!-6W4D!.
  Note that compound periods only work for year and month pairs or week and day pairs.
  If left blank or omitted, it defaults to \lstinline!0D!.
  Note also that negative periods are allowed where they make sense.
  If a whole number is given without the unit suffix, it is assumed that it is a number of days i.e.\ 
  that the suffix is \lstinline!D!.

  \item \lstinline!PaymentLagUnit! [optional]:
  Determines how the \lstinline!PaymentLag! is used to calculate the payment date from an anchor date.
  If omitted, or \lstinline!BusinessDays! is given, the \lstinline!PaymentLag! is applied in the usual way.
  In other words the anchor date is shifted by the period defined by the \lstinline!PaymentLag! using the 
  \lstinline!PaymentCalendar! and \lstinline!PaymentConvention!.
  If \lstinline!CalendarDays! is given, the \lstinline!PaymentLag! period must have day units.
  In this case, the anchor date is shifted by \lstinline!PaymentLag! \emph{calendar} days.
  If the resulting date is not a good business day with respect to the \lstinline!PaymentCalendar!,
  it is adjusted using the \lstinline!PaymentCalendar! and \lstinline!PaymentConvention!.
  The anchor date used for the payment date generation can be influenced by the \lstinline!PaymentLagAnchor! below.

  Allowable values: \lstinline!BusinessDays! or \lstinline!CalendarDays!.
  Defaults to \lstinline!BusinessDays! if omitted.

  \item \lstinline!PaymentLagAnchor! [optional]:
  Determines the base or anchor date to use when calculating a payment date using the \lstinline!PaymentLag!.
  If omitted, or \lstinline!Adjusted! is given, the anchor date is the underlying schedule date.
  This schedule date may have had adjustments applied.
  If \lstinline!Unadjusted! is given, the anchor date is the underlying schedule date 
  before any adjustments have been applied.

  Allowable values: \lstinline!Adjusted! or \lstinline!Unadjusted!. Defaults to \lstinline!Adjusted! if omitted.
...
\end{listing}
  ...
  \item InitialPrice [Optional]: The equity (or bond) price of the underlying
    ...
    \item a (dirty) price for Bond, ForwardBond and Convertible Bond underlyings, the format is dependent on the price quotation method of the referenced bond:
      ...
      \item Currency per Unit: the InitialPrice should be given as e.g. $99.90$ in currency (InitialPriceCurrency $=$ KRW)
        for a dirty price quoted as $9{,}990$ KRW per bond unit, where a bond unit is worth $10{,}000$ KRW.
        
        Note that Korean bonds typically use Currency per Unit quotation method, whereas other bonds typically use Percentage of Par.
      \end{itemize}
...
\item FXConversion [Optional]: Determines what Fx rate is used in the payoff calculation, the rate at the  \emph{Start} of 
the period (default) or the rate at the \emph{End}.  Only has an impact If the underlying asset currency is different from 
the return currency.

With the default FXConversion \emph{Start} the accrual pricing return is:\\
(Ntl x today's price x today's Fx rate) - (Ntl x initial price x initial Fx rate)\\
When the as of date is the date of the final price (end of period) we have:\\
(Ntl x final price x final Fx rate) - (Ntl x initial price x initial Fx rate)

With the FXConversion \emph{End}  the accrual pricing return is:\\
(Ntl x todays price x today's Fx rate) - (Ntl x initial price x today's Fx rate)\\
When the as of date is the date of the final price (end of period) we have:\\
(Ntl x final price x final Fx rate) - (Ntl x initial price x final Fx rate)

Allowable values: \emph{Start}, \emph{End}. Defaults to \emph{Start} if omitted.
    ...
    <TotalReturnSwapData>
      <UnderlyingData>
        <Trade>
          ...
          <BondFutureData>
            <ContractName>TYH25</ContractName>
            <ContractNotional>100000000</ContractNotional>
...
\end{minted}
\caption{Generic Total Return Swap with Bond Future underlying}
\label{lst:trsdata35}
\end{listing}
...
<TotalReturnSwapData>
  <UnderlyingData>
    <PortfolioIndexTradeData>
      <BasketName>TEST-BASKET</BasketName>
      <IndexQuantity>100000</IndexQuantity>
      <PriceIsPerUnit>true</PriceIsPerUnit>
    </PortfolioIndexTradeData>
  </UnderlyingData>
  <!-- omitting other data for brevity -->
</TotalReturnSwapData>
\end{minted}
\caption{Generic TRS on a \lstinline!PortfolioIndexTradeData! underlying}
\label{lst:trsdata_pitd_bond-future-option}
\end{listing}

\begin{listing}[H]
%\hrule\medskip
\begin{minted}[fontsize=\footnotesize]{xml}
<ReferenceDatum id="TEST-BASKET">
  <Type>PortfolioBasket</Type>
  <PortfolioBasketReferenceData>
    <Components>
      <Trade>
        <TradeType>BondFutureOption</TradeType>
        <BondFutureOptionData>
          <OptionData>
            ...
            <ExerciseDates>
              <ExerciseDate>2026-04-01</ExerciseDate>
            </ExerciseDates>
          </OptionData>
          <ContractName>TYM26</ContractName>
          <ContractNotional>24.0186141</ContractNotional>
          <Strike>1.0825</Strike>
        </BondFutureOptionData>
      </Trade>
      <Trade>
        <TradeType>BondFuture</TradeType>
        <BondFutureData>
          <ContractName>TYM26</ContractName>
          <ContractNotional>250.429169</ContractNotional>
          <LongShort>L</LongShort>
      ...
      <Trade>
        <TradeType>CashPosition</TradeType>
        <CashPositionData>
          <Currency>USD</Currency>
          <Amount>26.26224675</Amount>
        </CashPositionData>
      </Trade>
    </Components>
  </PortfolioBasketReferenceData>
...
\end{minted}
\caption{The \lstinline!PortfolioIndexTradeData! reference data}
\label{lst:trsdata_pitd_bond-future-option-ref-data}
\end{listing}
