...
For Equity- and CommodityAccumulators: The strike value (global and local) for one unit/share/contract of the underlying equity or commodity, expressed in the currency the equity or commodity is quoted in or any other currency if a \listinline!FxIndex! for the conversion is specified. For EquityAccumulators, the \lstinline!StrikeData! node (see above) should be used.
...
\item FxIndex [Optional]: Required for composite options, where the strike, knockout barrier and pay currency are different from the underlying quote currency. A node with the underlying of the FX conversion from quote currency to pay currency.
