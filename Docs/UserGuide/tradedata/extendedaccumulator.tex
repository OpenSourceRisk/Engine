...
\item BarrierLevel: the value of the index below which a Trigger Event occurs on Extension Decision Date. It is expressed in the same way as Strike outlined above. If the underlying on the extension decision date is above the barrier, it triggers the extension on conditional observation dates. Else, the trade terminate at last observation date.
...
IF Underlying(ExtensionDecisionDate)>BarrierLevel THEN
