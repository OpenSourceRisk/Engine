...
A Swaption Straddle is a combination of a payer swaption and a receiver swaption with the same underlying swap characteristics. This structure gives the buyer the right to enter into either a payer swap (paying fixed, receiving floating) or a receiver swap (receiving fixed, paying floating) at expiry. Swaption Straddles are typically used when the buyer expects significant interest rate volatility but is uncertain about the direction.
...
\vspace{5mm}
The Swaption Straddle shares most characteristics with the standard Swaption (see section \ref{ss:swaption}). Supported exercise styles are \emph{European}, \emph{Bermudan}, and \emph{American}. Swaption Straddles of all exercise styles can have an arbitrary number of legs, with each leg represented by a \lstinline!LegData! sub-node. Cross currency swaption straddles are not supported for either exercise style, i.e. the Currency element must have the same value for all \lstinline!LegData! sub-nodes of a swaption straddle. There must be at least one full coupon period after the exercise date for European Swaption Straddles, and after the last exercise date for Bermudan and American Swaption Straddles. See Table \ref{tab:swaption_straddle_requirements} for further details on requirements for Swaption Straddles.\\
\vspace{5mm}
