...
The OptionData trade component sub-node is outlined in section \ref{ss:option_data}. 
...
\item  \lstinline!Style! The allowable values are \emph{European} or \emph{Bermudan}.
...
\item \lstinline!NoticeCalendar![Optional]: The calendar used to compute the notice date from the exercise date. If not given
  defaults to the \emph{NullCalendar} (no holidays, weekends are no holidays either). Allowable values: See Table \ref{tab:calendar} Calendar.
...
\item An \lstinline!ExerciseDates! node where exactly one ExerciseDate date element must be given for \emph{European} style swaptions, and for \emph{Bermudan} style swaptions  at least two ExerciseDate date elements must be given. \\

\item Optional \lstinline!PremiumAmount!,  \lstinline!PremiumCurrency!, and \lstinline!PremiumPayDate! fields to specify the Swaption premium. 
