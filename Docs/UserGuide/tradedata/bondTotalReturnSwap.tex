...
\begin{itemize}
\item The {\tt BondData} block specifies the underlying bond, usually by specifying the security id and the quantity /
  ...
  \item SecurityId: The underlying security identifier\\
      ...
      Note that Convertible Bonds are not supported as underlyings for BondTRS. For Convertible Bonds, trade type \emph{TotalReturnSwap} should be used instead.
...
\item The {\tt TotalReturnData} block specifies
  ...
  \item FXTerms [Mandatory when underlying bond and BondTRS currencies differ]: Required if the bond currency is different from the return currency, which is always
    ...
    \begin{itemize}
      ...
      \item ApplyFXIndexFixingDays [Optional]: If set to \emph{true}, the FX fixing date is moved back by the usual number of fixing days (for example, 2 days before the valuation date), using the FX index calendar to skip holidays.
      If \emph{false}, the FX fixing date is the same as the valuation date.

      Allowable values: \emph{true} (use fixing lag) or \emph{false} (use valuation date). Defaults to \emph{false} if left blank or omitted.
