...
\item FixedAmount: the notional amount in 
...
REQUIRE {FixedAmount >= 0} AND {Strike >= 0};
...
NUMBER d, Fixing;
    ...
    FOR d IN (1, SIZE(ObservationDates), 1) DO
        ...
        Value = Value + PAY(FixedAmount*Strike-FixedAmount*Fixing, ObservationDates[d], ObservationSettlementDates[d], PayCurrency);
    END;
    ...
    IF Underlying(ExtensionDecisionDate)>ExtensionTrigger THEN
        FOR d IN (1, SIZE(ConditionalObservationDates), 1) DO
            ...
            Value = Value + PAY(FixedAmount*Strike-FixedAmount*Fixing, ConditionalObservationDates[d], ConditionalSettlementDates[d], PayCurrency);
    END;
END;
\end{minted}
