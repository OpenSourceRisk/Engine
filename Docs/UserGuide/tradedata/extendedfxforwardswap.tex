...
\item FixingAmount: the notional amount in 
...
NUMBER d, Fixing;
    ...
    FOR d IN (1, SIZE(ObservationDates), 1) DO
        ...
        Value = Value + PAY(FixingAmount*Strike-FixingAmount*Fixing, ObservationDates[d], ObservationSettlementDates[d], PayCurrency);
    END;
    ...
    IF Underlying(ExtensionDecisionDate)>ExtensionTrigger THEN
        FOR d IN (1, SIZE(ConditionalObservationDates), 1) DO
            ...
            Value = Value + PAY(FixingAmount*Strike-FixingAmount*Fixing, ConditionalObservationDates[d], ConditionalSettlementDates[d], PayCurrency);
    END;
END;
\end{minted}
