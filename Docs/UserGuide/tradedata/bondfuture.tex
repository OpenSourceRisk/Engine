...
The bondfuture instrument is on ore object of its own, but employs the bond forward instrument in the background and in turn uses the bond forward pricing engine. 
However, the premium data segment is left blank and thus unused. At the outset, the bondfuture can specify a set of deliverable securities, 
from which one is chosen, then the instrument "turns" into a bond forward referencing one specific underlying – the CTD bond.

To match the traded market price of the future, we need to an imply a spread on the underlying CTD security. Depending on the flag FairPrice the market price is applied 
as a strike (true) or as the final value (false). Obviously this spread differs slightly to the spread when solving for the bond price.

A bondfuture is set up using a {\tt BondFutureData} block as shown below and the trade type is \emph{BondFuture}. 

\begin{itemize}
  \item ContractName: The id identifying the future contract. The ContractName is also used to identify which bond future reference datum to take. 
  This defines the security specific spread to be used for pricing.
...
{\tt ContractName}, {\tt ContractNotional} and {\tt LongShort} are mandatory fields for the "short" version.
As one can see from the below example the trade exploits the reference data to fully specify the trade. 
See the combination of listing \ref{lst:bondfuture_with_refdata} and listing \ref{lst:bondfuturerefdata}.

For the long version, the bond future details are inlined in the trade as shown in listing \ref{lst:bondfuture_explicit}.
The next set of fields can be provided explicitly within the trade (long version) or can be read from the bond future reference datum (short version). 

\begin{itemize}
  \item Currency: The currency of the notional, and thus of the future. Allowable values: See Table \ref{tab:currency}.
  \item DeliveryBasket: A list of eligible securities/bond identifiers. For those Ids BondReferenceData should be available in the reference data manager. 
  ...
  \item DeliverableGrade: This is used for derivation of the conversion factor, if the factor is not made available within the market data. 
  The derivation is currently implemented for USD treasuries only. 
  The deliverable grade identifies the nature of the eligible underlying bond, to be delivered. 
  Allowable values are: \emph{ZT, Z3N, ZF, ZN, TN, TWE, ZB, UB} or the corresponding Bloomberg codes. 
  \item FairPrice: indicates whether the strike amount is equal to zero (\emph{false}), or to the settlement price (\emph{true}) as given by the market data. 
  Fallback value is true, leading to a "fair" MtM, i.e. NPV of zero, when the spread is implied from market quotes. 
  \item Settlement [Optional]: Cash or Physical. Optional, defaults to Physcial.
  \item SettlementDirty [Optional]: A flag that determines whether the settlement amount reflects a clean (\emph{false}) or dirty (\emph{true}) price. 
  In either case, the dirty amount is actually paid on the forward maturity date, 
  i.e. if SettlementDirty = false, the (forward) accruals are computed internally and added to the given amount to get the actual settlement amount. 
  Optional, defaults to false, i.e. clean.
  \item LastTradingDate: Serves as the expiry date of the future, akin to the ForwardMaturityDate of the forward bond (see \ref{ss:forwardbond}) 
  \item LastDeliveryDate: Serves as the settlement date of the future, akin to the ForwardSettlementDate of the forward bond (see \ref{ss:forwardbond}) 
\end{itemize}
...
\begin{listing}[H]
  ...
  \label{lst:bondfuture_with_refdata}
\end{listing}
...
\begin{listing}[H]
  ...
  \begin{minted}[fontsize=\footnotesize]{xml}
    <BondFutureData>
      <ContractName>explicit</ContractName>
      <ContractNotional>1000000</ContractNotional>
      <LongShort>L</LongShort>
      <FairPrice>true</FairPrice>
      <Currency>USD</Currency>
      <DeliverableGrade>ZN</DeliverableGrade>
      <LastTradingDate>2025-03-20</LastTradingDate>
      <LastDeliveryDate>2025-03-31</LastDeliveryDate>
      <DeliveryBasket>
        <SecurityId>ISIN:US91282CDJ71</SecurityId>
        <SecurityId>ISIN:US91282CEP23</SecurityId>
        <SecurityId>ISIN:US91282CLM19</SecurityId>
  ...
  \end{minted}
  \caption{BondFutureData Explicit}
  \label{lst:bondfuture_explicit}
\end{listing}

\subsubsection*{Derivation of the LastTradingDate and LastDeliveryDate}
The exampe with the reference data block above, i.e. listing \ref{lst:bondfuturerefdata}, shows how to set up a future referencing an USD 10-Year-T-Note. 
...
which is the same as in the long version, i.e. listing \ref{lst:bondfuture_explicit}.
...
Be \emph{sp} the quoted future settlement price, \emph{ai} accrued interest, \emph{cf} the bond specific conversion factor and \emph{bp} the bond price. 
...
$$ bp - (sp \cdot cf) $$
is least. As the decision is taking place at future expiry, the bond price is a forward value. 
