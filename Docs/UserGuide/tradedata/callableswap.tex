...
\item OptionData: This is a trade component sub-node outlined in section \ref{ss:option_data}. The exercise dates
  ...
  \ref{ss:swaption} using the \lstinline!ExerciseData! node within OptionData. Note that if an exercise triggers the exchange of notionals in a cross currency callable swap, this exchange must be captured as two exercise fees in the respective currencies.
...
\item LegData: This is a trade component sub-node described in section \ref{ss:leg_data} outlining each leg of the
  ...
  i.e. multiple \lstinline!LegData! nodes.  The LegType elements must be of types \emph{Floating}, \emph{Fixed},
  \emph{Cashflow} or \emph{RangeAccrual}. Cross currency underlyings are supported.
