...
\begin{itemize}
	...
	\item Currency: The payment currency of the equity forward. If underlying currency and payment currency don't match, one need to specify a FXIndex in the settlement data node.\\
	...
	\item StrikeCurrency: [Optional] The currency of the strike value. Defaults to the underlying currency.\\
	Allowable values:  See Fiat Currencies and Minor Currencies in Table \ref{tab:currency}.	
...
\begin{itemize}
\item FXIndex: The FX reference index for determining the FX fixing at the payment date. This field is required if the underlying currency doesn't match the deal currency. Otherwise, it is ignored. \\
...
Allowable values: See \lstinline!Date! in Table \ref{tab:allow_stand_data}. If left blank or omitted, defaults to the maturity date with some adjustments applied
