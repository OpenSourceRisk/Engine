...
\subsection{Commodity Spread Option}
...
Details on these are described in \ref{ss:commodityfloatingleg}. The resulting Legs must produce a single cashflow (i.e. a single \emph{calculation period}).
...
Within the two \lstinline!LegData!, the \lstinline!Quantity! node has a special meaning with respect to contract units: it represents the amount of relevant underlying contract quantity units required to sum up to
\lstinline!CommoditySpreadOptionData! \lstinline!Quantity!, and it is used to support underlying contracts quoted in different units (e.g. financial oil and natural gas).
Let $U_{l}$ be the units for the long position commodity underlying contract, and $U_{s}$ the units for the short position one.
Also, let $Q_{l}$ and $Q_s$ the quantities, expressed in $U_l$ and $U_s$ respectively that equate the number $Q_{\text{trade}}$ of units $U_{\text{trade}}$ covered by this transaction.\\
It must hold:
$$
\frac{Q_l \cdot U_l}{Q_{\text{trade}} \cdot U_{\text{trade}}} = \frac{Q_s \cdot U_s}{Q_{\text{trade}} \cdot U_{\text{trade}}} = 1.
$$
...
\begin{itemize}
    ...
    \item \lstinline!Quantity!: the number of units of the underlying commodity covered by the transaction.
Allowable values: Any positive real number.
