...
\hline
Indexed Corridor Variance Swap & \lstinline!IndexedCorridorVarianceSwap! \\
\hline
...
\hline
Pairwise Geometric Variance Dispersion Swap & \lstinline!PairwiseGeometricVarianceDispersionSwap! \\
\hline
...
A capped and/or floored variance swap with a knock-in/knock-out feature. Note that this trade type is also used for regular capped/floored
variance swaps without barrier, by setting the \lstinline!BarrierType! to \emph{UpIn} and \lstinline!BarrierLevel! to zero.
...
The \emph{KIKOVarianceSwap} script referenced in the trade above is shown in listing
...
<Trade id="DualEuroBinaryOptionDoubleKO">
  ...
  <DualEuroBinaryOptionDoubleKOData>
    ...
    <isTerminalOnly type="bool">false</isTerminalOnly>
...
IF Underlying(Expiry) >= SpotBarrierLevel AND isTerminalOnly == 1  THEN
  Alive = 0;
END;
...
\begin{itemize}
  ...
  Allowable values: \emph{Long, Short}.
  \item{}[index] \lstinline!Underlying!: Underlying index. For FX, where the underlying is provided in the form \lstinline!FX-SOURCE-CCY1-CCY2! (see Table \ref{tab:fxindex_data})\\
  Allowable values: See Section \ref{data_index} for allowable values.
  \item{}[event] \lstinline!ValuationSchedule!: The base schedule which defines the start dates of the variance accrual schedule. This node is also used to derive the variance accrual schedule and defines the observation dates of the SPOT KO. \\
  Allowable values: See Section \ref{ss:schedule_data}.
  \item{}[number] \lstinline!SpotBarrierLevel!: The agreed Spot barrier level. For Fx, the Fx barrier level is defined as amount in domestic currency (CCY2) for one unit of foreign currency (CCY1). \\
  For Equity and Commodity: The barrier level for one unit/share/contract of the underlying equity or commodity, expressed in the domestic currency (CCY2).\\
  Allowable values: Any non-negative real number.
  \item{}[number] \lstinline!VolBarrierLevel!: The agreed volatility barrier level. Expressed in decimal form, it is a volatlity.\\
  Allowable values: Any non-negative real number.
  ...
  Allowable values: Any non-negative real number.
  \item{}[boolean] \lstinline!isTerminalOnly!: Determines if the Spot Barrier occurs on every valuation date (\emph{false}) or at expiry (\emph{true}). \\
  Allowable values: Boolean node, the set of allowable values is given in Table \ref{tab:boolean_allowable}.
  \item{}[event] \lstinline!SettlementDate!: The date on which the option payoff is settled. \\
  Allowable values: See \lstinline!Date! in Table \ref{tab:allow_stand_data}.
  \item{}[event] \lstinline!Expiry!: The option expiration date. When isTerminalOnly is \emph{true} the Expiry is also the Spot Barrier observation date.\\
  Allowable values: See \lstinline!Date! in Table \ref{tab:allow_stand_data}. Must be on or after the valuation end date, and must be on or before the SettlementDate.
  ...
  Allowable values: Any non-negative real number.
  \item{}[currency] \lstinline!PayCcy!: The currency of the Premium and also the currency of the digital option payout, i.e.  of the SettlementAmount.
\end{itemize}
...
The \emph{CorridorVarianceSwap} script referenced in the trade above is shown in listing
...
Also called Cross Corridor Variance Swap. The payoff depends on the variance of one equity index (the Underlying) for the days another
...
\begin{minted}[fontsize=\scriptsize]{xml}
<Trade id="EQ_IndexedCorridorVarianceSwap">
  <TradeType>ScriptedTrade</TradeType>
  <Envelope>
    .....
  </Envelope>
  <IndexedCorridorVarianceSwapData>
    <LongShort type="longShort">Long</LongShort>
    <Strike type="number">0.316129</Strike>
    <Notional type="number">1380</Notional>
    <Underlying type="index">EQ-RIC:.SPX</Underlying>
    <CorridorIndex type="index">EQ-RIC:.STOXX50E</CorridorIndex>
    <SquaredPayoff type="bool">true</SquaredPayoff>
    <ValuationSchedule type="event">
        <ScheduleData>
            <Rules>
    ...
    <UpperBarrierLevel type="number">3469.18</UpperBarrierLevel>
    <LowerBarrierLevel type="number">2207.66</LowerBarrierLevel>
...
The \emph{IndexedCorridorVarianceSwap} script referenced in the trade above is shown in listing
\ref{lst:indexed_corridor_variance_swap}
...
FOR d IN (2, SIZE(ValuationSchedule), 1) DO
  currPrice = Underlying(ValuationSchedule[d]);
  prevPrice = Underlying(ValuationSchedule[d-1]);
  currCorrIndexPrice = CorridorIndex(ValuationSchedule[d]);
  prevCorrIndexPrice = CorridorIndex(ValuationSchedule[d-1]);

  IF {currCorrIndexPrice > LowerBarrierLevel AND currCorrIndexPrice < UpperBarrierLevel AND
      prevCorrIndexPrice > LowerBarrierLevel AND prevCorrIndexPrice < UpperBarrierLevel} THEN
        realisedVariance = realisedVariance + pow(ln(currPrice/prevPrice), 2);
...
Swap = PAY(payoff, ValuationSchedule[SIZE(ValuationSchedule)],
           SettlementDate, PayCcy);
\end{minted}
\caption{Payoff script for an IndexedCorridorVarianceSwap.}
\label{lst:indexed_corridor_variance_swap}
\end{listing}
...
where the $realisedVariance$ only consists of variance accrual contributions of the Underlying for when the CorridorIndex price/level
was trading within the window defined by the corridor.

The meanings and allowable values for the \lstinline!IndexedCorridorVarianceSwapData! node below.
...
\begin{itemize}
  ...
  \item{}[index] \lstinline!Underlying!: Underlying index. \\
  Allowable values: See ore/Docs/ScriptedTrade's Index Section for allowable values.
    \item{}[index] \lstinline!CorridorIndex!: Corridor index. \\
  Allowable values: See ore/Docs/ScriptedTrade's Index Section for allowable values.
  \item{}[event] \lstinline!ValuationSchedule!: The schedule defining the (daily) observation period for the variance accrual. \\
  Allowable values: See Section \ref{ss:schedule_data}.
  \item{}[bool] \lstinline!SquaredPayoff!: Flag indicating whether the trade is a variance swap (\emph{True}) or a volatility
...
A capped and/or floored corridor variance swap with a KnockIn/KnockOut feature.
...
The \emph{KIKOCorridorVarianceSwap} script referenced in the trade above is shown in listing
\ref{lst:kiko_variance_swap}
...
\emph{ConditionalVarianceSwap01} is a type of conditional variance swap that accrues variance contributions only when the underlying trades within a
specified single barrier window - (differing from the \emph{CorridorVarianceSwap} that has a double barrier window.) In the \emph{01} variant, the variance amount used to scale the notional is calculated directly based on the Strike parameter.
It supports several features such as caps, floors, accrual adjustments, and counting both observations, providing a way to conditionally accumulate
variance with notional scaling tied closely to the Strike.
...
The \emph{ConditionalVarianceSwap01} script referenced in the trade above is shown in listing
...
additional VarianceReference parameter. This parameter acts as a separate variance strike used to scale the notional amount, distinct
from the Strike parameter used in the payoff calculation. This added flexibility allows for more sophisticated structuring, where the
strike for variance scaling and the strike for payoff determination can differ.
...
The \emph{ConditionalVarianceSwap02} script referenced in the trade above is shown in listing
...
The \emph{VarianceDispersionSwap} script referenced in the trade above is shown in listing
...
The \emph{CorridorVarianceDispersionSwap} script referenced in the trade above is shown in listing
...
With the corridor feature, variance is accrued only when the price of the first underlying of each pair of
...
The \emph{KOCorridorVarianceDispersionSwap} script referenced in the trade above is shown in listing
...
\begin{minted}[fontsize=\scriptsize]{xml}
<Trade id="EQ_PairwiseGeometricVarianceDispersionSwap">
  <TradeType>ScriptedTrade</TradeType>
  <Envelope>
    .....
  </Envelope>
  <PairwiseGeometricVarianceDispersionSwapData>
		<PairCount type="number">3</PairCount>
		<BasketCount type="number">2</BasketCount>
		<Notionals type="number">
			<Value>100</Value>
			<Value>150</Value>
			<Value>200</Value>
		</Notionals>
		...
		<StrikesBVS type="number">
			<Value>20</Value>
			<Value>25</Value>
			<Value>30</Value>
		</StrikesBVS>
		<BasketWeights type="number">
			<Value>0.5</Value>
			<Value>0.5</Value>
		</BasketWeights>
		<VarianceWeights type="number">
			<Value>0.48</Value>
			<Value>0.49</Value>
			<Value>0.47</Value>
			<Value>0.48</Value>
		...
		<Lag type="number">5</Lag>
		<CapAmountMultiplier type="number">2.5</CapAmountMultiplier>
		<Cap type="number">10</Cap>
		<Floor type="number">-10</Floor>
		<!--DividendAdjustment type="bool">false</DividendAdjustment-->
		<ValuationSchedule type="event">
			<ScheduleData>
				<Rules>
...
The PairwiseGeometricVarianceDispersionSwap script referenced in the trade above is shown in listing
\ref{lst:pairwise_geometric_variance_dispersion_swap}

\begin{listing}[hbt]
\begin{minted}[fontsize=\scriptsize]{Basic}
REQUIRE {SIZE(StrikesVS) == PairCount * BasketCount} AND {SIZE(StrikesBVS) == PairCount};
REQUIRE {SIZE(Underlyings) == PairCount * BasketCount} AND {SIZE(Notionals) == PairCount};
REQUIRE {SIZE(BasketWeights) == BasketCount} AND {SIZE(VarianceWeights) == PairCount * BasketCount};

NUMBER currentNotional;
currentNotional = Notionals[1];

NUMBER D, expectedN, normalizationFactor;

D = SIZE(ValuationSchedule);
expectedN = D - 1;
normalizationFactor = 252 / ((expectedN - Lag + 1) * Lag);

NUMBER pair, d, basket, id;
NUMBER basketVariance, realisedVariance, currPrice, prevPrice;
NUMBER equityAmountBVS, equityAmountVS, equityAmount;

FOR pair IN (1, PairCount, 1) DO
  FOR d IN (Lag + 1, D, 1) DO
	FOR basket IN (1, BasketCount, 1) DO
	  id = (basket - 1) * PairCount + pair;
	  currPrice = Underlyings[id](ValuationSchedule[d]);
	  prevPrice = Underlyings[id](ValuationSchedule[d - Lag]);
	  basketVariance = basketVariance + BasketWeights[basket] * ln(currPrice/prevPrice);
	END;
	realisedVariance = realisedVariance + pow(basketVariance, 2);
	basketVariance = 0;
  END;

  equityAmountBVS = 1 / (2 * StrikesBVS[pair] * 100) * (pow(min(100 * sqrt(realisedVariance * normalizationFactor), CapAmountMultiplier * StrikesBVS[pair] * 100),2) - pow(StrikesBVS[pair] * 100, 2));
  realisedVariance = 0;
  ...
  FOR basket IN (1, BasketCount, 1) DO
	...
	FOR d IN (2, D, 1) DO
	  currPrice = Underlyings[id](ValuationSchedule[d]);
	  prevPrice = Underlyings[id](ValuationSchedule[d - 1]);
	  realisedVariance = realisedVariance + pow(ln(currPrice/prevPrice), 2);
	END;
	equityAmountVS = equityAmountVS + VarianceWeights[id] * 1 / (2 * StrikesVS[id] * 100) * (pow(min(100 * sqrt(252 / expectedN * realisedVariance), CapAmountMultiplier * StrikesVS[id] * 100), 2) - pow(StrikesVS[id] * 100, 2));
	realisedVariance = 0;
  END;

  equityAmount = equityAmount + Notionals[pair] * max(Floor, min(Cap, equityAmountVS - equityAmountBVS));
  equityAmountVS = 0;
END;

Swap = PAY(LongShort * equityAmount, ValuationSchedule[D], SettlementDate, PayCcy);
\end{minted}
\caption{Payoff script for a PairwiseGeometricVarianceDispersionSwap.}
\label{lst:pairwise_geometric_variance_dispersion_swap}
\end{listing}

The meanings and allowable values for the \lstinline!PairwiseGeometricVarianceDispersionSwapData! node below.

\begin{itemize}
  \item{}[longShort] \lstinline!LongShort!: Own party position in the swap. \emph{Long} corresponds to selling volatility on
  the Variance Swap transaction and buying volatility on Basket Variance Swap transaction. \\
  Allowable values: \emph{Long, Short}.
  \item{}[number] \lstinline!PairCount!: Number of pairs. \\
  Allowable values: Any positive integer.
  \item{}[number] \lstinline!BasketCount!: Number of baskets. \\
  Allowable values: Any positive integer.
  \item{}[number] \lstinline!Notionals!: The vega notional amount for each basket. \\
  Allowable values: Any real number.
  \item{}[index] \lstinline!Underlyings!: The underlyings index, ordered sequentially in each basket before moving to the next basket \\
  Allowable values: For each underlying, see \ref{ss:underlying}.
  \item{}[number] \lstinline!StrikesVS!: The volatility strike $K_{VS,u,vol}$ for each underlying $u$ in the Variance Swap transaction part, quoted in absolute terms. If the swap was
  struck in terms of variance, the square root of that variance should be used here. The strikes should be ordered sequentially in each basket before moving to the next basket.\\
  Allowable values: Any positive number, as a percentage expressed in decimal form.
  \item{}[number] \lstinline!StrikesBVS!: The volatility strike $K_{BVS,u,vol}$ for each pair in the Basket Variance Swap transaction part, quoted in absolute terms. If the swap was
  struck in terms of variance, the square root of that variance should be used here.\\
  Allowable values: Any positive number, as a percentage expressed in decimal form.
  \item{}[number] \lstinline!BasketWeights!: List of weights applied to the realised volatility of each basket in the Basket Variance Swap transaction. \\
  Allowable values: Any positive number.
  \item{}[number] \lstinline!VarianceWeights!: List of weights applied to the realised volatility of each underlying in the Variance Swap transaction,
  ordered sequentially in each basket before moving to the next basket. \\
  Allowable values: Any positive number.
  \item{}[number] \lstinline!Lag!: The number of lag days when calculating the realised variance in the Basket Variance Swap transaction.\\
  Allowable values: Any positive integer.
  \item{}[number] \lstinline!CapAmountMultiplier!: The multiplier used to calculate the Cap Amount from StrikesVS and StrikesBVS.
  $CapAmount_{VS/BVS,u,vol} = K_{VS/BVS,u,var} \cdot CapAmountMultiplier$\\
  Allowable values: Any positive integer.
  \item{}[number] \lstinline!Cap!: The cap on the difference between Variance Swap equity amount and Basket Variance Swap equity amount. \\
  Allowable values: Any number.
  \item{}[number] \lstinline!Floor!: The floor on the difference between Variance Swap equity amount and Basket Variance Swap equity amount. \\
  Allowable values: Any number.
  \item{}[bool] \lstinline!DividendAdjustment!: Whether or not the underlying price is adjusted for dividend payments. This feature is
  not yet supported.
  Allowable values: Boolean node, allowing \emph{Y}, \emph{N}, \emph{1}, \emph{0}, \emph{true}, \emph{false}, etc.
...
The \emph{GammaSwap} script referenced in the trade above is shown in listing
\ref{lst:gamma_swap}
