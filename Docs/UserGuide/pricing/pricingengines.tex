...
\item Credit_Volatility:  The volatility of the CrLGM process, only needed when enable credit is true.
\item Credit_Reversion: The mean reversion of the CrLGM process, only neededd if enable credit is true.
\item Tolerance: Error tolerance for calibration
...
\item Regression.VarGroupMode [optional]: Global, Trivial. If not given, it defaults to Global.
  Depending on the choice the regressors are assigned to var groups as follows:
    \begin{itemize}
    \item Global: All regressors are assigned to a sinlge var group
