...
Model Parameters (Black/AnalyticalApproximation, the MonteCarlo engine requires LogNormal volatilites):
\begin{itemize}
    \item Volatility [optional]: Selects the pricing model. If omitted, defaults to AsInputVolatilityType.
    \begin{itemize}
        \item \textbf{AsInputVolatilityType}: Use Model according to volatility type of the vol curve (normal volatilities = bachelier model, lognormal volatilities = Kirk Approximation)
        \item \textbf{Normal}: Use Bachelier model, if needed log-normal input vols are converted into normal vols
        \item \textbf{LogNormal}: Use Moment-Matching approximation, if needed and possible normal input vols are converted into log-normal vols 
    \end{itemize}
\end{itemize}
...
You can set \texttt{beta} and \texttt{volatility} for each underlying or spread pair individually:
\begin{itemize}
