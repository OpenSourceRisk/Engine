%--------------------------------------------------------
\section{Pricing Engine Configuration}
%--------------------------------------------------------
...
\begin{itemize}
\item SpreadOnIncomeCurve [optional]: whether to apply the security spread on the income curve for compounding, default to true
\item TreatSecuritySpreadAsCreditSpread [optional]: If true, the security spread is not incoroporated into the benchmark curve $b$
  as described above, but rather added as a spread on top of the credit curve. Defaults to false if not given
...
\item TimestepPeriod [optional]: discretization interval for zero bond pricing, defaults to 3M if not given
\item SensitivityTemplate [optional]: the sensitivity template to use
...
<Product type="Bond">
    ...
    <ModelParameters>
      ...
      <Parameter name="TreatSecuritySpreadAsCreditSpread">false</Parameer>
    </ModelParameters>
...
%--------------------------------------------------------
\subsection{Product Type: CallableBond}
%--------------------------------------------------------

Used by trade type: CallableBond
...
LGM/Grid builds a NumericLgmCallableBondEngine using LgmConvolutionSolver as a solver. The rollback follows the paper
``Hagan, P: Methodology for callable swaps and Bermudan exercise into swaptions''. A sample configuration is shown in
listing \ref{lst:peconfig_CallableBond_LGM_Grid}
...
\end{minted}
\caption{Configuration for Product CallableBond, Model LGM, Engine Grid}
\label{lst:peconfig_CallableBond_LGM_Grid}
\end{longlisting}

LGM/FD builds a NumericLgmCallableBondEngine using LgmFdSolver as a solver using finite difference. A sample
configuration is shown in listing \ref{lst:peconfig_CallableBond_LGM_FD}
...
\end{minted}
\caption{Configuration for Product CallableBond, Model LGM, Engine FD}
...
\item TreatSecuritySpreadAsCreditSpread: If true, the security spread is not incoroporated into the benchmark curve $b$
  ...
  the bond.
\item MesherIsStatic: whether to use the same finite-difference mesher under scenario calculations, this is restricted
  to sensitivity runs
...
Used by trade type: Swaption, for European exercise, single-currency, if the configured engine does not support the
trade (e.g. underlying has cap-floored coupons), ore will fall back to the builder configured for BermudanSwaption
...
LGM/Grid builds a NumericLgmMultiLegOptionEngine using LgmConvolutionSolver as a solver. The rollback follows the paper
``Hagan, P: Methodology for callable swaps and Bermudan exercise into swaptions''. A sample configuration is shown in
...
\begin{itemize}
\item Calibration: Bootstrap, BestFit, None
\item CalibrationStrategy: None, CoterminalDealStrike, CoterminalATM, DeltaGammaAdjusted
\item FXCalibration: None, Bootstrap (only relevant for cross-currency swaption)
\item ReferenceCalibrationGrid: An optional grid, only one calibration instrument per interval is kept
\item Reversion: The mean reversion (can be attributed by currency)
\item ReversionType: Hagan, HullWhite
\item Volatility: The volatility (start value for calibration if calibrated)
\item VolatilityType: Hagan, HullWhite
\item FxVolatility: The fx volatility (start value for calibration), only relevant for cross-currency swaption, can be
  attributed by currency pair in this case
\item ShiftHorizon: Shift horizon for LGM model as fraction of deal maturity
\item Tolerance: Error tolerance for calibration
...
<Product type="EuropeanSwaption">
    ...
    <ModelParameters>
        ...
        <Parameter name="ReferenceCalibrationGrid">400,3M</Parameter>
        <Parameter name="Reversion">0.0</Parameter>
        <Parameter name="Reversion_EUR">0.0</Parameter>
        <Parameter name="Reversion_SEK">0.0</Parameter>
        <Parameter name="ReversionType">HullWhite</Parameter>
        <Parameter name="Volatility">0.01</Parameter>
        <Parameter name="VolatilityType">Hagan</Parameter>
        <Parameter name="FxVolatility">0.10</Parameter>
        <Parameter name="FxVolatility_EURSEK">0.10</Parameter>
        <Parameter name="ShiftHorizon">0.5</Parameter>
        <Parameter name="Tolerance">0.20</Parameter>
    </ModelParameters>
    <Engine>MC</Engine>
...
\end{minted}
\caption{Configuration for Product EuropeanSwaption, Model LGM, Engine MC}
\label{lst:peconfig_EuropeanSwaption_LGM_MC}
\end{longlisting}

LGM/MCCG builds a MacCgMultiLegOptionEngine. The configuration is similar to \ref{lst:peconfig_EuropeanSwaption_LGM_MC}
with some additional EngineParameters:

\begin{itemize}
\item UseAD: whether ad is used for first order sensitivity runs
\item TimeStepsPerYear: minimum time steps per year used for process evolution
...
\item IndicatorSmoothingForDerivatives: as above, smoothing parameter for indicator derivatives calculation. Defaults to $0.0$.
\item SqrtSmoothingForDerivatives: as above, smoothing parameter for sqrt derivative calculation. Defaults to $0.0$.
\end{itemize}

LGM/AMC builds a McMultiLegOptionEngine for use in AMC simulations. We refer to the AMC module documentation for further
details.

LGM/AMCCG builds a McMultiLegOptionEngine for use in AMCCG simulations. We refer to the AMC module documentation for
further details.

%--------------------------------------------------------
\subsection{Product Type: EuropeanSwaption\_XCcy}
%--------------------------------------------------------
\label{sec:EuropeanSwaption_XCcy_pt}
Used by trade type: Swaption, for European exercise, cross-currency.
...
%--------------------------------------------------------
\subsection{Product Type: EuropeanSwaption\_NonStandard}
%--------------------------------------------------------
...
Used by trade type: Swaption, for Bermudan exercise, single-currency, or fallback for European exercise on non-supported
underlying coupon types (see \ref{sec:EuropeanSwaption_pt}).
...
LGM/MCCG builds a McCgMultiLegOptionEngine. The configuration is similar to \ref{lst:peconfig_BermudanSwaption_LGM_MC}
with some additional EngineParameters:
...
%--------------------------------------------------------
\subsection{Product Type: BermudanSwaption\_XCcy}
%--------------------------------------------------------
\label{sec:BermudanSwaption_XCcy_pt}
Used by trade type: Swaption, for Bermudan exercise, cross-currency
...
%--------------------------------------------------------
\subsection{Product Type: BermudanSwaption\_NonStandard}
%--------------------------------------------------------
...
LGM/MCCG builds a McCgMultiLegOptionEngine. The configuration is similar to \ref{lst:peconfig_AmericanSwaption_LGM_MC}
with some additional EngineParameters:
...
%--------------------------------------------------------
\subsection{Product Type: AmericanSwaption\_XCcy}
%--------------------------------------------------------
\label{sec:AmericanSwaption_XCcy_pt}
Used by trade type: Swaption, for American exercise, cross-currency
...
%--------------------------------------------------------
\subsection{Product Type: IborRangeAccrualLeg}
%--------------------------------------------------------

Used by trade type: any trade with a Range Accrual leg (leg type RangeAccrual)

Available Model/Engine pairs:

\begin{itemize}
\item BGM/FloatingRateCouponPricer
\item Black/CallSpreadCouponPricer
\end{itemize}

Engine description:

BGM/FloatingRateCouponPricer builds a \lstinline!RangeAccrualPricerByBgm! for each coupon of the range accrual leg. The
pricer is based on the BGM (Brace--Gatarek--Musiela) model and uses smile sections from a swaption volatility surface to
price the range accrual condition. Normal (Bachelier) volatility surfaces are automatically converted to lognormal
(Black) volatilities internally. A sample configuration is shown in listing
\ref{lst:peconfig_IborRangeAccrualLeg_BGM_FloatingRateCouponPricer}.

The parameters have the following meaning:

\begin{itemize}
\item Correlation [optional]: The correlation parameter used by the BGM range accrual pricer. Defaults to \emph{1.0} if
  omitted.

  Allowable values: Any real number.

\item withFlatVol [optional]: If \emph{true}, a flat volatility smile section is used instead of the market swaption
  volatility surface. Defaults to \emph{false} if omitted.

  Allowable values: \emph{true}, \emph{false}.

\item FlatVol [optional]: The flat volatility value used when \lstinline!withFlatVol! is set to \emph{true}. Defaults to
  \emph{1E-6} if omitted.

  Allowable values: Any non-negative real number.

\item ByCallSpread [optional]: If \emph{true}, the range accrual condition is priced using a call-spread approximation,
  which naturally incorporates the volatility smile (each spread leg is priced at its own strike vol).
  If \emph{false}, a Black--Scholes digital pricing is used: the digital value is computed analytically using the
  volatility at the strike, without accounting for the slope of the smile. Defaults to \emph{true} if omitted.
...
\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="IborRangeAccrualLeg">
    <Model>BGM</Model>
    <ModelParameters/>
    <Engine>FloatingRateCouponPricer</Engine>
    <EngineParameters>
        <Parameter name="Correlation">1.0</Parameter>
        <Parameter name="withFlatVol">false</Parameter>
        <Parameter name="ByCallSpread">true</Parameter>
        <Parameter name="SensitivityTemplate">IR_Analytical</Parameter>
    </EngineParameters>
</Product>
\end{minted}
\caption{Configuration for Product IborRangeAccrualLeg, Model BGM, Engine FloatingRateCouponPricer}
\label{lst:peconfig_IborRangeAccrualLeg_BGM_FloatingRateCouponPricer}
\end{longlisting}

Black/CallSpreadCouponPricer builds a \lstinline!RangeAccrualPricerByCallSpread! for the range accrual leg. The pricer
uses the optionlet (capFloor) volatility surface directly and prices each daily range observation as a call-spread on
vanilla caplet/floorlet prices:
$$\text{Digital}(K) \approx \frac{C(K - \varepsilon/2,\, \sigma_{K-\varepsilon/2}) - C(K + \varepsilon/2,\, \sigma_{K+\varepsilon/2})}{\varepsilon}$$
This is the same model-free replication approach used by the \emph{RateDigitalOption} trade type. It supports both
Black (shifted-lognormal) and Bachelier (normal) optionlet volatility surfaces natively, without conversion.
A sample configuration is shown in listing
\ref{lst:peconfig_IborRangeAccrualLeg_Black_CallSpreadCouponPricer}.

The parameters have the following meaning:

\begin{itemize}
\item CallSpreadEps [optional]: The width of the call-spread in rate terms. Smaller values approach the analytical
  digital; larger values smooth the replication. Defaults to \emph{1E-4} (1 basis point) if omitted.

  Allowable values: Any positive real number.

\item withFlatVol [optional]: If \emph{true}, a flat zero-volatility surface is used instead of the market capFloor
  surface, resulting in intrinsic (zero time value) digital pricing. Defaults to \emph{false} if omitted.
...
<Product type="IborRangeAccrualLeg">
    <Model>Black</Model>
    <ModelParameters/>
    <Engine>CallSpreadCouponPricer</Engine>
    <EngineParameters>
        <Parameter name="CallSpreadEps">1E-4</Parameter>
        <Parameter name="withFlatVol">false</Parameter>
        <Parameter name="SensitivityTemplate">IR_Analytical</Parameter>
    </EngineParameters>
</Product>
\end{minted}
\caption{Configuration for Product IborRangeAccrualLeg, Model Black, Engine CallSpreadCouponPricer}
\label{lst:peconfig_IborRangeAccrualLeg_Black_CallSpreadCouponPricer}
\end{longlisting}

%--------------------------------------------------------
\subsection{Product Type: CapFlooredInterpolatedIborLeg}
%--------------------------------------------------------

Used by trade type: any trade with a cap / floored interpolated ibor / rfr term rate leg

Available Model/Engine pairs: BlackOrBachelier/BlackInterpolatedIborCouponPricer

Engine description:

BlackOrBachelier/BlackInterpolatedIborCouponPricer builds a BlackInterpolatedIborCouponPricer. A sample configuration is
shown in listing \ref{lst:peconfig_CapFlooredInterpolatedIborLeg_BlackOrBachelier_BlackInterpolatedIborCouponPricer}.
...
\end{minted}
\caption{Configuration for Product CapFlooredInterpolatedIborLeg, Model BlackOrBachelier, Engine BlackInterpolatedIborCouponPricer}
\label{lst:peconfig_CapFlooredInterpolatedIborLeg_BlackOrBachelier_BlackInterpolatedIborCouponPricer}
\end{longlisting}
...
\begin{itemize}
\item SensitivityTemplate [optional]: the sensitivity template to use
\item \item useLastFixingDate [deprecated]: not used anymore, the total
variance should always be computed from the last known fixing date. This parameter will be ignored.
\end{itemize}
...
Black/AnalyticalApproximation builds a CommodityAveragePriceOptionAnalyticalEngine.

Model Parameters:
\begin{itemize}
    \item Volatility [optional]: Selects the pricing model. If omitted, defaults to AsInputVolatilityType.
    \begin{itemize}
        \item \textbf{AsInputVolatilityType}: Use Model according to volatility type of the vol curve (normal volatilities = bachelier model, lognormal volatilities = moment-matching approximation)
        \item \textbf{Normal}: Use Bachelier model, if needed log-normal input vols are converted into normal vols
        \item \textbf{Lognormal}: Use first-two-moment Turnbull-Wakeman approximation, if needed and possible normal input vols are converted into log-normal vols
        \item \textbf{ShiftedLognormal}: Use first-two-moment Turnbull-Wakeman approximation, if needed convert vols in shifted lognormal vols with a given displacement
    \end{itemize}
    \item Displacement [optional]: Only relevant for volatility = \textbf{ShiftedLognormal}. The displacement parameter in the shifted lognormal model. Defaults to 0 if omitted.
\end{itemize}

The correlation between two future contracts is parametrized as
...
You can set \texttt{beta}, \texttt{volatility} and \texttt{displacement} for each underlying individually:
\begin{itemize}
    \item \texttt{beta\_NYMEX:CL} sets beta for Average Price Options with NYMEX:CL underlying.
    \item \texttt{volatility\_NYMEX:CL} sets the model used to price Average Price Options with NYMEX:CL underlying.
\end{itemize}
...
Only Lognormal input volatilities are supported in the MonteCarlo pricer.

%--------------------------------------------------------
\subsection{Product Type: CommodityAveragePriceBarrierOption}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: CommodityForward}
%--------------------------------------------------------
...
If true, it apply a spread to the individual constituent curves to match the index CDS spread.
...
\end{minted}
\caption{Configuration for Product IndexCreditDefaultSwapOption, Model Black, Engine BlackIndexCdsOptionEngine}
...
\end{minted}
\caption{Configuration for Product IndexCreditDefaultSwapOption, Model LognormalAdjustedIndexSpread, Engine NumericalIntegrationEngine}
\label{lst:peconfig_CreditDefaultSwapOption_LognormalAdjustedIndexSpread_NumericalIntegrationEngine}
\end{longlisting}

%--------------------------------------------------------
\subsection{Product Type: CpiCapFloor}
%--------------------------------------------------------
...
\item useAssumedRecovery: whether to use specialized default curves, which are bootstrapped using the assumed recovery rate
\item recoveryRateGrid: recovery rate grid for the recovery rate grid
\item recoveryRateProbabilities: recovery rate probabilities for the recovery rate grid
...
The recovery rate grid and probabilities are defined as a comma-separated list of values. The previous allowed values \emph{Markit2020} and \emph{constant} are deprecated and
...
\item RequiredTolerance: max tolerance for MC error, if 0 (not specified) RequiredSamples must be given
...
%--------------------------------------------------------
\subsection{Product Type: CommodityAsianOptionArithmeticStrike}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: CommodityAsianOptionGeometricPrice}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: CommodityAsianOptionGeometricStrike}
%--------------------------------------------------------
...
\begin{itemize}
\item BlackScholes/CommoditySpreadOptionEngine
\end{itemize}

\subsubsection{BlackScholes/CommoditySpreadOptionEngine}

This engine derives the spread volatility from the individual volatility surfaces of both underlying commodities and
a correlation assumption. It reads individual commodity vol surfaces from the market and computes the spread option
price using either the Bachelier model or the Kirk approximation.

Model Parameters:
\begin{itemize}
    \item Volatility [optional]: Selects the pricing model. If omitted, defaults to AsInputVolatilityType.
    \begin{itemize}
        \item \textbf{AsInputVolatilityType}: Use model according to the volatility type of the vol curve (normal
          volatilities use Bachelier model, lognormal volatilities use Kirk Approximation).
        \item \textbf{Normal}: Use Bachelier model. Both underlyings are assumed to follow a normal (arithmetic
          Brownian motion) process. The spread $S = F_1 - F_2$ is then also normally distributed and the spread
          volatility is computed from the individual volatilities and the correlation as
          $\sigma_{\text{spread}} = \sqrt{\sigma_1^2 + \sigma_2^2 - 2\rho\,\sigma_1\,\sigma_2}$.
          If the input vol curves are lognormal, they are converted to normal vols.
        \item \textbf{Lognormal}: Use Kirk approximation, if needed and possible normal input vols are converted into
          log-normal vols.
        \item \textbf{ShiftedLognormal}: Use Kirk approximation where both underlyings follow a shifted lognormal
          process, if needed and possible input vols are converted into shifted lognormal vols with a given displacement.
    \end{itemize}
    \item Displacement [optional]: Only relevant for volatility = \textbf{ShiftedLognormal}. The displacement parameter
      in the shifted lognormal model. Defaults to 0 if omitted.
\end{itemize}

Engine description:

BlackScholes/CommoditySpreadOptionEngine builds a CommoditySpreadOptionAnalyticalEngine, which uses the Bachelier model or the Kirk
approximation described in Iain J. Clark, Commodity Option Pricing, Section 2.9. The correlation between two different commodities are
taken from correlation termstructure and the intra-asset correlation between two future contracts of the same underlying (calendar spread)
is parametrized as
...
You can set \texttt{beta}, \texttt{volatility} and \texttt{displacement} for each underlying or spread pair individually:
\begin{itemize}
    \item \texttt{beta\_NYMEX:CL} sets beta for WTI calendar spreads.
    \item For spreads between two different underlyings, use \texttt{volatility\_underlying1\_underlying2} as the parameter name.
\end{itemize}

Example configuration:
\begin{itemize}
    \item NYMEX:CL calendar spreads use the Bachelier model (\texttt{Volatility = normal}) with beta 3.05.
    \item All other calendar spreads use the Kirk approximation (\texttt{Volatility = lognormal}) with beta 2.05.
    \item Spreads between NYMEX:CL and ICE:B also use the Bachelier model.
    \item All other spread pairs default to use the Kirk approximation.
\end{itemize}
...
<Product type="CommoditySpreadOption">
    <Model>BlackScholes</Model>
    <ModelParameters>
        <Parameter name="Volatility_NYMEX:CL">normal</Parameter>
        <Parameter name="Volatility_NYMEX:CL_ICE:B">normal</Parameter>
    </ModelParameters>
    <Engine>CommoditySpreadOptionEngine</Engine>
    <EngineParameters>
        <Parameter name="beta">2.05</Parameter>
        <Parameter name="beta_NYMEX:CL">3.05</Parameter>
        <Parameter name="SensitivityTemplate">COMM_Analytical</Parameter>
    </EngineParameters>
    </Product>
\end{minted}
...
%--------------------------------------------------------
\subsection{Product Type: CommodityCalendarSpreadOption}
%--------------------------------------------------------

Used by trade type: CommodityCalendarSpreadOption

Available Model/Engine pairs:

\begin{itemize}
\item BlackScholes/CommoditySpreadOptionEngine
\item BlackScholes/CommodityCalendarSpreadOptionBachelierEngine
\end{itemize}

\subsubsection{BlackScholes/CommoditySpreadOptionEngine}

This is the same engine as described for CommoditySpreadOption above. It derives the spread volatility from the
individual volatility surfaces of the two underlying commodity contracts and uses a correlation assumption to compute a
calendar spread option price via either the Bachelier model or the Kirk approximation. All model parameters, engine
parameters, and the qualifier-based parameter lookup described in the CommoditySpreadOption section apply here as well.

See section \ref{lst:peconfig_CommoditySpreadOption_BlackScholes_CommoditySpreadOptionEngine} for configuration details.

\subsubsection{BlackScholes/CommodityCalendarSpreadOptionBachelierEngine}

This engine prices calendar spread options using a directly quoted normal (Bachelier) spread volatility surface. It is
designed specifically for calendar spreads where both legs reference the same underlying commodity but different contract
months.

\textbf{How it works:}

The engine constructs the spread vol curve name from the underlying commodity name and the calendar spread offset as

\texttt{<underlying>\_CALENDAR\_SPREAD\_<offset>}

where \texttt{offset} is the number of contract months between the long and short legs (e.g.\ for a spread between the
1st and 2nd nearby contracts, offset = 1). It then looks up the corresponding commodity volatility surface from the
market and prices the option using the Bachelier formula with the discount curve and the spread volatility.

\textbf{Volatility Curve Setup:}

The spread volatility surface must be configured as a \texttt{CommodityVolatility} curve in the curve configuration
using the naming convention \texttt{<underlying>\_CALENDAR\_SPREAD\_<offset>}. The volatility type should be Normal.
For example, for WTI 1-month calendar spreads, the curve would be named \texttt{NYMEX:CL\_CALENDAR\_SPREAD\_1}.

There are two ways to set up the curve configuration:

\begin{enumerate}
\item \textbf{Using \texttt{InstrumentType = COMMODITY\_CALENDAR\_SPREAD\_OPTION}}: Set the instrument type to
  \texttt{COMMODITY\_CALENDAR\_SPREAD\_OPTION} and provide the additional fields \texttt{CalendarSpreadOffset} (the
  offset between the two contract months) and \texttt{CalendarSpreadUnderlyingName} (the underlying commodity name,
  e.g.\ \texttt{NYMEX:CL}). These must be consistent with the \texttt{CurveId}. With this setup, a calendar spread
  forward curve will be built automatically from the underlying commodity forward curve and the commodity future
  conventions if it is not explicitly provided. The quote names follow the pattern
  \texttt{COMMODITY\_CALENDAR\_SPREAD\_OPTION/RATE\_NVOL/<underlying>/<offset>/<ccy>/...}.

\item \textbf{Using \texttt{InstrumentType = COMMODITY\_OPTION} (default)}: Use the regular instrument type with normal
  volatility quotes. The \texttt{CurveId} must still follow the
  \texttt{<underlying>\_CALENDAR\_SPREAD\_<offset>} naming convention. In this case, the calendar spread forward curve
  must be explicitly provided as a commodity price curve in the today's market parameters --- it will not be built
  automatically. The quote names follow the standard pattern
  \texttt{COMMODITY\_OPTION/RATE\_NVOL/<curveId>/<ccy>/...}.
\end{enumerate}

\textbf{Engine Parameters:}
...
A sample configuration is shown in listing
\ref{lst:peconfig_CommodityCalendarSpreadOption_BlackScholes_BachelierEngine}.

\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="CommodityCalendarSpreadOption">
    <Model>BlackScholes</Model>
    <ModelParameters/>
    <Engine>CommodityCalendarSpreadOptionBachelierEngine</Engine>
    <EngineParameters>
        <Parameter name="SensitivityTemplate">COMM_Analytical</Parameter>
    </EngineParameters>
</Product>
\end{minted}
\caption{Configuration for Product CommodityCalendarSpreadOption, Model BlackScholes, Engine CommodityCalendarSpreadOptionBachelierEngine}
\label{lst:peconfig_CommodityCalendarSpreadOption_BlackScholes_BachelierEngine}
\end{longlisting}
...
\begin{itemize}
    \item Volatility [optional]: Selects the pricing model. If omitted, defaults to AsInputVolatilityType.
    \begin{itemize}
        \item \textbf{AsInputVolatilityType}: Use Model according to volatility type of the vol curve
        \item \textbf{Normal}: Use Bachelier model, if needed log-normal input vols are converted into normal vols
        \item \textbf{Lognormal}: Use Black model, if needed and possible normal input vols are converted into log-normal vols
        \item \textbf{ShiftedLognormal}: Use shifted Black model with displacement, if needed convert vols in shifted lognormal vols with the given displacement
    \end{itemize}
    \item Displacement [optional]: Only relevant for volatility = \textbf{ShiftedLognormal}. The displacement parameter in the shifted lognormal model. Defaults to 0 if omitted.
\end{itemize}

You can set \texttt{volatility} and \texttt{displacement} for each underlying individually:
\begin{itemize}
    \item \texttt{volatility\_NYMEX:CL} sets the model used to price options with NYMEX:CL as underlying.
    \item \texttt{displacement\_NYMEX:CL} set the displacement parameter for the shifted Black model for options with NYMEX:CL as underlying.
\end{itemize}
...
%--------------------------------------------------------
\subsection{Product Type: CommodityOptionForward}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: CommoditySwaption}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: EquityAsianOptionArithmeticStrike}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: EquityAsianOptionGeometricPrice}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: EquityAsianOptionGeometricStrike}
%--------------------------------------------------------
...
Its possible to price this product by replication. For the replication approach no dedicated pricing engine configuration for the product is needed.

There is alternative method to price it as a scripted trade.
...
ScriptedTrade/ScriptedTrade delegates to the scripted trade engine and the associated pricing engine configuration for
Product Type ScriptedTrade, see there for details. A sample configuration is given in listing
\ref{lst:peconfig_FxEuropeanBarrierOption_ScriptedTrade_ScriptedTrade}. Only FxBarrierOptions are supported at the moment.
Typical use case for this pricing engine config would XVA calculation within the AMC framework.
...
\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="FxEuropeanBarrierOption">
    <Model>ScriptedTrade</Model>
    <ModelParameters/>
    <Engine>ScriptedTrade</Engine>
    <EngineParameters>
...
\end{minted}
\caption{Configuration for Product FxEuropeanBarrierOption, Model ScriptedTrade, Engine ScriptedTrade}
\label{lst:peconfig_FxEuropeanBarrierOption_ScriptedTrade_ScriptedTrade}
\end{longlisting}
...
\begin{itemize}
\item BlackScholesMerton/AnalyticEuropeanEngine (equity)
\item GarmanKohlhagen/AnalyticEuropeanEngine (fx)
\item GarmanKohlhagen/CallSpreadEngine (fx)
\end{itemize}

Engine description:

For equity, BlackScholesMerton/AnalyticEuropeanEngine builds an AnalyticEuropeanEngine.

For fx, GarmanKohlhagen/AnalyticEuropeanEngine builds a cash-settled European engine (AnalyticCashSettledEuropeanEngine),
which correctly handles both a payment date equal to and a payment date after the option expiry date. The engine name
AnalyticEuropeanEngine is retained for backward compatibility with existing pricing engine configurations. Alternatively,
GarmanKohlhagen/CallSpreadEngine builds an FxDigitalCallSpreadEngine which prices the digital by call/put spread
replication, thereby naturally incorporating the volatility smile. A sample configuration is shown in listing
\ref{lst:peconfig_FxDigitalOption_GarmanKohlhagen_AnalyticEuropeanEngine} for fx (analytic engine) and in listing
\ref{lst:peconfig_FxDigitalOption_GarmanKohlhagen_CallSpreadEngine} for fx (call spread engine). The configuration for
equity is identical to the analytic fx configuration except the Model is set to BlackScholesMerton.

The parameters have the following meaning:

\begin{itemize}
\item CallSpreadEps [optional, CallSpreadEngine only]: the width of the call/put spread used for the replication,
  defaults to $10^{-4}$
\item SensitivityTemplate [optional]: the sensitivity template to use
\end{itemize}
...
\begin{minted}[fontsize=\footnotesize]{xml}
 <Product type="FxDigitalOption">
    <Model>GarmanKohlhagen</Model>
    <ModelParameters/>
    <Engine>CallSpreadEngine</Engine>
    <EngineParameters>
        <Parameter name="CallSpreadEps">1E-4</Parameter>
        <Parameter name="SensitivityTemplate">FX_Analytical</Parameter>
    </EngineParameters>
...
\end{minted}
\caption{Configuration for Product FxDigitalOption, Model GarmanKohlhagen, Engine CallSpreadEngine}
\label{lst:peconfig_FxDigitalOption_GarmanKohlhagen_CallSpreadEngine}
\end{longlisting}
...
\end{minted}
\caption{Configuration for Product EquityForward, Model DiscountedCashflows, Engine DiscountingEquityForwardEngine}
\label{lst:peconfig_EquityForward_DiscountedCashflows_DiscountingEquityForwardEngine}
\end{longlisting}

%--------------------------------------------------------
\subsection{Product Type: EquityFutureOption}
%--------------------------------------------------------
...
\end{minted}
\caption{Configuration for Product EquityFutureOption, Model BlackScholes, Engine AnalyticEuropeanForwardEngine}
\label{lst:peconfig_EquityFutureOption_BlackScholes_AnalyticEuropeanForwardEngine}
\end{longlisting}

%--------------------------------------------------------
\subsection{Product Type: EquityLeg}
%--------------------------------------------------------

Used by trade type: any trade referencing an Equity leg

Available Model/Engine pairs:

\begin{itemize}
  \item DiscountedCashflows/EquityCouponPricer
  \item BlackQuantoAdjusted/BlackQuantoEquityCouponPricer
\end{itemize}

Engine description:

DiscountedCashflows/EquityCouponPricer is the default engine pricer for equity legs. BlackQuantoAdjusted/BlackQuantoEquityCouponPricer
will apply a convexity adjustments following 
\[e^{\sigma_{\text{eq}}(t) \cdot \sigma_{\text{fx}}(t) \cdot \text{corr}(\text{eq}, \text{fx}, t) \cdot t}\]
A sample configuration is shown in listing
\ref{lst:peconfig_EquityLeg_BlackQuantoAdjusted_BlackQuantoEquityCouponPricer}.

DiscountedCashflows/EquityCouponPricer does not take any parameters, while the parameters for BlackQuantoAdjusted/BlackQuantoEquityCouponPricer have the following meaning:

\begin{itemize}
\item ApplyConvexityAdjustment: whether convexity adjustment will be used for equity swap.
\item QuantoCurrencyPairs: the currency pairs that the convexity adjustment will apply on.

\end{itemize}

\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="EquityLeg">
    <Model>BlackQuantoAdjusted</Model>
    <ModelParameters/>
    <Engine>BlackQuantoEquityCouponPricer</Engine>
    <EngineParameters>
        <Parameter name="ApplyConvexityAdjustment">true</Parameter>
        <Parameter name="QuantoCurrencyPairs">EURUSD</Parameter>
    </EngineParameters>
</Product>
\end{minted}
\caption{Configuration for Product EquityLeg, Model BlackQuantoAdjusted, Engine BlackQuantoEquityCouponPricer}
\label{lst:peconfig_EquityLeg_BlackQuantoAdjusted_BlackQuantoEquityCouponPricer}
\end{longlisting}
...
%--------------------------------------------------------
\subsection{Product Type: EquityOptionEuropeanCS}
%--------------------------------------------------------
...
BlackScholesMerton/FdBlackScholesVanillaEngine builds a FdBlackScholesVanillaEngine. A sample configuration is shown in listing
\ref{lst:peconfig_QuantoEquityOptionAmerican_BlackScholesMerton_FdBlackScholesVanillaEngine}
...
\label{lst:peconfig_QuantoEquityOptionAmerican_BlackScholesMerton_FdBlackScholesVanillaEngine}
\end{longlisting}
...
%--------------------------------------------------------
\subsection{Product Type: BondFuture}
%--------------------------------------------------------

Used by trade type: BondFuture

Available Model/Engine pairs: DiscountedCashflows/DiscountingBondFutureEngine

Engine description:

DiscountedCashflows/DiscountingBondFutureEngine builds a DiscountingBondFutureEngine. A sample configuration is shown
in listing \ref{lst:peconfig_BondFuture_DiscountedCashflows_DiscountingBondFutureEngine}.
...
\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="BondFuture">
    <Model>DiscountedCashflows</Model>
    <ModelParameters/>
    <Engine>DiscountingBondFutureEngine</Engine>
    <EngineParameters/>
        <Parameter name="SensitivityTemplate">IR_Analytical</Parameter>
    </EngineParameters>
</Product>
\end{minted}
\caption{Configuration for Product BondFuture, Model DiscountedCashflows, Engine BondFutureEngine}
\label{lst:peconfig_BondFuture_DiscountedCashflows_DiscountingBondFutureEngine}
\end{longlisting}

%--------------------------------------------------------
\subsection{Product Type: BondFutureOption}
%--------------------------------------------------------

Used by trade type: BondFutureOption

Available Model/Engine pairs: BlackScholes/AnalyticEuropeanEngine

Engine description:

BlackScholes/AnalyticEuropeanEngine builds an AnalyticEuropeanEngine. A sample configuration is shown in listing
\ref{lst:peconfig_BondFutureOption_BlackScholes_AnalyticEuropeanEngine}

The parameters have the following meaning:

\begin{itemize}
\item SensitivityTemplate [optional]: the sensitivity template to use
\item SeparateCallPutVols [optional]: This parameter can be set to true or false or to a regex inclusion pattern.
...
\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="BondFutureOption">
    <Model>BlackScholes</Model>
    <ModelParameters/>
    <Engine>AnalyticEuropeanEngine</Engine>
    <EngineParameters>
        <Parameter name="SeparateCallPutVols">True</Parameter>
    </EngineParameters>
</Product>
\end{minted}
\caption{Configuration for Product BondFutureOption, Model BlackScholes, Engine AnalyticEuropeanEngine}
\label{lst:peconfig_BondFutureOption_BlackScholes_AnalyticEuropeanEngine}
\end{longlisting}

%--------------------------------------------------------
\subsection{Product Type: BondFutureOptionAmerican}
%--------------------------------------------------------

Used by trade type BondFutureOption if exercise style is American
...
BlackScholes/FdBlackScholesVanillaEngine builds a FdBlackScholesVanillaEngine. A sample configuration is shown in listing
\ref{lst:peconfig_BondFutureOptionAmerican_BlackScholes_FdBlackScholesVanillaEngine}
...
\item TimeGridMinimumSize [optional]: Minimum number in resulting time grid, defaults to $1$ if not given
\item SensitivityTemplate [optional]: the sensitivity template to use
\item SeparateCallPutVols [optional]: if set to true, there will be a separate volatility surface 
    used to price puts and calls on the same underlying bond future contract. The default is false 
    where the same volatility surface is used for puts and calls.
\end{itemize}

\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="BondFutureOptionAmerican">
  <Model>BlackScholes</Model>
  <ModelParameters/>
  <Engine>FdBlackScholesVanillaEngine</Engine>
  <EngineParameters>
    <Parameter name="Scheme">Douglas</Parameter>
    <Parameter name="TimeGridPerYear">365</Parameter>
    <Parameter name="XGrid">100</Parameter>
    <Parameter name="DampingSteps">0</Parameter>
    <Parameter name="EnforceMonotoneVariance">true</Parameter>
    <Parameter name="SeparateCallPutVols">True</Parameter>
...
\end{minted}
\caption{Configuration for Product BondFutureOptionAmerican, Model BlackScholes, Engine FdBlackScholesVanillaEngine}
\label{lst:peconfig_BondFutureOptionAmerican_BlackScholes_FdBlackScholesVanillaEngine}
\end{longlisting}

BlackScholes/BaroneAdesiWhaleyApproximationEngine builds a BaroneAdesiWhaleyApproximationEngine. A sample configuration is 
shown in listing \ref{lst:peconfig_BondFutureOptionAmerican_BlackScholes_BaroneAdesiWhaleyApproximationEngine}
...
\end{minted}
\caption{Configuration for Product BondFutureOptionAmerican, Model BlackScholes, Engine BaroneAdesiWhaleyApproximationEngine}
\label{lst:peconfig_BondFutureOptionAmerican_BlackScholes_BaroneAdesiWhaleyApproximationEngine}
\end{longlisting}

As outlined in Section~\ref{sec:pe-global-parameters}, there is a global parameter \lstinline!BondFutureAmericanAsEuropean! 
that can be used to price all bond future option trades, or bond future option trades on certain underlying bond future 
contracts, with American exercise as if they have European exercise i.e. with a European bond future option engine.
...
%--------------------------------------------------------
\subsection{Product Type: FxAsianOptionArithmeticStrike}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: FxAsianOptionGeometricPrice}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: FxAsianOptionGeometricStrike}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: RateDigitalOption}
%--------------------------------------------------------

Used by trade type: RateDigitalOption

Available Model/Engine pairs:

\begin{itemize}
\item Black/CallSpreadEngine
\end{itemize}

Engine description:

Black/CallSpreadEngine builds a \lstinline!RateDigitalCallSpreadEngine! that prices a European cash-or-nothing digital
option on an interest rate fixing via call-spread replication. For a digital call paying $P$ if the fixing $L > K$:

$$\text{PV} = P \times \frac{C(K - \varepsilon/2,\, \sigma_{-}) - C(K + \varepsilon/2,\, \sigma_{+})}{\varepsilon}$$

where $C(K, \sigma)$ is the vanilla caplet/floorlet price at strike $K$ using the market optionlet volatility $\sigma(K)$
from the cap/floor volatility surface. The formula for a digital put is analogous using put prices. Both Black
(shifted-lognormal) and Bachelier (normal) volatility types are supported automatically depending on the optionlet
surface type.

A sample configuration is shown in listing
\ref{lst:peconfig_RateDigitalOption_Black_CallSpreadEngine}.

The parameters have the following meaning:

\begin{itemize}
\item CallSpreadEps [optional]: The width of the call-spread in rate terms. Smaller values approximate the analytical
  digital more closely; larger values smooth the replication. Defaults to \emph{1E-4} (1 bp) if omitted.

  Allowable values: Any positive real number.

\item withFlatVol [optional]: If \emph{true}, replaces the market optionlet volatility surface with a flat zero surface,
  effectively pricing the digital at its intrinsic value. Defaults to \emph{false} if omitted.
...
\begin{longlisting}
\begin{minted}[fontsize=\footnotesize]{xml}
<Product type="RateDigitalOption">
    <Model>Black</Model>
    <ModelParameters/>
    <Engine>CallSpreadEngine</Engine>
    <EngineParameters>
...
\end{minted}
\caption{Configuration for Product RateDigitalOption, Model Black, Engine CallSpreadEngine}
\label{lst:peconfig_RateDigitalOption_Black_CallSpreadEngine}
\end{longlisting}
...
%--------------------------------------------------------
\subsection{Product Type: RiskParticipationAgreement\_Vanilla\_XCcy}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: RiskParticipationAgreement\_Structured}
%--------------------------------------------------------
...
%--------------------------------------------------------
\subsection{Product Type: ScriptedTrade}
%--------------------------------------------------------
...
\label{sec:pe-global-parameters}
%--------------------------------------------------------
...
\item AllowModelFallbacks: If set to true a missing market object during the build of a model will not cause the build
  to fail. Instead, a fallback will be used. For example, if a volatility surface is missing during the build of the LGM
  or CAM model, a zero volatility surface might be used as a fallback and the calculation continues. Optional, defaults
  to false.
...
\item StrikeSpread: Defaults to 0.1. If set, overrides the default value used to calculate the strike spread for replication 
  engines.
\item BondFutureAmericanAsEuropean: This parameter can be set to true or false or to a regex inclusion pattern.
  If set to false, the default, there is no effect.
  If set to true, all bond future option trades with American exercise are priced as if they had European exercise 
  i.e.\ with a European bond future option engine.
  If set to a regex pattern, all bond future option trades where the underlying contract name matches the regex 
  will be priced as if they had European exercise. Bond future option trades where the underlying contract name 
  does not match the regex will not be impacted i.e. if they have American exercise, they will be priced as such.
\end{itemize}
