...
For example, you can define a grid for different CDS tiers, such as SNRFOR. The parameter name for a specific grid is `recoveryRateGrid_SNRFOR`, 
and the corresponding probabilities are `recoveryRateProbabilities_SNRFOR`.
...
you can add the `IndexSubFamily` identifier to the index credit reference data. 
This allows you to set up independent grids for groups of credit indices, 
such as a grid for CDX High Yields Tranches `recoveryRateGrid_CDX.NA.HY_SNRFOR`.

The recovery rate grid and probabilities are defined as a comma-separated list of values. The previous allowed values are deprecated and 
will cause a warning. If recovery rate grid is set to Markit2020, a 3-pillar recovery grid $[0.1, R, 2R-0.1]$ is used.
