...
\begin{itemize}
    \item Volatility [optional]: Selects the pricing model. If omitted, defaults to AsInputVolatilityType.
    \begin{itemize}
        \item \textbf{AsInputVolatilityType}: Use Model according to volatility type of the vol curve (normal volatilities use Bachelier model, lognormal volatilities use Kirk Approximation)
        \item \textbf{Normal}: Use Bachelier model, if needed log-normal input vols are converted into normal vols
        \item \textbf{Lognormal}: Use Kirk approximation, if needed and possible normal input vols are converted into log-normal vols
        \item \textbf{ShiftedLognormal}: Use Kirk approximation where both underlyings following a shifted lognormal process, if needed and possible input vols are converted into a shifted lognormal vols with a given displacement.
    \end{itemize}
    \item Displacement [optional]: Only relevant for volatility = \textbf{ShiftedLognormal}. The displacement parameter in the shifted lognormal model. Defaults to 0 if omitted.
\end{itemize}
