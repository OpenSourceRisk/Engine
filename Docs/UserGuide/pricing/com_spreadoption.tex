...
European Commodity Spread Options are priced using the Kirk approximation described in Section 2.9.2 of \cite{Clark_2014} or using the Bachelier model.

\textbf{Bachelier Model}

We assume that the spot commodity prices $S^1$ and $S^2$ follow two correlated Arithemtic Brownian motions.
...
$S_1(T)$ and $S_2(T)$ are both normal distributed, the difference $Z(T)=S_1(T) -S_2(T)$ is also normally distributed, with variance 
...
\begin{equation}
\left[ \omega \left( Z(T) - K \right) \right]^{+} N
\end{equation}

We can then use the Bachelier formula to compute the option price. See Bachelier Model, Section \ref{sec:models}, for more details.

\textbf{Kirk Approximation}
