...
\begin{table}[h]
  \begin{tabular}{p{5cm}|p{11cm}}
    ...
    pricing model. The default value is true. \\ \hline
...
number of time steps per year (specified as \verb+Bootstrap.TimeStepsPerYear+) is used for the numerical solution of the
PDE later on, i.e.
...
concentrated around and containing $\ln(S)$ as a grid point will be constructed following the approach described in
...
\item div\_passth: dividend passthroughs:
  \begin{itemize}
    \item \verb+@0.17+: dividend pass-through event with threshold $0.17$
  \end{itemize}
