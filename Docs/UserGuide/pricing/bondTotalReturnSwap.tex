...
P_D(t,t_i)$ in the discount factors $\bar{P}(t,t_i)$
...
\underline{Limitation of the method}

The model assumes a \textbf{golden CSA}, which is common for most derivative trades.
However, in practice, TRS contracts often override the CSA.
TRS agreements typically specify a simple accrual based PV calculation for variation margin, calculated daily.
This makes TRS margining similar to repo transactions, which use the same approach.
As a result, variation margin disputes are rare for TRS trades.
However, the accrual PV method has several weaknesses (please compare the limitation of the generic TRS model) and usually produces values that differ from Bond TRS model PVs.
This means the golden CSA assumption is often not valid for TRS. Nonetheless, it is industry practice.

\smallskip

The model ignores \textbf{gap and wrong way risk}, please compare the corresponding section within \ref{pricing:limitation_trs_wrong_way_risk} of the generic TRS model.
...
\item when bond defaults, close out at market value is assumed, i.e. the funding leg interest is valued using CSA discounting for closeout.
\item interest and hazard rates are assumed deterministic.
\item the underlying is assumed to be non-callable.
