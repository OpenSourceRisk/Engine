...
RA(K_s) = \frac{\text{NPV}_{K,cpn} + Acc_{cpn}}{N_{vd} \cdot K_s \cdot S_K(t_e) \cdot P_{OIS}(0,t_e)}
\end{equation}
...
F_p = 1- \frac{\text{NPV}}{N_{vd} \cdot P_{*}(0,t_e)} - \frac{\text{FEP}}{N_{vd} \cdot P_{OIS,t}(0,t_e)}
\end{equation}
...
\begin{equation}\label{pricing:cr_adjusted_strike_spread}
...
\begin{equation}\label{pricing:cr_black_spread-volatility}
  \text{NPV}_{opt} = P_{OIS,t}(0,t_e) \cdot S(t_e) \cdot  RA \cdot N_{vd} \cdot \text{Black} \left( K_s', S_p, \sigma\sqrt{t_e}, \omega \right)
\end{equation}

with a call (protection buyer) / put (protection seller) indicator $\omega = 1, -1$ and a volatility $\sigma$
corresponding to the strike $K$ and expiry $t_e$.
...
The unrealized FEP adjusted forward price $F_{p,u}$ is calculated as

\begin{equation}\label{pricing:cr_indexcdsoption_ufep_adjusted_forward}
F_{p,u} = 1- \frac{\text{NPV}}{N_{vd} \cdot P_{*}(0,t_e)} - \frac{\text{FEP}_u}{N_{vd}}
\end{equation}

The unrealized FEP adjusted forward spread $S_{p,u}$ is modeled as a lognormal variable

\begin{equation}\label{pricing:cr_indexcdsoption_Sp2}
S_{p,u}(x) = m e^{-\frac{1}{2} \sigma^2 t_e + x \sigma \sqrt{t_e}}
\end{equation}
...
The unrealized FEP adjusted index value $V_c(S_{p,u})$ as a function of the unrealized FEP adjusted forward spread
$S_{p,u}$ is then given as
...
triangle'' $\lambda \approx S_{p,u}/(1-R)$ where $R$ is the index recovery rate (used to bootstrap the index CDS
curve). This yields
...
\begin{eqnarray}
w &=& \left(\frac{S_{p,u}}{1-R} + r_A \right) \cdot (t_m-t_e) \\
...
The free parameter $m$ from \ref{pricing:cr_indexcdsoption_Sp2} is calibrated to $F_{p,u}$
(see \ref{pricing:cr_indexcdsoption_ufep_adjusted_forward}) by solving

\begin{equation}
\int_{-\infty}^{\infty} V_c(x,m) \, \phi(x) \, dx = 1 - F_{p,u}
\end{equation}
...
with $x_L= x^*, x_U=\infty$ for a call option ($\omega=1$) or $x_L=-\infty, x_U=x^*$ for a put option ($\omega=-1$). As
...
\begin{table}[hbt]
  \begin{tabular}{|l|l|}
    ...
    $F_o$                     & fepAdjustedForwardPrice \\
    $F_{p,u}$                  & unrealizedFepAdjustedForwardPrice \\
    $S_p$                     & fepAdjustedForwardSpread \\
    $S_{p,u}$                  & unrealizedFepAdjustedForwardSpread \\
    $RA$                      & riskyAnnuity \\
