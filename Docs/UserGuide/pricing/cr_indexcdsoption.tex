...
stochastic quantity on the other hand. The adjusted strike spread $K_s'$ is calculated as
...
adjusted index value $V_c(S_P)$ as a function of the FEP-adjusted forward spread $S_P$ is given as

\begin{equation}
V_c = (S_P - c)\cdot a
\end{equation}

Note: Contrary to the Black pricing approach where we approximated $S_P$ by a deterministic value,
see \ref{pricing:cr_indexcdsoption_Sp}, we now model $S_P$ as a stochastic quantity. We also approximate the true risky
annuity by a continuous risky annuity $a$ to speed up calculations. The continuous risky annuity is defined as
