%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\subsubsection{Bermudan/American Swaption}
\label{pricing:ir_bermswaption}

Bermudan/American Swaptions are priced using a one-factor Linear Gauss Markov (LGM) model, which
...
(constant) value given as the parameter ``Volatility'' will be used as the initial guess for the calibration in this
...
too), ``CoterminalATM'', ``CoterminalDealStrike'', ``DeltaGammaAdjusted''.
...
In case of ``DeltaGammaAdjusted'' the calibration swaption strikes and maturity are adjusted to the exercise-into
underlying following \cite{Andersen_Piterbark_2010}, section 19.4.3.
...
where $x$ is the LGM model state, see \cite{Hagan_LGM}, section 4.1, and $P(0,t)$ is the market zero bond with maturity
