...
In the normal case the discrete average is again normal distributed. Once we know the expected value and the variance of the average, we can 
compute the option price with the Bachelier formula. The expected value is given as

$$
\mu_A = \frac{1}{n} \sum_{i=0}^n F(0, t_i)
$$

and the variance is

$$
var_A =  \frac{1}{n^2} \sum_{i,j=1}^n \rho_{T(t_i), T(t_j)} \sigma_{T(t_i)} \sigma_{T(t_j)} t_{\min(i,j)}.
$$
