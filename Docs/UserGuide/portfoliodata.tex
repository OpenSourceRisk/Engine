...
not being rules based (the sub-node Rules doesn't exist), the \lstinline!ScheduleData! node includes a \lstinline!Dates! sub-node
where the schedule is determined directly by \lstinline!Date! child elements. The schedule can also be generated from a
set of rules based on the entries of the sub-node Rules: StartDate, EndDate, Tenor, Calendar, Convention, TermConvention, and Rule
...
Convention, TermConvention, and Rule. If not existing, the Dates sub-node is required.
...
\item Rule: Rule for the generation of the schedule using given
...
\item Dates: This is an alternative sub-node to Rules and contains child elements of type
