...
This section documents the OpenCL framework.
...
Input variables, input variates and intermediate result variables $v_i$ are numbered as $i= 0, 1, 2, 3, \ldots$. They occur
as contingent blocks in this sequence, first the input variables, then the variates and then the intermediate
results.
...
being an output to retrieve from the calculation.
...
\item for conditional expectation, intermediate result variables for all arguments (filter, regressor1, ..., regressorn)
  and the actual result of the operation (the conditional expectation) are generated. This allows us to easily pass the
  arguments and result values from device to host and bock if the calculation of the conditional expectation is
  performed on the host.
\end{itemize}

The result variable ids are generated by \verb+generateResultId()+. This function will reuse variable ids that were
freed by \verb+freeVariable()+, provided that the freed variable is not an input variable or input variate.

The different variable types are represented as follows, \verb+w+ denoting the global work id.

\begin{itemize}
\item input variables: {\tt input[offset + w]}, with input a buffer in the global address space
\item input variates: {\tt rn[offset + w]}, with rn a buffer in the global address space
\item intermediate variables: \verb+v_i+, i.e. a local variable in the generated kernel code
\item output variables: {\tt output[offset + w]}, with output a buffer in the global address space
\end{itemize}

There is an additional buffer \verb+values+ that is used to load local variables from and cache local variables to
in multipart kernels. A kernel can be split into multiple parts for several reasons:

\begin{itemize}
\item conditional expectations require a kernel to be stopped, so that the non-pathwise operation can be performed by
  another kernel or on the host side. A new kernel part will be started whenever a conditional expectation result
  appears as an argument on the rhs of an operation.
\item a number of local variables was generated for a single kernel that exceed the maximum allowed threshold for local
  variables to be used in a single kernel\footnote{todo, I suspect this is the source of ressource errors that I see for
  exposure calcs sometimes}
\end{itemize}

The following local variables are loaded from the \verb+values+ buffer at the start of a kernel part. This happens
starting with the second kernel part in a multi-part kernel:

\begin{itemize}
  \item every variable that appears on the rhs in the current ssa and
  \item does not appear on the lhs in the current ssa before its usage on the rhs
\end{itemize}

The following local variables are cached to the \verb+values+ buffer at the end of a kernel part $p$. This happens for
all kernels but the last in a multi-part kernel:

\begin{itemize}
  \item Every variable that is loaded in a later part $m > p$, but not cached in a later part $q$, $m > q > p$ (in case
    such a $q$ exists)
  \item Every variable from the sets that are used for conditional expectations in the current part $p$
\end{itemize}
