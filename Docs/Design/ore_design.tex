...
30 April 2026 & LSEG & updates for release 16\\
\hline
\end{supertabular}
\end{center}
\newpage
...
int main(int argc, char** argv) {
	...
	ext::shared_ptr<Parameters> params = ext::make_shared<Parameters>();
	try {
...
const ext::shared_ptr<TradeFactory>& tf
= ext::make_shared<TradeFactory>());
//! Load from XML Node
...
void build(const ext::shared_ptr<EngineFactory>&);
//! Return trade list
const std::vector<ext::shared_ptr<Trade>>& trades() const { return trades_; }
// ...
};
\end{minted}
...
virtual void build(const ext::shared_ptr<EngineFactory>&) = 0;
...
const ext::shared_ptr<InstrumentWrapper>& instrument() { return instrument_; }
...
virtual const ext::shared_ptr<FixingManager>& fixingManager() const = 0;
protected:
Real numeraire_;
};
\end{minted}
...
ext::shared_ptr<ScenarioGenerator>& scenarioGenerator() { return scenarioGenerator_; }
//! Set aggregation data
ext::shared_ptr<AggregationScenarioData>& aggregationScenarioData() { return asd_; }
//! Set scenarioFilter
ext::shared_ptr<ScenarioFilter>& filter() { return filter_; }
//! Update market snapshot and relevant fixing history
void update(const Date& d) override;
//! Reset sim market to initial state
virtual void reset() override;
//! Return the fixing manager
const ext::shared_ptr<FixingManager>& fixingManager() const override { return fixingManager_; }
// ...
};
\end{minted}
...
virtual ext::shared_ptr<Scenario> clone() const = 0;
// ...
};
\end{minted}
...
%%\hrule\medskip
%\begin{minted}[fontsize=\footnotesize]{cpp}
%	ext::shared_ptr<XvaRunner> xva = getXvaRunner();
...
void PricingAnalyticImpl::runAnalytic( 
    const ext::shared_ptr<ore::data::InMemoryLoader>& loader, 
...
\begin{minted}[fontsize=\scriptsize]{c++}
    ...
    auto paylog = ext::make_shared<PayLog>();
...
BondFutureVolatilityCurveSpec & ored\_curvespec.i \\
...
BondFutureOptionQuote & ored\_marketdatum.i \\
...
\bibitem{piterbarg_deltas_callable_libor_exotics} V. Piterbarg, Computing Deltas of Callable Libor Exotics in Forward
  Libor Models, 2003, \url{https://ssrn.com/abstract=396180}
