...
Note: We generally decompose a portfolio into subportfolios and compute the dynamic delta on each subportfolio
separately. See \ref{aad:dynamic_delta_subportfolios}
...
\subsubsection{Computing derivatives of conditional portfolio npvs to model parameters $m$}
...
\subsubsection{Computing derivatives of model parameters $m$ to zero market risk factors $z$}

The relevant model parameters are dsc, lgm\_zeta, fxbs\_sigma and logFxSpot.

\underline{IR zero rate deltas}
...
\subsubsection{Computing derivatives of zero market risk factors $z$ to par market risk factors $p$}

\underline{IR zero to par rate deltas}
...
\underline{IR vega par conversion}
...
\underline{FX vega par conversion}
...
Linear trades like vanilla ir single and cross currency swaps and fx forwards fall into the first category while trades
...
conditional expectations. In general, conditional expectations for both values and derivatives of plain vanilla trades
can be calculated using a simple linear regression with polynomial basis functions, as assposed to complex trades that
might require more advanced models. The complexity of e.g. QR or SVD underlying the linear regression analysis is
$O(mn^2)$ with $m$ the number of observations, $n$ the number of regressor variables, $m > n$, i.e. quadratic in the
number of regressors. Therefore it is reasonable to run the regression analyses on subportfolios with a smallest
possible set of regressors instead of combining trades with disjoint regressors.
...
V_e := E( v_e | \mathcal{F}_t )
\end{equation}

and $f_e$

\begin{equation}\label{aad:calc2}
F_e := E( f_e | \mathcal{F}_t )
\end{equation}
...
t_1$). Not that we floor $F_e$ at $0$ which is not guaranteed due to artifacts of the regression model.
...
calculation of $F_e$ are not detailled here, we refer to the known result that such indicators do not require
differentiation, see e.g. the discussion in \cite{piterbarg_deltas_callable_libor_exotics}, section 10. Finally, we do
not consider derivatives of $1_{F_e > 0}$ because this indicator reflects regression model artifcats only.
...
and then take the conditional expectation, which would be more consistent with the plan vanilla trades and much simpler
in terms of implementation. However, this would not allow to floor $F_e$ at zero as in \ref{aad:calc3} and - much more
importantly - this would also require to include suitable regressors to explain past exercise decisions represented by
$E_i$, such as e.g. suitable model states at all past exercise times $t_1, t_2, \ldots, t_i$.

To keep the implementation of the above decomposition approach generic, we introduce
