...
conditional expectations. In general, conditional expectations can be calculated using a simple linear regression with
polynomial basis functions. The complexity of e.g. QR or SVD underlying the linear regression analysis is $O(mn^2)$ with
$m$ the number of observations, $n$ the number of regressor variables, $m > n$, i.e. quadratic in the number of
regressors. Therefore it is reasonable to run the regression analyses on subportfolios with a smallest possible set of
regressors instead of combining trades with disjoint regressors.
...
differentiation. Finally, we do not consider derivatives of $1_{F_e > 0}$ because this indicator reflects regression
model artifcats only.
...
and then take the conditional expectation, which would be more consistent with the plan vanilla case and much simpler in
terms of implementation. However, this would not allow to floor $F_e$ at zero as in \ref{aad:calc3} and - much more
importantly - this would also require to include regressors to properly determine all past exercise decisions.
