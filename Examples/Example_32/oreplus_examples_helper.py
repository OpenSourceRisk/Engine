    ...
    def _locate_ore_exe(self):
        if os.name == 'nt':
            if platform.machine()[-2:] == "64":
                if os.path.isfile("..\..\\AppPlus\\bin\\x64\\Release\\orePlus.exe"):
                    self.ore_exe = "..\\..\\AppPlus\\bin\\x64\\Release\\orePlus.exe"
                elif os.path.isfile("..\\..\\build\\AppPlus\\ore_plus.exe"):
                    self.ore_exe = "..\\..\\build\\AppPlus\\ore_plus.exe"
                else:
                    print_on_console("ORE+ executable not found.")
                    quit()
            else:
                if os.path.isfile("..\\..\\App\\bin\\Win32\\Release\\orePlus.exe"):
                    self.ore_exe = "..\\..\\App\\bin\\Win32\\Release\\orePlus.exe"
                elif os.path.isfile("..\\..\\build\\App\\ore_plus.exe"):
                    self.ore_exe = "..\\..\\build\\App\\ore_plus.exe"
                else:
                    print_on_console("ORE+ executable not found.")
                    quit()
        else:
            if os.path.isfile("../../AppPlus/build/ore_plus"):
                self.ore_exe = "../../AppPlus/build/ore_plus"
            elif os.path.isfile("../../build/AppPlus/ore_plus"):
                self.ore_exe = "../../build/AppPlus/ore_plus"
            elif os.path.isfile("../../AppPlus/ore_plus"):
                self.ore_exe = "../../AppPlus/ore_plus"
            else:
                print_on_console("ORE+ executable not found.")
                quit()
