...
This example demonstrades a SIMM calculation under AMC scenarios with sensitivities
computed using AAD (Input/Dim2/ore_amccg.xml). See the Jupyter notebook

<code>python -m jupyterlab ore_dynamicsimm.ipynb</code>

for a demonstration of Dynamic SIMM.

This section also contains tools for validating Dynamic SIMM against
(significantly slower) full SIMM calculations along a small number of MC
paths (Input/DimValidation/ore_simm.xml), using an IR/FX CRIF generator added
to ORE for this validation purpose, applied to simulated market data in Input/DimValidation.
As test portfolio we start with the same IR/FX portfolio as above, adding a
Bermudan Swaption to demonstrate the wider product scope of
