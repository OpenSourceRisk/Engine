...
void StaticallyCorrectedYieldTermStructureTest::testCorrectedYts() {
    ...
    Handle<Quote> hq(q);
    Handle<YieldTermStructure> floating(boost::make_shared<FlatForward>(
        0, NullCalendar(), hq, Actual365Fixed()));

    Handle<YieldTermStructure> source(
        boost::make_shared<FlatForward>(refDate, 0.02, Actual365Fixed()));
    Handle<YieldTermStructure> target(
    ...
    Handle<YieldTermStructure> corrected1(
        boost::make_shared<StaticallyCorrectedYieldTermStructure>(
            floating, source, target, ConstantDiscounts));
    Handle<YieldTermStructure> corrected2(
        boost::make_shared<StaticallyCorrectedYieldTermStructure>(
            floating, source, target, ForwardForward));

    Real tol = 1.0E-12;

    BOOST_CHECK_MESSAGE(
        std::fabs(target->discount(1.0) - corrected1->discount(1.0)) < tol,
        "can not verify corrected1 df ("
            << corrected1->discount(1.0) << ") against target df ("
            << target->discount(1.0) << ") on eval date "
            << Settings::instance().evaluationDate() << ", difference is "
            << (corrected1->discount(1.0) - target->discount(1.0))
            << ", tolerance is " << tol);
    BOOST_CHECK_MESSAGE(
        std::fabs(target->discount(1.0) - corrected2->discount(1.0)) < tol,
        "can not verify corrected2 df ("
            << corrected2->discount(1.0) << ") against target df ("
            << target->discount(1.0) << ") on eval date "
            << Settings::instance().evaluationDate() << ", difference is "
            << (corrected2->discount(1.0) - target->discount(1.0))
            << ", tolerance is " << tol);
    ...
    BOOST_CHECK_MESSAGE(
        std::fabs(floating->discount(1.0) * target->discount(1.0) /
                      source->discount(1.0) -
                  corrected1->discount(1.0)) < tol,
        "can not verify corrected1 df ("
            << corrected1->discount(1.0) << ") against expected df ("
            << floating->discount(1.0) * target->discount(1.0) /
                   source->discount(1.0)
    ...
    Real t = Actual365Fixed().yearFraction(
    ...
    BOOST_CHECK_MESSAGE(
        ...
        "can not verify corrected1 df ("
            ...
            << ", tolerance is " << tol);
    BOOST_CHECK_MESSAGE(
        std::fabs(floating->discount(1.0) * target->discount(t + 1.0) *
                      source->discount(t) /
                      (target->discount(t) * source->discount(t + 1.0)) -
                  corrected2->discount(1.0)) < tol,
        "can not verify corrected2 df ("
            << corrected2->discount(1.0) << ") against expected df ("
            << floating->discount(1.0) * target->discount(t + 1.0) *
                   source->discount(t) /
                   (target->discount(t) * source->discount(t + 1.0))
            << ") on eval date " << Settings::instance().evaluationDate()
            << ", difference is "
...
test_suite *StaticallyCorrectedYieldTermStructureTest::suite() {
    test_suite *suite =
        BOOST_TEST_SUITE("StaticallyCorrectedYieldTermStructure tests");
