...
void run_case(std::vector<Real> l, std::string fileName, double detachmentRatio = 0.2) {
    ...
    BOOST_CHECK_CLOSE(trancheLossMC, calculatedLossTrancheHWFullBucketing, 1.0);
    BOOST_CHECK_CLOSE(trancheLossMC, calculatedLossTrancheHWTrancheBucketing, 1.0);
...
BOOST_AUTO_TEST_CASE(testHullWhiteBucketingSingleStateExpectedLossNonHomogenous) {
    ...
    for (auto detachmentPoint : {0.03, 0.05, 0.07, 0.15, 0.25, 0.35}) {
