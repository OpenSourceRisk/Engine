...
BOOST_AUTO_TEST_CASE(testSwaptionPricing) {
  ...
  /*
    QuantLib::ext::shared_ptr<LgmConvolutionSolver2> solver(lgm, 0.1, 100, 0.1, 200);
    Size americanExerciseTimeStepsPerYear = 24;
    NumericLgmMultiLegOptionEngineBase engine(solver, d.discountCurve, americanExerciseTimeStepsPerYear);

    
    Volatility volat(0.0075);
    QuantLib::ext::shared_ptr<BlackVolTermStructure> fxv(
            new BlackConstantVol(Settings::instance().evaluationDate(), NullCalendar(), volat, ActualActual(ActualActual::ISDA)));

    QuantLib::Date today(15, QuantLib::May, 2022);
    QuantLib::Settings::instance().evaluationDate() = today;
    QuantLib::Calendar calendar = QuantLib::NullCalendar();
    QuantLib::DayCounter dayCount = QuantLib::Actual365Fixed();   
   
    QuantLib::ext::shared_ptr<PricingEngine> engineNew =
        QuantLib::ext::make_shared<BlackMultiLegOptionEngineBase>(d.discountCurve, fxv);
...
BOOST_AUTO_TEST_CASE(testMultiLegEngine) {
    ...
    for (double strike = -0.01; strike < 0.05; strike += 0.01)
    {
        ...
        // Get Bachelier Price
        Volatility vola(vol);
        QuantLib::ext::shared_ptr<PricingEngine> bachEngine = QuantLib::ext::make_shared<BachelierSwaptionEngine>(eurYtsHandle, vola);
        swaption->setPricingEngine(bachEngine);
        Real bachPrice = swaption->NPV();
        ...
        QuantLib::IborLeg floatLeg= QuantLib::IborLeg(schedule, liborIndex).withNotionals(1.0)
            .withPaymentAdjustment(ModifiedFollowing).withPaymentLag(2).withPaymentCalendar(TARGET());
        std::vector<Leg> legs={fixedLeg, floatLeg};
        ...
        swapMulti -> setPricingEngine(engineMulti);
        Real multiPrice = swapMulti->NPV();
        
        BOOST_TEST_MESSAGE("Receiver Swaption (Strike = " << strike * 100.0 << "%): ");
        BOOST_TEST_MESSAGE("    LGM Model: " << lgmPrice * 10000.0 << " bp.");
        BOOST_TEST_MESSAGE("    Bachelier Model: " << bachPrice * 10000.0 << " bp.");
        BOOST_TEST_MESSAGE("    Berm Model: " << bermPrice * 10000.0 << " bp.");
        BOOST_TEST_MESSAGE("    Multileg Model: " << multiPrice * 10000.0 << " bp.");
        BOOST_TEST_MESSAGE("------------");
        
        if (std::fabs(bachPrice - bermPrice) < 10e-4)
            BOOST_CHECK(true);
        else
            BOOST_CHECK_CLOSE(bachPrice, bermPrice, 1.0); // Tolerance of 1%
    }
}

/*BOOST_AUTO_TEST_CASE(testBermSwaption) {
    Calendar calendar = TARGET();
    Date settlementDate(15, July, 2015);
    Date expiryDate(10, July, 2017);
    boost::shared_ptr<Exercise> exercise = boost::make_shared<EuropeanExercise>(expiryDate); // T=2
