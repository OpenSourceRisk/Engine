...
struct SwaptionVolatilityEUR {
    ...
    SwaptionVolatilityEUR()
        : optionTenors(4), swapTenors(4), strikeSpreads(5), nVols(optionTenors.size(), swapTenors.size(), 0.0), lnVols(nVols),
          slnVols_1(nVols), slnVols_2(nVols), shifts_1(nVols), shifts_2(nVols), nVolSpreads(16, vector<Handle<Quote> >(5, Handle<Quote>())),
	  lnVolSpreads(16, vector<Handle<Quote> >(5, Handle<Quote>())), slnVolSpreads(16, vector<Handle<Quote> >(5, Handle<Quote>())) {
	...
	for (Size i = 0; i < optionTenors.size(); ++i) {
	  for (Size j = 0; j < swapTenors.size(); ++j) {
	    for (Size k = 0; k < strikeSpreads.size(); ++k) {
	      ...
	      Real nVolSpread = k == atmStrikeIndex ? 0.0 : nVolSpreadMin + rng.nextReal() * (nVolSpreadMax - nVolSpreadMin);	      
	      nVolSpreads[index][k] = Handle<Quote>(boost::shared_ptr<SimpleQuote>(new SimpleQuote(nVolSpread)));
	      Real lnVolSpread = k == atmStrikeIndex ? 0.0 : lnVolSpreadMin + rng.nextReal() * (lnVolSpreadMax - lnVolSpreadMin);	      
	      lnVolSpreads[index][k] = Handle<Quote>(boost::shared_ptr<SimpleQuote>(new SimpleQuote(lnVolSpread)));
	      Real slnVolSpread = k == atmStrikeIndex ? 0.0 : slnVolSpreadMin + rng.nextReal() * (slnVolSpreadMax - slnVolSpreadMin);	      
	      slnVolSpreads[index][k] = Handle<Quote>(boost::shared_ptr<SimpleQuote>(new SimpleQuote(slnVolSpread)));
	    }
	  }
	}
...
struct SwaptionConventionsEUR {
    ...
    SwaptionConventionsEUR()
        : settlementDays(2), fixedTenor(Period(1, Years)), fixedCalendar(TARGET()), fixedConvention(ModifiedFollowing),
          fixedDayCounter(Thirty360(Thirty360::BondBasis)), floatIndex(boost::make_shared<Euribor6M>()), swapIndex(boost::make_shared<EuriborSwapIsdaFixA>(10*Years)), shortSwapIndex(boost::make_shared<EuriborSwapIsdaFixA>(2*Years)) {}
