...
struct TestData {
    ...
    TestData() {
        ...
        yts2 = Handle<YieldTermStructure>(boost::make_shared<FlatForward>(refDate, 0.02, Actual365Fixed()));

        ovtLn = Handle<OptionletVolatilityStructure>(boost::make_shared<ConstantOptionletVolatility>(
            refDate, TARGET(), Following, 0.20, Actual365Fixed(), ShiftedLognormal, 0.0));
        ovtSln = Handle<OptionletVolatilityStructure>(boost::make_shared<ConstantOptionletVolatility>(
            refDate, TARGET(), Following, 0.10, Actual365Fixed(), ShiftedLognormal, 0.01));
        ovtN = Handle<OptionletVolatilityStructure>(boost::make_shared<ConstantOptionletVolatility>(
            refDate, TARGET(), Following, 0.0075, Actual365Fixed(), Normal, 0.0));

        swLn = Handle<SwaptionVolatilityStructure>(boost::make_shared<ConstantSwaptionVolatility>(
            refDate, TARGET(), Following, 0.20, Actual365Fixed(), ShiftedLognormal, 0.0));
        swSln = Handle<SwaptionVolatilityStructure>(boost::make_shared<ConstantSwaptionVolatility>(
            refDate, TARGET(), Following, 0.10, Actual365Fixed(), ShiftedLognormal, 0.01));
        swN = Handle<SwaptionVolatilityStructure>(boost::make_shared<ConstantSwaptionVolatility>(
            refDate, TARGET(), Following, 0.0075, Actual365Fixed(), Normal, 0.01));

        fxVol = Handle<BlackVolTermStructure>(
            boost::make_shared<BlackConstantVol>(refDate, TARGET(), 0.20, Actual365Fixed()));

        blackPricerLn = boost::make_shared<BlackIborCouponPricer>(ovtLn);
        blackPricerSln = boost::make_shared<BlackIborCouponPricer>(ovtSln);
        blackPricerN = boost::make_shared<BlackIborCouponPricer>(ovtN);

        reversion = Handle<Quote>(boost::make_shared<SimpleQuote>(0.01));
        cmsPricerLn = boost::make_shared<LinearTsrPricer>(swLn, reversion, yts2);
        cmsPricerSln = boost::make_shared<LinearTsrPricer>(swSln, reversion, yts2);
        ...
        correlationTS = Handle<QuantExt::CorrelationTermStructure>(boost::make_shared<FlatCorrelation>(
            refDate, Handle<Quote>(boost::make_shared<SimpleQuote>(0.6)), Actual365Fixed()));
        cmsspPricerLn = boost::make_shared<LognormalCmsSpreadPricer>(cmsPricerLn, correlation, yts2, 32);
        cmsspPricerSln = boost::make_shared<LognormalCmsSpreadPricer>(cmsPricerSln, correlation, yts2, 32);
        cmsspPricerN = boost::make_shared<LognormalCmsSpreadPricer>(cmsPricerN, correlation, yts2, 32);

        quantoCorrelation = Handle<Quote>(boost::make_shared<SimpleQuote>(0.8));
        quantoCorrelationTS = Handle<QuantExt::CorrelationTermStructure>(boost::make_shared<FlatCorrelation>(
            refDate, Handle<Quote>(boost::make_shared<SimpleQuote>(0.8)), Actual365Fixed()));
        blackQuantoPricerLn =
            boost::make_shared<QuantExt::BlackIborQuantoCouponPricer>(fxVol, quantoCorrelation, ovtLn);
        blackQuantoPricerSln =
            boost::make_shared<QuantExt::BlackIborQuantoCouponPricer>(fxVol, quantoCorrelation, ovtSln);
        blackQuantoPricerN = boost::make_shared<QuantExt::BlackIborQuantoCouponPricer>(fxVol, quantoCorrelation, ovtN);
        ...
        std::map<std::string, boost::shared_ptr<IborCouponPricer>> iborPricers;
        std::map<std::string, boost::shared_ptr<CmsCouponPricer>> cmsPricers;
        std::map<std::string, Handle<BlackVolTermStructure>> fxVols, emptyFxVols;
        fxVols["EUR"] = fxVol; // vs USD
	std::string key = boost::make_shared<Euribor>(6 * Months)->name();
        iborPricers[key] = blackPricerLn;
        cmsPricers[key] = cmsPricerLn;
        formulaPricerLn = boost::make_shared<MCGaussianFormulaBasedCouponPricer>(
            "EUR", iborPricers, cmsPricers, emptyFxVols, indCorrelationTS, yts2, samples);
        formulaPricerUSDLn = boost::make_shared<MCGaussianFormulaBasedCouponPricer>(
            "USD", iborPricers, cmsPricers, fxVols, indCorrelationTS, yts2, samples);
        iborPricers[key] = blackPricerSln;
        cmsPricers[key] = cmsPricerSln;
        formulaPricerSln = boost::make_shared<MCGaussianFormulaBasedCouponPricer>(
            "EUR", iborPricers, cmsPricers, emptyFxVols, indCorrelationTS, yts2, samples);
        formulaPricerUSDSln = boost::make_shared<MCGaussianFormulaBasedCouponPricer>(
            "USD", iborPricers, cmsPricers, fxVols, indCorrelationTS, yts2, samples);
        iborPricers[key] = blackPricerN;
        cmsPricers[key] = cmsPricerN;
        formulaPricerN = boost::make_shared<MCGaussianFormulaBasedCouponPricer>(
            "EUR", iborPricers, cmsPricers, emptyFxVols, indCorrelationTS, yts2, samples);
        formulaPricerUSDN = boost::make_shared<MCGaussianFormulaBasedCouponPricer>(
            "USD", iborPricers, cmsPricers, fxVols, indCorrelationTS, yts2, samples);
    }
    ...
    boost::shared_ptr<IborCouponPricer> blackPricerLn, blackPricerSln, blackPricerN;
    boost::shared_ptr<IborCouponPricer> blackQuantoPricerLn, blackQuantoPricerSln, blackQuantoPricerN;
    boost::shared_ptr<CmsCouponPricer> cmsPricerLn, cmsPricerSln, cmsPricerN;
    boost::shared_ptr<CmsSpreadCouponPricer> cmsspPricerLn, cmsspPricerSln, cmsspPricerN;
    boost::shared_ptr<FormulaBasedCouponPricer> formulaPricerLn, formulaPricerSln, formulaPricerN;
    boost::shared_ptr<FormulaBasedCouponPricer> formulaPricerUSDLn, formulaPricerUSDSln, formulaPricerUSDN;
};

void runTest(boost::shared_ptr<FloatingRateCoupon> cpn, boost::shared_ptr<FloatingRateCoupon> cpnRef,
             const boost::shared_ptr<FloatingRateCouponPricer>& pricer,
             const boost::shared_ptr<FloatingRateCouponPricer>& pricerRef, const std::string& testLabel,
             const Real tol) {
    ...
    auto cfRef = boost::dynamic_pointer_cast<CappedFlooredCoupon>(cpnRef);
    if (cfRef)
        cfRef->underlying()->setPricer(pricerRef);
    auto cf = boost::dynamic_pointer_cast<CappedFlooredCoupon>(cpn);
    if (cf)
...
BOOST_AUTO_TEST_CASE(testCappedLiborCoupon) {
    ...
    auto euribor6m = boost::make_shared<Euribor>(6 * Months, d.yts2);
    ...
    auto indexPlain = boost::make_shared<FormulaBasedIndex>(
        "libor-family", std::vector<boost::shared_ptr<InterestRateIndex>>{euribor6m}, formulaPlain,
        euribor6m->fixingCalendar());
    auto indexCapped = boost::make_shared<FormulaBasedIndex>(
        "libor-family", std::vector<boost::shared_ptr<InterestRateIndex>>{euribor6m}, formulaCapped,
        euribor6m->fixingCalendar());

    auto undRef = boost::make_shared<IborCoupon>(Date(23, February, 2029), 10000.0, Date(23, February, 2028),
                                                 Date(23, February, 2029), 2, euribor6m, 1.0, 0.0, Date(), Date(),
                                                 Actual360(), false);
    auto cappedRef = boost::make_shared<CappedFlooredCoupon>(undRef, 0.03);

    boost::shared_ptr<QuantExt::FormulaBasedCoupon> und(
    ...
    boost::shared_ptr<QuantExt::FormulaBasedCoupon> capped(
        new QuantExt::FormulaBasedCoupon(EURCurrency(), Date(23, February, 2029), 10000.0, Date(23, February, 2028),
...
BOOST_AUTO_TEST_CASE(testCappedCmsCoupon) {
    ...
    auto cms10y = boost::make_shared<EuriborSwapIsdaFixA>(10 * Years, d.yts2, d.yts2);

    CompiledFormula formulaPlain = CompiledFormula(Size(0));                              // plain payoff
    CompiledFormula formulaCapped = min(CompiledFormula(Size(0)), CompiledFormula(0.03)); // capped cms coupon, at 0.03

    auto indexPlain =
        boost::make_shared<FormulaBasedIndex>("cms-family", std::vector<boost::shared_ptr<InterestRateIndex>>{cms10y},
                                              formulaPlain, cms10y->fixingCalendar());
    auto indexCapped =
        boost::make_shared<FormulaBasedIndex>("cms-family", std::vector<boost::shared_ptr<InterestRateIndex>>{cms10y},
                                              formulaCapped, cms10y->fixingCalendar());

    auto undRef = boost::make_shared<CmsCoupon>(Date(23, February, 2029), 10000.0, Date(23, February, 2028),
...
BOOST_AUTO_TEST_CASE(testCappedCmsSpreadCoupon) {
    ...
    auto cms2y = boost::make_shared<EuriborSwapIsdaFixA>(2 * Years, d.yts2, d.yts2);
    auto cms10y = boost::make_shared<EuriborSwapIsdaFixA>(10 * Years, d.yts2, d.yts2);
    auto cms10y2y = boost::make_shared<SwapSpreadIndex>("cms10y2y", cms10y, cms2y);
    ...
    auto indexPlain = boost::make_shared<FormulaBasedIndex>(
        "cmssp-family", std::vector<boost::shared_ptr<InterestRateIndex>>{cms2y, cms10y}, formulaPlain,
        cms10y2y->fixingCalendar());
    auto indexCapped = boost::make_shared<FormulaBasedIndex>(
        "cmssp-family", std::vector<boost::shared_ptr<InterestRateIndex>>{cms2y, cms10y}, formulaCapped,
        cms10y2y->fixingCalendar());

    auto undRef = boost::make_shared<CmsSpreadCoupon>(Date(23, February, 2029), 10000.0, Date(23, February, 2028),
