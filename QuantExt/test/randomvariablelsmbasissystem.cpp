...
BOOST_AUTO_TEST_CASE(testVarGroups) {
    ...
    auto bs = RandomVariableLsmBasisSystem::multiPathBasisSystem(
        5, 5, QuantLib::LsmBasisSystem::PolynomialType::Monomial, {{0, 1}, {2, 3, 4}});

    auto bs1 =
        RandomVariableLsmBasisSystem::multiPathBasisSystem(2, 5, QuantLib::LsmBasisSystem::PolynomialType::Monomial);
    auto bs2 =
        RandomVariableLsmBasisSystem::multiPathBasisSystem(3, 5, QuantLib::LsmBasisSystem::PolynomialType::Monomial);
    ...
    for (Size i = 0; i < nSamplePoints; ++i) {
        ...
        for (Size d = 0; d < 2; ++d) {
            ...
            samplePointRvPtr1[d] = &samplePointRv1[d];
        }
...
BOOST_AUTO_TEST_CASE(testVarGroupsOverlappingIncomplete) {

    BOOST_TEST_MESSAGE(
        "Testing lsm basis system for random variables with var groups (overlapping and incomplete) ...");

    auto bs = RandomVariableLsmBasisSystem::multiPathBasisSystem(
        5, 5, QuantLib::LsmBasisSystem::PolynomialType::Monomial, {{0, 1}, {1, 3}});
    ...
    std::vector<RandomVariable> samplePointRv(5);
    std::vector<RandomVariable> samplePointRv1(2);
    std::vector<RandomVariable> samplePointRv2(2);
    std::vector<const RandomVariable*> samplePointRvPtr(5);
    std::vector<const RandomVariable*> samplePointRvPtr1(2);
    std::vector<const RandomVariable*> samplePointRvPtr2(2);
    ...
    for (Size i = 0; i < nSamplePoints; ++i) {
        ...
        samplePointRv1[0] = RandomVariable(1, samplePoint[0]);
        samplePointRv1[1] = RandomVariable(1, samplePoint[1]);
        samplePointRvPtr1[0] = &samplePointRv1[0];
        samplePointRvPtr1[1] = &samplePointRv1[1];

        samplePointRv2[0] = RandomVariable(1, samplePoint[1]);
        samplePointRv2[1] = RandomVariable(1, samplePoint[3]);
        samplePointRvPtr2[0] = &samplePointRv2[0];
        samplePointRvPtr2[1] = &samplePointRv2[1];
