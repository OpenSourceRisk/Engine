...
struct CommoditySchwartzSeasonalityModelTestData : public qle::test::TopLevelFixture {
    CommoditySchwartzSeasonalityModelTestData(bool driftFreeState) :
        ts(QuantLib::ext::make_shared<InterpolatedPriceCurve<Linear>>(periods, prices, ActualActual(ActualActual::ISDA), USDCurrency())) {
        ...
        parametrization = QuantLib::ext::make_shared<CommoditySchwartzSeasonalityParametrization>(USDCurrency(), "WTI", ts, fx, sigma, kappa, times_a_a, a_a, QuantLib::ext::make_shared<QuantLib::NoConstraint>(), driftFreeState);
        QL_REQUIRE(parametrization != NULL, "CommoditySchwartzSeasonalityParametrization has null pointer");

        // TODO test Euler discretization
        model =
            QuantLib::ext::make_shared<CommoditySchwartzSeasonalityModel>(parametrization, CommoditySchwartzModel::Discretization::Exact);
        BOOST_TEST_MESSAGE("after model ");
        QL_REQUIRE(model != NULL, "CommoditySchwartzSeasonalityModel has null pointer");
    }
    ...
    Handle<QuantExt::PriceTermStructure> ts;
    Real kappa, sigma;
    QuantLib::ext::shared_ptr<CommoditySchwartzSeasonalityParametrization> parametrization;
    QuantLib::ext::shared_ptr<CommoditySchwartzSeasonalityModel> model;
}; // CommoditySchwarzModelTestData

BOOST_DATA_TEST_CASE(testMartingaleProperty2,
    bdata::make(driftFreeState) * bdata::make(steps),
    driftFreeState, steps) {

//BOOST_AUTO_TEST_CASE(testMartingaleProperty) {

BOOST_TEST_MESSAGE("Testing martingale property in the COM Schwartz model with Seasonality ...");

CommoditySchwartzSeasonalityModelTestData data(driftFreeState);
QuantLib::ext::shared_ptr<StochasticProcess> process = data.model->stateProcess();
QL_REQUIRE(process != NULL, "process has null pointer!");
