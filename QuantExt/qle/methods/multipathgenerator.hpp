...
public:
    ...
    MultiPathGenerator(const boost::shared_ptr<StochasticProcess>&, const TimeGrid&, GSG generator,
...
template <class GSG>
MultiPathGenerator<GSG>::MultiPathGenerator(const boost::shared_ptr<StochasticProcess>& process, const TimeGrid& times,
                                            GSG generator, bool brownianBridge)
    : brownianBridge_(brownianBridge), process_(process), generator_(generator),
      next_(MultiPath(process->size(), times), 1.0), bb_(times) {
...
template <class GSG> inline const typename MultiPathGenerator<GSG>::sample_type& MultiPathGenerator<GSG>::next() const {
    return next(false);
}
...
const typename MultiPathGenerator<GSG>::sample_type& MultiPathGenerator<GSG>::next(bool antithetic) const {
    ...
    if (brownianBridge_) {
        ...
        for (Size j = 0; j < n; ++j) {
            bb_.transform(reorderedSeq_.begin() + j * l, reorderedSeq_.begin() + (j + 1) * l,
    ...
    for (Size i = 1; i <= l; i++) {
        ...
        if (brownianBridge_) {
            if (antithetic)
                std::transform(reorderedSeq_.begin() + offset, reorderedSeq_.begin() + offset + n, temp.begin(),
                               std::negate<Real>());
            else
                std::copy(reorderedSeq_.begin() + offset, reorderedSeq_.begin() + offset + n, temp.begin());
        } else {
            if (antithetic)
