...
SubPeriodsSwap::SubPeriodsSwap(const Date& effectiveDate, Real nominal, const Period& swapTenor, bool isPayer,
    ...
    legs_[1] = QuantExt::SubPeriodsLeg(floatSchedule_, floatIndex_)
...
MakeSubPeriodsSwap::MakeSubPeriodsSwap(const Period& swapTenor, const boost::shared_ptr<IborIndex>& index, Rate fixedRate, 
    ...
    fixedConvention_(ModifiedFollowing), fixedRule_(DateGeneration::Backward), floatDayCounter_(index->dayCounter()),
      subCouponsType_(QuantExt::SubPeriodsCoupon::Compounding)  {}
...
MakeSubPeriodsSwap& MakeSubPeriodsSwap::withFixedLegDayCount(const DayCounter& dc) {
    ...
    MakeSubPeriodsSwap& MakeSubPeriodsSwap::withSubCouponsType(const QuantExt::SubPeriodsCoupon::Type& st) {
    subCouponsType_ = st;
    return *this;
}
