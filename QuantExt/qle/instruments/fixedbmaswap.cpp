...
namespace QuantExt {
    ...
    FixedBMASwap::FixedBMASwap(Type type,
        Real nominal,
        // Fixed leg
    ...
    Real FixedBMASwap::fixedRate() const {
        return fixedRate_;
    }

    Real FixedBMASwap::nominal() const {
        return nominal_;
    }

    FixedBMASwap::Type FixedBMASwap::type() const {
        return type_;
    }

    const Leg& FixedBMASwap::fixedLeg() const {
        return legs_[0];
    }

    const Leg& FixedBMASwap::bmaLeg() const {
        return legs_[1];
    }
    ...
    MakeFixedBMASwap::MakeFixedBMASwap(const Period& swapTenor,
        ...
        : swapTenor_(swapTenor), bmaIndex_(index),
        fixedRate_(fixedRate), forwardStart_(forwardStart),
        settlementDays_(bmaIndex_->fixingDays()),
        fixedCalendar_(index->fixingCalendar()),
        bmaCalendar_(index->fixingCalendar()),
        type_(FixedBMASwap::Payer), nominal_(1.0),
        bmaLegTenor_(3*Months),
        fixedConvention_(ModifiedFollowing),
        fixedTerminationDateConvention_(ModifiedFollowing),
        bmaConvention_(ModifiedFollowing),
        bmaTerminationDateConvention_(ModifiedFollowing),
        fixedRule_(DateGeneration::Backward), bmaRule_(DateGeneration::Backward),
        fixedEndOfMonth_(false), bmaEndOfMonth_(false),
        fixedFirstDate_(Date()), fixedNextToLastDate_(Date()),
        bmaFirstDate_(Date()), bmaNextToLastDate_(Date()),
        bmaSpread_(0.0),
        bmaDayCount_(index->dayCounter()) {}
    ...
    MakeFixedBMASwap::operator boost::shared_ptr<FixedBMASwap>() const {
        ...
        if (effectiveDate_ != Date())
            ...
            Date spotDate = bmaCalendar_.advance(refDate,
                settlementDays_*Days);
            startDate = spotDate + forwardStart_;
            if (forwardStart_.length()<0)
                startDate = bmaCalendar_.adjust(startDate,
                    Preceding);
            else
                startDate = bmaCalendar_.adjust(startDate,
                    Following);
        ...
        if (endDate == Date()) {
            if (bmaEndOfMonth_)
                endDate = bmaCalendar_.advance(startDate,
                    swapTenor_,
                    ModifiedFollowing,
                    bmaEndOfMonth_);
            else
        ...
        Schedule bmaSchedule(startDate, endDate,
            bmaLegTenor_, bmaCalendar_,
            bmaConvention_,
            bmaTerminationDateConvention_,
            bmaRule_, bmaEndOfMonth_,
            bmaFirstDate_, bmaNextToLastDate_);
        ...
        if (fixedRate_ == Null<Rate>()) {
            FixedBMASwap temp(type_, nominal_,
                fixedSchedule,
                0.0, // fixed rate
                fixedDayCount,
        ...
        boost::shared_ptr<FixedBMASwap> swap(new
            FixedBMASwap(type_, nominal_,
                fixedSchedule,
                usedFixedRate, fixedDayCount,
                bmaSchedule,
                bmaIndex_, bmaDayCount_));
    ...
    MakeFixedBMASwap& MakeFixedBMASwap::withDiscountingTermStructure(
    ...
    MakeFixedBMASwap& MakeFixedBMASwap::withPricingEngine(
