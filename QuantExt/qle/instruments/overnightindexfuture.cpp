...
OvernightIndexFuture::OvernightIndexFuture(const ext::shared_ptr<OvernightIndex>& overnightIndex,
                                           const ext::shared_ptr<Payoff>& payoff, const Date& valueDate,
                                           const Date& maturityDate, const Handle<YieldTermStructure>& discountCurve,
                                           const Handle<Quote>& convexityAdjustment)
    : Forward(overnightIndex->dayCounter(), overnightIndex->fixingCalendar(), overnightIndex->businessDayConvention(),
              0, payoff, valueDate, maturityDate, discountCurve),
...
Real OvernightIndexFuture::spotValue() const {
    ...
    if (today > valueDate_) {
        ...
        const TimeSeries<Real>& history = IndexManager::instance().getHistory(overnightIndex_->name());
        Date d1 = valueDate_;
        while (d1 < today) {
            Real r = history[d1];
            QL_REQUIRE(r != Null<Real>(), "missing rate on " << d1 << " for index " << overnightIndex_->name());
    ...
    Real convAdj = convexityAdjustment_.empty() ? 0.0 : convexityAdjustment_->value();
    Real R = convAdj + (prod - 1) / dayCounter_.yearFraction(valueDate_, maturityDate_);
