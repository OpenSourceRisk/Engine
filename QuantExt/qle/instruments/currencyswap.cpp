...
namespace QuantExt {
    ...
    CurrencySwap::CurrencySwap(const std::vector<Leg>& legs,
        ...
        QL_REQUIRE(currency.size()==legs_.size(),
                   "size mismatch between currency (" << currency.size() <<
                   ") and legs (" << legs_.size() << ")");
    ...
    void CurrencySwap::arguments::validate() const {
        ...
        QL_REQUIRE(currency.size() == legs.size(),
    ...
    VanillaCrossCurrencySwap::VanillaCrossCurrencySwap (bool payFixed,
                           Currency fixedCcy,
                           Real fixedNominal,
    ...
    //-------------------------------------------------------------------------
    CrossCurrencySwap::CrossCurrencySwap (bool payFixed,
                           Currency fixedCcy,
        ...
        legs_[3].push_back(boost::shared_ptr<CashFlow>(
                 new SimpleCashFlow(-floatNominals[0],
                                    floatSchedule.dates().front())));
        QL_REQUIRE(floatNominals.size() < floatSchedule.size(),
    ...
    //-------------------------------------------------------------------------
    CrossCurrencySwap::CrossCurrencySwap (bool pay1,
                           Currency ccy1,
                           std::vector<Real> nominals1,
                           const Schedule& schedule1,
                           std::vector<Rate> rates1,
    ...
    CrossCurrencySwap::CrossCurrencySwap (bool pay1,
                           ...
                           std::vector<Real> nominals1,
                           const Schedule& schedule1,
                           const boost::shared_ptr<IborIndex>& iborIndex1,
