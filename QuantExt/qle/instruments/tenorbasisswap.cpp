...
namespace QuantExt {
          ...
          public:
            ...
            Real operator()(Spread shortSpread) const {
                ...
                for (it = shortLeg_.begin(); it != shortLeg_.end(); ++it) {
                    boost::shared_ptr<SubPeriodsCoupon> c =
                        boost::dynamic_pointer_cast<SubPeriodsCoupon>(*it);
    ...
    TenorBasisSwap::TenorBasisSwap(const Date& effectiveDate,
        Real nominal,
    ...
    : Swap(2),
      ...
      longSpread_(longSpread),
      shortIndex_(shortIndex),
      shortSpread_(shortSpread),
      shortPayTenor_(shortPayTenor),
      includeSpread_(includeSpread),
      type_(type) {

        // Checks
        Period longTenor = longIndex_->tenor();
        QL_REQUIRE(shortPayTenor_ >= shortIndex_->tenor(),
            "Expected short payment tenor to exceed/equal shortIndex tenor");
        QL_REQUIRE(shortPayTenor_ <= longTenor,
            "Expected short payment tenor to be at most longSchedule tenor");
    ...
    TenorBasisSwap::TenorBasisSwap(Real nominal,
        bool payLongIndex,
    ...
    : Swap(2),
      nominal_(nominal),
      payLongIndex_(payLongIndex),
      longSchedule_(longSchedule),
      longIndex_(longIndex),
      longSpread_(longSpread),
      shortSchedule_(shortSchedule),
      shortIndex_(shortIndex),
      shortSpread_(shortSpread),
      includeSpread_(includeSpread),
      type_(type) {
        ...
        QL_REQUIRE(longTenor == longIndex_->tenor(),
            "Expected longSchedule tenor to equal longIndex tenor");
        shortPayTenor_ = shortSchedule_.tenor();
        QL_REQUIRE(shortPayTenor_ >= shortIndex_->tenor(),
            "Expected shortSchedule tenor to exceed/equal shortIndex tenor");
        QL_REQUIRE(shortPayTenor_ <= longTenor,
            "Expected shortSchedule tenor to be at most longSchedule tenor");
    ...
    Real TenorBasisSwap::longLegBPS() const {
        calculate();
        QL_REQUIRE(legBPS_[longNo_] != Null<Real>(),
            "Long leg BPS not available");
        return legBPS_[longNo_];
    }

    Real TenorBasisSwap::longLegNPV() const {
        calculate();
        QL_REQUIRE(legNPV_[longNo_] != Null<Real>(),
            "Long leg NPV not available");
        return legNPV_[longNo_];
    }

    Rate TenorBasisSwap::fairLongLegSpread() const {
        calculate();
        QL_REQUIRE(fairLongSpread_ != Null<Spread>(),
            "Long leg fair spread not available");
        return fairLongSpread_;
    }

    Real TenorBasisSwap::shortLegBPS() const {
        calculate();
        QL_REQUIRE(legBPS_[shortNo_] != Null<Real>(),
            "Short leg BPS not available");
        return legBPS_[shortNo_];
    }

    Real TenorBasisSwap::shortLegNPV() const {
        calculate();
        QL_REQUIRE(legNPV_[shortNo_] != Null<Real>(),
            "Short leg NPV not available");
        return legNPV_[shortNo_];
    }

    Spread TenorBasisSwap::fairShortLegSpread() const {
        calculate();
        QL_REQUIRE(fairShortSpread_ != Null<Spread>(),
            "Short leg fair spread not available");
    ...
    void TenorBasisSwap::fetchResults(const PricingEngine::results* r) const {
        ...
        if (fairShortSpread_ == Null<Spread>()) {
            if (shortPayTenor_ == shortIndex_->tenor() || !includeSpread_) {
                ...
                fairShortSpread_ =
                    shortSpread_ - NPV_ / (legBPS_[shortNo_] / basisPoint);
                }
            } else {
                ...
                if (engine) {
                    ...
                    Spread step = 1e-4;
                    Real accuracy = 1e-8;
                    FairShortSpreadHelper f(*this,
                        discountCurve, legNPV_[longNo_]);
                    Brent solver;
