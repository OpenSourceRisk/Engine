...
namespace QuantExt {
    ...
    AverageOISRateHelper::AverageOISRateHelper(const Handle<Quote>& fixedRate,
    ...
    Spread AverageOISRateHelper::onSpread() const {
        return onSpread_.empty() ? 0.0 : onSpread_->value();
    }

    boost::shared_ptr<AverageOIS> AverageOISRateHelper::averageOIS() const {
        return averageOIS_;
    }
