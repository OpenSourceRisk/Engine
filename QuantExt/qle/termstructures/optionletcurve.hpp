...
template <class T>
inline QuantLib::Rate InterpolatedOptionletCurve<T>::maxStrike() const {
    return QL_MAX_REAL;
}

template <class T>
inline QuantLib::VolatilityType InterpolatedOptionletCurve<T>::volatilityType() const {
    return volatilityType_;
}

template <class T>
inline QuantLib::Real InterpolatedOptionletCurve<T>::displacement() const {
    return displacement_;
}

template <class T>
inline const std::vector<QuantLib::Time>&
InterpolatedOptionletCurve<T>::times() const {
    return this->times_;
}

template <class T>
inline const std::vector<QuantLib::Date>&
InterpolatedOptionletCurve<T>::dates() const {
    return dates_;
}
...
InterpolatedOptionletCurve<T>::smileSectionImpl(QuantLib::Time optionTime) const {
    ...
    return boost::make_shared<QuantLib::FlatSmileSection>(optionTime, vol, dayCounter(), 
        QuantLib::Null<QuantLib::Rate>(), volatilityType_, displacement_);
}
...
template <class T>
void InterpolatedOptionletCurve<T>::initialise() {
    ...
    for (QuantLib::Size i = 1; i < dates_.size(); ++i) {
        ...
        QL_REQUIRE(!QuantLib::close(this->times_[i], this->times_[i-1]), "The " << QuantLib::io::ordinal(i) << 
            " date " << dates_[i] << " and " << QuantLib::io::ordinal(i - 1) << " date " << dates_[i - 1] << 
            " correspond to the same time, " << this->times_[i] << ", under this curve's day count convention, " << 
            dayCounter());
