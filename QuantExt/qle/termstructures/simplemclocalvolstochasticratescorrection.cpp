...
void SimpleMcLocalVolStochasticRatesCorrection::performCalculations() const {
    ...
    for (Size pass = 0; pass < nPasses_; ++pass) {

        FxModel::Discretization fxDisc = pass == 0 ? FxModel::Discretization::Euler : FxModel::Discretization::PC;
        ...
        for (Size i = 1; i < timeGrid_.size(); ++i) {
            ...
            for (Size p = 0; p < nPaths_; ++p) {
                ...
                Array sStateUpd =
                    S_->marginalStep(t0, sState[p], dt, getProjectedArray(dw, m_r + m_q, m_s), rs, qs, fxDisc);
                ...
                Array sStateUpd_u =
                    S_->marginalStep(t0, sState_u[p], dt, getProjectedArray(dw, m_r + m_q, m_s), rs_u, qs_u, fxDisc);
                applyCorrection_ = true;
