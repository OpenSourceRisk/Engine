...
DynamicBlackVolTermStructure<mode>::DynamicBlackVolTermStructure(
    const Handle<BlackVolTermStructure> &source, Natural settlementDays,
...
template <typename mode> void DynamicBlackVolTermStructure<mode>::update() {
    BlackVolTermStructure::update();
}

template <typename mode>
Date DynamicBlackVolTermStructure<mode>::maxDate() const {
...
template <typename mode>
Real DynamicBlackVolTermStructure<mode>::minStrike() const {
...
template <typename mode>
Real DynamicBlackVolTermStructure<mode>::maxStrike() const {
    if (stickyness_ == StickyStrike) {
...
template <typename mode>
Volatility DynamicBlackVolTermStructure<mode>::blackVolImpl(Time t,
                                                            Real strike) const {
