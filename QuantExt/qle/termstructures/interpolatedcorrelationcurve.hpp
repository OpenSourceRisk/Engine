...
using namespace QuantLib;
    ...
    class InterpolatedCorrelationCurve
        : public CorrelationTermStructure,
    ...
    template <class T>
    InterpolatedCorrelationCurve<T>::InterpolatedCorrelationCurve(
                                    const std::vector<Time>& times,
                                    const std::vector<Handle<Quote> >& quotes,
                                    const DayCounter& dayCounter,
                                    const Calendar& calendar,
                                    const T& interpolator)
    : CorrelationTermStructure(0, calendar, dayCounter),
      InterpolatedCurve<T>(std::vector<Time>(), std::vector<Real>(), interpolator), quotes_(quotes)
    {
        ...
        for (Size i = 0; i < this->quotes_.size(); i++) {
            QL_REQUIRE(std::fabs(this->quotes_[i]->value()) <= 1.0, "correlation not in range (-1.0,1.0): " << this->data_[i]);
