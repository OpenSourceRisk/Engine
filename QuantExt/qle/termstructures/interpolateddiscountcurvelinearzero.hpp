...
/*! \file interpolateddiscountcurvelinearzerp.hpp
...
//! InterpolatedDiscountCurveLinearZero as in QuantLib, but with floating discount quotes and floating reference date
/*! InterpolatedDiscountCurveLinearZero as in QuantLib, but with
    ...
    i.e. settlement days are zero and calendar is NullCalendar()
        ...
        \ingroup termstructures
*/
class InterpolatedDiscountCurveLinearZero : public YieldTermStructure, public LazyObject {
public:
    //! \name Constructors
    //@{
    //! times based constructor, note that times should be consistent with day counter dc passed
    InterpolatedDiscountCurveLinearZero(const std::vector<Time>& times, const std::vector<Handle<Quote> >& quotes,
                               const DayCounter& dc)
    ...
    InterpolatedDiscountCurveLinearZero(const std::vector<Date>& dates, const std::vector<Handle<Quote> >& quotes,
                                        const DayCounter& dc)
...
protected:
    void performCalculations() const {
        today_ = Settings::instance().evaluationDate();
        data_[0] = std::log(quotes_[1]->value()) / -times_[1];
        for (Size i = 1; i < times_.size(); ++i) {
            data_[i] = std::log(quotes_[i]->value()) / -times_[i];            
        }        
