...
bool Thailand::SetImpl::isBusinessDay(const Date& date) const {
    ...
    if (isWeekend(w)
        ...
        // Songkran Festival
        || (d == 13 && m == April) || ((d == 14 || (d == 16 && w == Tuesday)) && m == April) ||
        ((d == 15 || (d == 16 && w == Monday)) && m == April)
        // Labour Day
        || ((d == 1 || ((d == 2 || d == 3) && w == Monday)) && m == May)
        // H. M. King's Birthday
        || ((d == 28 || ((d == 29 || d == 30) && w == Monday)) && m == July)
        // Queens Birthday
        || ((d == 12 || ((d == 13 || d == 14) && w == Monday)) && m == August)
        // Memorial Day for King
        || ((d == 13 || ((d == 14 || d == 15) && w == Monday)) && m == October)
        // Chulalongkorn Day
        ...
        // New Years Eve
        || (d == 31 && m == December) || ((d == 2 || d == 3) && w == Tuesday && m == January))
        return false;
    return true;
}
