...
TRSCashFlow::TRSCashFlow(const Date& paymentDate, const Date& fixingStartDate, const Date& fixingEndDate,
                         ...
                         const QuantLib::ext::shared_ptr<FxIndex>& fxIndex, const bool applyFXIndexFixingDays)
    : paymentDate_(paymentDate), fixingStartDate_(fixingStartDate), fixingEndDate_(fixingEndDate), notional_(notional),
      index_(index), initialPrice_(initialPrice), fxIndex_(fxIndex), applyFXIndexFixingDays_(applyFXIndexFixingDays) {
    registerWith(index_);
...
Real TRSCashFlow::fxStart() const {
    return fxIndex_ ? fxIndex_->fixing(fxFixingStartDate()) : 1.0;
}

Real TRSCashFlow::fxEnd() const {
    return fxIndex_ ? fxIndex_->fixing(fxFixingEndDate()) : 1.0;
}

Date TRSCashFlow::fxFixingStartDate() const {
    if (fxIndex_ == nullptr){
        return Date();
    }
    return applyFXIndexFixingDays_ ? fxIndex_->fixingDate(fixingStartDate_)
                                   : fxIndex_->fixingCalendar().adjust(fixingStartDate_, Preceding);
}

Date TRSCashFlow::fxFixingEndDate() const {
    if (fxIndex_ == nullptr){
        return Date();
    }
    return applyFXIndexFixingDays_ ? fxIndex_->fixingDate(fixingEndDate_)
                                   : fxIndex_->fixingCalendar().adjust(fixingEndDate_, Preceding);
}
...
TRSLeg& TRSLeg::withApplyFXIndexFixingDays(bool applyFXIndexFixingDays) {
...
TRSLeg::operator Leg() const {
    ...
    for (Size i = 0; i < valuationDates_.size() - 1; ++i) {
        ...
        leg.push_back(QuantLib::ext::make_shared<TRSCashFlow>(paymentDates_[i], valuationDates_[i], valuationDates_[i + 1],
                                                          notional_, index_, initialPrice, fxIndex_, applyFXIndexFixingDays_));
    }
    return leg;
}
