...
namespace QuantExt {
    ...
    Date AverageONIndexedCoupon::fixingDate() const {
        return fixingDates_[fixingDates_.size() - rateCutoff_];
    }

    void AverageONIndexedCoupon::accept(AcyclicVisitor& v) {
        Visitor<AverageONIndexedCoupon>* v1 =
            dynamic_cast<Visitor<AverageONIndexedCoupon>*>(&v);
        if (v1 != 0) {
            v1->visit(*this);
        } else {
    ...
    AverageONLeg::AverageONLeg(const Schedule& schedule,
        const boost::shared_ptr<OvernightIndex>& i)
    : schedule_(schedule), overnightIndex_(i), paymentAdjustment_(Following),
      paymentCalendar_(Calendar()), rateCutoff_(0) {}
    ...
    AverageONLeg& AverageONLeg::withAverageONIndexedCouponPricer(
        const boost::shared_ptr<AverageONIndexedCouponPricer>& couponPricer) {
