...
SubPeriodsLeg1::SubPeriodsLeg1(const Schedule& schedule, const QuantLib::ext::shared_ptr<InterestRateIndex>& index)
    : schedule_(schedule), index_(index), notionals_(std::vector<Real>(1, 1.0)), paymentAdjustment_(Following),
      paymentCalendar_(Calendar()), type_(SubPeriodsCoupon1::Compounding), includeSpread_(false), paymentLag_(0) {}
...
SubPeriodsLeg1& SubPeriodsLeg1::withPaymentLag(Integer lag) {
    paymentLag_ = lag;
    return *this;
}

SubPeriodsLeg1& SubPeriodsLeg1::withPaymentDates(const std::vector<Date>& paymentDates) {
...
SubPeriodsLeg1::operator Leg() const {
    ...
    if (!paymentDates_.empty()) {
        QL_REQUIRE(paymentDates_.size() == numPeriods, "Expected the number of explicit payment dates ("
            << paymentDates_.size() << ") to equal the number of calculation periods (" << numPeriods << ")");
    }
    ...
    for (Size i = 0; i < numPeriods; ++i) {
        ...
        if (!paymentDates_.empty()) {
            ...
            paymentDate = paymentCalendar.advance(endDate, paymentLag_, Days, paymentAdjustment_);
        }
        ...
        try {
            auto cashflow = ext::make_shared<SubPeriodsCoupon1>(paymentDate, detail::get(notionals_, i,
                notionals_.back()), startDate, endDate, index_, type_, paymentAdjustment_,
                detail::get(spreads_, i, 0.0), paymentDayCounter_, includeSpread_, detail::get(gearings_, i, 1.0));
    ...
    QuantExt::setCouponPricer(cashflows, ext::make_shared<SubPeriodsCouponPricer1>());
