...
namespace QuantExt {
    ...
    FloatingAnnuityCoupon::FloatingAnnuityCoupon(
                            Real annuity,
                            bool underflow,
                            const boost::shared_ptr<Coupon>& previousCoupon,
                            ...
                            bool isInArrears)
        : Coupon(paymentDate, 0.0, startDate, endDate, refPeriodStart, refPeriodEnd),
          annuity_(annuity), underflow_(underflow), previousCoupon_(previousCoupon),
          fixingDays_(fixingDays), index_(index), gearing_(gearing), spread_(spread),
          dayCounter_(dayCounter), isInArrears_(isInArrears) {
    ...
    Rate FloatingAnnuityCoupon::previousNominal() const {
        return previousCoupon_->nominal();
    }
    ...
    Rate FloatingAnnuityCoupon::rate() const {
        return gearing_ * (indexFixing() + spread_);
    }
    ...
    Real FloatingAnnuityCoupon::indexFixing() const {
    ...
    void FloatingAnnuityCoupon::accept(AcyclicVisitor& v) {
        Visitor<FloatingAnnuityCoupon>* v1 =
            dynamic_cast<Visitor<FloatingAnnuityCoupon>*>(&v);
        if (v1 != 0)
            v1->visit(*this);
        else
            Coupon::accept(v);
