...
DiscountingCommodityCurrencySwapEngine::DiscountingCommodityCurrencySwapEngine(
    ...
    : DiscountingCurrencySwapEngine(discountCurves, fxQuotes, currencies, npvCurrency,
                                    includeSettlementDateFlows, settlementDate, npvDate,
                                    spotFXSettleDateVec) {}

void DiscountingCommodityCurrencySwapEngine::calculate() const {
    // Run base class calculation (leg NPVs, total NPV, etc.)
    DiscountingCurrencySwapEngine::calculate();

    // Compute currentNotional: max current cashflow amount across legs, converted to NPV currency
    Date asof = Settings::instance().evaluationDate();
    Real currentNotional = Null<Real>();
    std::string notionalCcy = npvCurrency_.code();
    ...
    for (Size i = 0; i < arguments_.legs.size(); ++i) {
        for (Size j = 0; j < arguments_.legs[i].size(); ++j) {
            ...
            if (flow->date() > asof) {
                ...
                // Convert to NPV currency using FX quote
                Handle<Quote> fx = fetchFX(arguments_.currency[i]);
                if (!fx.empty())
                    amount *= fx->value();
    ...
    if (currentNotional != Null<Real>()) {
        ...
        results_.additionalResults["notionalCurrency"] = notionalCcy;
    }
}
