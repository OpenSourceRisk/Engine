...
namespace QuantExt {
    ...
    void VarSwapEngine::calculate() const {
        ...
        if (arguments_.startDate > today) {
            ...
            //cout << setprecision(6);
            //cout << "Total " << totalTime << " Acc:" << accTime << " Fut:"<<futTime << endl;
            //cout << "Accrued Variance = " << accVar << endl;
            //cout << "Future Variance = " << futVar << endl;
            //cout << "XX," << QuantLib::io::iso_date(today) << "," << equityPrice_->value() << "," << totalTime << "," << accTime << ","<<futTime << "," << variance << "," << futVar << ","; //endl;
        }
    ...
    Real VarSwapEngine::calculateFutureVariance() const {
        ...
        //cout << "Price " << equityPrice_->value() << " Fwd " << fwd << " (time " << time << ")" << endl;
        Real dk = stepSize_ * sqrt(time);
        vector<Real> callStrikes (numCalls_);
        for (Size i = 0; i < numCalls_; i++)
            callStrikes[i] = fwd * (1 + i*dk);

        vector<Real> putStrikes (numPuts_);
        ...
        //cout << "FutVariance " << " time " << time << " process(time) " << process->time(arguments_.maturityDate) << " dk " << dk << " npv " << vs->NPV() << endl;
        return vs->variance();
    }
