...
CrossAssetModelImpliedSwaptionVolTermStructure::CrossAssetModelImpliedSwaptionVolTermStructure(
    ...
    : SwaptionVolatilityStructure(bdc, dc == DayCounter() ? model->irlgm1f(0)->termStructure()->dayCounter() : dc),
      model_(model), ccyIndex_(model_->ccyIndex(swapIndex->currency())),
      impliedDiscountCurve_(impliedDiscountCurve),
      impliedIborIndices_(impliedIborIndices), 
      swapIndex_(swapIndex), shortSwapIndex_(shortSwapIndex),
...
Real CrossAssetModelImpliedSwaptionVolTermStructure::volatilityImpl(Time optionTime, Time swapLength,
    ...
    Period swapTenor = int(floor(swapLength + 0.1)) * Years;
    ...
    std::vector<Period> tenorGrid = {1 * Days,  1 * Years,  2 * Years,  3 * Years, 5 * Years,
    ...
    /*********************************************
     * Forward starting ATM Swap                 *
     * FIXME: The fixed period convention should *
     *        depend on the underlying term      *
     *********************************************/

    Period expiryTenor = optionTime < 1 ? optionTime * 12 * Months : optionTime * Years;
    Date expiry = referenceDate_ + expiryTenor;
    ...
    try {
        price = swaption->NPV();
        impliedVol = swaption->impliedVolatility(price, discountCurve, 0.1);
