...
/*! \file commodityschwartzconstantparametrization.hpp
    \brief Schwartz commodity model constant parametrization
    \ingroup models
*/

#ifndef quantext_com_schwartz_constant_parametrization_hpp
#define quantext_com_schwartz_constant_parametrization_hpp
...
/*! COM parametrization for the Schwartz (1997) mean-reverting one-factor model
    ...
    dF(t,T) / F(t,T) = exp (b) * sigma * exp(-kappa * (T-t)) * dW    
    where b is a constant seasonality adjustment factor.
    ...
    The stochastic future price curve in terms of X(t) is
        F(t,T) = F(0,T) * exp(b) * exp( X(t) * exp(-kappa*(T-t) - 1/2 * (V(0,T) - V(t,T))
    with
        V(t,T) = sigma^2 * exp(b) * (1 - exp(-2*kappa*(T-t))) / (2*kappa)
    and
    ...
    \ingroup models
*/
class CommoditySchwartzConstantParametrization : public CommoditySchwartzParametrization {
public:
    /*! The currency refers to the commodity currency, the
        fx spot is as of today (i.e. the discounted spot) */
    CommoditySchwartzConstantParametrization(const Currency& currency, const std::string& name,
...
protected:
    ...
    const QuantLib::ext::shared_ptr<PseudoParameter> a_;
};

inline Real CommoditySchwartzConstantParametrization::direct(const Size i, const Real x) const { 
        return x * x; 
}

inline Real CommoditySchwartzConstantParametrization::inverse(const Size i, const Real y) const {   
        return std::sqrt(y);
}

inline Real CommoditySchwartzConstantParametrization::variance(const Time t) const {
...
inline Real CommoditySchwartzConstantParametrization::sigma(const Time u) const {
    Real sig = direct(0, sigma_->params()[0]);
    Real kap = direct(0, kappa_->params()[0]);
    if (driftFreeState_)
...
inline Real CommoditySchwartzConstantParametrization::sigmaParameter() const {
    return direct(0, sigma_->params()[0]);
}

inline Real CommoditySchwartzConstantParametrization::kappaParameter() const {
    return direct(0, kappa_->params()[0]);
}

inline Real CommoditySchwartzConstantParametrization::m(const QuantLib::Time t) const{
    return std::exp(a(t));
}

inline Real CommoditySchwartzConstantParametrization::a(const QuantLib::Time t) const{
    return direct(0, a_->params()[0]);
}
inline const QuantLib::ext::shared_ptr<Parameter> CommoditySchwartzConstantParametrization::parameter(const Size i) const {
