...
Real CrCirpp::intensity(Time t, Real y_t) const {
    if (parametrization_->shifted())
        return psi(t) + y_t;
    else
        return y_t;
...
Real CrCirpp::psi(Time t) const {

    Real hazard_market_t = parametrization_->termStructure()->hazardRate(t);
    Real psi_t = hazard_market_t + dlnPCir(t);

    return psi_t;
}

Real CrCirpp::dlnPCir(Time t) const {
    /*
    Real a_ = a();
    Real theta_ = theta();
    Real sigma_ = sigma();
    ...
    //Differentiation was done by QuickMath.com
    //ln(((2*h*exp( (a+h) * T/2 ) )/( 2*h + (a+h) * (exp(T*h)-1)  ))^(2*a*o/s^2) * exp ( -(2*(exp(T*h) -1) ) / ( 2*h +
    (a+h)*(exp(T*h)-1)  ) * y0 ))
    //differentiate with respect to T

    Real h = sqrt(a_*a_ + 2.0 * sigma_ * sigma_);
    Real ehT = exp(h*t);
    Real ah = a_+ h;
    Real sigma2 = sigma_ * sigma_;
    Real denom = (ehT-1.0)*ah + 2.0*h;
    Real expo = 2.0*a_*theta_/sigma2;

    Real first = exp( 2.0*(ehT-1.0)*y0_/denom - a_*ah*theta_*t/sigma2 );
    Real second = exp ( a_*ah*theta_*t/sigma2 - 2.0*(ehT-1.0)*y0_/denom );
    Real thirdt = pow( h/denom , expo );
    Real fourth = pow( 2.0 , expo );
    Real fifth1 = - 2.0*ehT*h*y0_/denom;
    Real fifth2 = + 2.0*ehT*(ehT-1.0)*h*ah*y0_/(denom*denom);
    Real fifth3 = + a_*ah*theta_/sigma2;
    Real sixth1 = a_*second*exp( h*t );
    Real sixth2 = h*ah*thirdt;
    Real sixth3 = theta_ * pow( 2.0, expo+1.0 );
    Real sixth4 = denom*sigma2;
    Real seventh = thirdt * fourth;

    Real result = first * ( second * thirdt * fourth * (fifth1+fifth2+fifth3) - sixth1*sixth2*sixth3/sixth4 ) / seventh;
*/
    Real dt = 0.0001;
    Real Erster = zeroBond(0.0, t - dt / 2.0, y0(t));
    Real Zweiter = zeroBond(0.0, t + dt / 2.0, y0(t));
    Real result = (std::log(Zweiter) - std::log(Erster)) / dt;
