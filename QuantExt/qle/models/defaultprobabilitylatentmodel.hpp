...
#ifndef quantext_default_latent_model_hpp
#define quantext_default_latent_model_hpp
...
*/
template <class copulaPolicy> class DefaultLatentModel : public LatentModel<copulaPolicy> {
...
protected:
    ...
    mutable QuantLib::ext::shared_ptr<Basket> basket_;
    ...
    */
    DefaultLatentModel(const std::vector<std::vector<Real>>& factorWeights,
        ...
        : LatentModel<copulaPolicy>(factorWeights, ini),
          integration_(LatentModel<copulaPolicy>::IntegrationFactory::createLMIntegration(factorWeights[0].size(),
                                                                                          integralType)) {}
    DefaultLatentModel(const Handle<Quote>& mktCorrel, Size nVariables,
                       LatentModelIntegrationType::LatentModelIntegrationType integralType,
                       const initTraits& ini = initTraits())
...
public: // open since users access it for performance on joint integrations.
    ...
    */
    Probability conditionalDefaultProbabilityInvP(Real invCumYProb, Size iName, const std::vector<Real>& m) const {
        Real sumMs = std::inner_product(factorWeights_[iName].begin(), factorWeights_[iName].end(), m.begin(), 0.);
...
protected:
    ...
    */
    Probability conditionalDefaultProbability(const Date& date, Size iName, const std::vector<Real>& mktFactors) const {
        const QuantLib::ext::shared_ptr<Pool>& pool = basket_->pool();
    ...
    Probability condProbProduct(Real invCumYProb1, Real invCumYProb2, Size iName1, Size iName2,
    ...
    const QuantLib::ext::shared_ptr<LMIntegration>& integration() const override { return integration_; }
...
public:
    ...
    */
    Probability probOfDefault(Size iName, const Date& d) const {
        ...
        return integratedExpectedValue(QuantLib::ext::function<Real(const std::vector<Real>& v1)>(
            QuantLib::ext::bind(&DefaultLatentModel<copulaPolicy>::conditionalDefaultProbabilityInvP, this,
                        inverseCumulativeY(pUncond, iName), iName, QuantLib::ext::placeholders::_1)));
    }
    ...
    */
    Probability probAtLeastNEvents(Size n, const Date& date) const {
        return integratedExpectedValue(QuantLib::ext::function<Real(const std::vector<Real>& v1)>(QuantLib::ext::bind(
            &DefaultLatentModel<copulaPolicy>::conditionalProbAtLeastNEvents, this, n, QuantLib::ext::cref(date), QuantLib::ext::placeholders::_1)));
    }
};

//---- Defines -----------------------------------------------------------

template <class CP> Real DefaultLatentModel<CP>::defaultCorrelation(const Date& d, Size iNamei, Size iNamej) const {
    QL_REQUIRE(basket_, "No portfolio basket set.");
    ...
    if (iNamei != iNamej) {
        E1i1j = integratedExpectedValue(QuantLib::ext::function<Real(const std::vector<Real>& v1)>(
            QuantLib::ext::bind(&DefaultLatentModel<CP>::condProbProduct, this, invPi, invPj, iNamei, iNamej, QuantLib::ext::placeholders::_1)));
    } else {
        E1i1j = pi;
    }
...
template <class CP>
Real DefaultLatentModel<CP>::conditionalProbAtLeastNEvents(Size n, const Date& date,
                                                           const std::vector<Real>& mktFactors) const {
    QL_REQUIRE(basket_, "No portfolio basket set.");
    ...
    */
    ...
    BigNatural limit = static_cast<BigNatural>(std::pow(2., (int)(poolSize)));
    ...
    for (BigNatural mask = static_cast<BigNatural>(std::pow(2., (int)(n)) - 1); mask < limit; mask++) {
        // cheap permutations
