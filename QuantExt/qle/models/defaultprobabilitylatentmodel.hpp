...
public:
    ...
    */
    Probability probOfDefault(Size iName, const Date& d) const {
        ...
        return integratedExpectedValue(std::function<Real(const std::vector<Real>& v1)>(
            std::bind(&DefaultLatentModel<copulaPolicy>::conditionalDefaultProbabilityInvP, this,
                        inverseCumulativeY(pUncond, iName), iName, std::placeholders::_1)));
    }
...
template <class CP> Real DefaultLatentModel<CP>::defaultCorrelation(const Date& d, Size iNamei, Size iNamej) const {
    ...
    if (iNamei != iNamej) {
        E1i1j = integratedExpectedValue(std::function<Real(const std::vector<Real>& v1)>(
            std::bind(&DefaultLatentModel<CP>::condProbProduct, this, invPi, invPj, iNamei, iNamej, std::placeholders::_1)));
    } else {
        E1i1j = pi;
    }
