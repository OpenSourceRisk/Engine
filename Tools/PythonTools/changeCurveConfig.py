...
def process_xml_file(file_path):
    ...
    for ycs in tree.iter():
        if ycs.tag == "YieldCurves":
            for yc in list(ycs):
                for segments in list(yc):
                    for segment in list(segments):
                        if segment.tag == "TenorBasis":
                            for child in list(segment):
                                if(child.tag == "ProjectionCurveLong"):
                                    child.tag = "ProjectionCurvePay"
                                if(child.tag == "ProjectionCurveShort"):
                                     child.tag = "ProjectionCurveReceive"
...
def main():
    ...
    for root, dirs, files in os.walk(folder_path):
        for file_name in files:
            if file_name.endswith("curveconfig.xml"):
