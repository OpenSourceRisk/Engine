...
void SensitivityAnalysis2Test::testSensitivities() {
    ...
    portfolio->add(buildSwap("1_Swap_EUR", "EUR", true, 10.0, 0, 10, 0.03, 0.00, "1Y", "30/360",
                                                "6M", "A360", "EUR-EURIBOR-6M"));
    portfolio->add(buildEuropeanSwaption(
        "5_Swaption_EUR", "Long", "EUR", true, 10.0, 10, 10, 0.03,
    ...
    // analytic results
    map<string, Real> analyticalResultsDelta = {
    ...
    map<string, Real> analyticalResultsGamma = {
        { "1_Swap_EUR DiscountCurve/EUR/0/1M", 0 },
    ...
    for (auto const& x : deltaMap) {
        ...
        if (analyticalResultsDelta.count(key) > 0) {
            if (!check(analyticalResultsDelta.at(key), scaledResult))
                BOOST_TEST_ERROR("Sensitivity analysis result " << key << " (" << scaledResult
    ...
    if (foundDeltas != analyticalResultsDelta.size())
        BOOST_TEST_ERROR("Mismatch between number of analytical results for delta ("
    ...
    if (foundGammas != analyticalResultsGamma.size())
        BOOST_TEST_ERROR("Mismatch between number of analytical results for gamma ("
