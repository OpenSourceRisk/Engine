...
protected:
    ...
    */
    ...
    std::map<CrifRecord::RiskType, std::vector<QuantLib::Real>> rwBucket_;
    std::map<std::pair<CrifRecord::RiskType, std::string>, std::vector<QuantLib::Real>> rwLabel_1_;
    ...
    */
    std::map<CrifRecord::RiskType, QuantLib::Matrix> interBucketCorrelation_;

    /*! Map from risk type to an intra-bucket correlation for that
        risk type i.e. correlation between qualifiers of the risk type that fall
        in the same bucket
    */
    std::map<CrifRecord::RiskType, std::vector<QuantLib::Real>> intraBucketCorrelation_;
