    ...
    for (XMLNode* testCase = XMLUtils::getChildNode(node, "StressTest"); testCase;
        ...
        if (fxVols) {
            for (XMLNode* child = XMLUtils::getChildNode(fxVols, "FxVolatility"); child;
                ...
                auto shiftType = QuantExt::parseShiftType(XMLUtils::getChildValue(child, "ShiftType"));
                ...
                if ((shiftsNode != nullptr) && (shiftExpiriesNode != nullptr)) {
                    ...
                    if (weightingSchema == "unadjusted") {
                        ...
                        QL_FAIL("FxVolStressTestData: unexpected weighting scheme, got "
                                << weightingSchema << " expected 'unadjusted' or 'weighted', please check config for "
                                << ccypair);
                    }
                    test.fxVolShifts[ccypair] = ext::make_shared<FXVolShiftData>(data);
                } else {
                    QL_FAIL(
                        "Expect either Shifts and Shiftsexpiries nodes or a WeightedShifts node, please check config "
        ...
        if (equityVols) {
            for (XMLNode* child = XMLUtils::getChildNode(equityVols, "EquityVolatility"); child;
                ...
                VolShiftData data;
                data.shiftType = QuantExt::parseShiftType(XMLUtils::getChildValue(child, "ShiftType"));
                data.shifts = XMLUtils::getChildrenValuesAsDoublesCompact(child, "Shifts", true);
                data.shiftExpiries = XMLUtils::getChildrenValuesAsPeriods(child, "ShiftExpiries", true);
        ...
        if (commodityCurves) {
            for (XMLNode* child = XMLUtils::getChildNode(commodityCurves, "CommodityCurve"); child;
                ...
                QL_REQUIRE(data.shifts.size() > 0,
                           "no shifts provided in commodity curve stress data for commodity = " << commodity);
        ...
        if (capVols) {
            for (XMLNode* child = XMLUtils::getChildNode(capVols, "CapFloorVolatility"); child;
                ...
                if (key.empty()) {
                    ...
                    if (!ccyAttr.empty()) {
                        key = ccyAttr;
                        WLOG("StressScenarioData: 'ccy' is deprecated as an attribute for CapFloorVolatilities, use "
                             "'key' "
                             "instead.");
                    }
                }
...
void volShiftDataToXml(ore::data::XMLDocument& doc, XMLNode* node,
                       const std::map<std::string, ext::shared_ptr<StressTestScenarioData::VolShiftData>>& data,
...
void commodityVolShiftDataToXml(
    ore::data::XMLDocument& doc, XMLNode* node,
    const std::map<std::string, ext::shared_ptr<StressTestScenarioData::CommodityVolShiftData>>& data,
    const std::string& identifier, const std::string& nodeName, const std::string& parentNodeName) {
    auto parentNode = XMLUtils::addChild(doc, node, parentNodeName);
    for (const auto& [key, data] : data) {
        auto childNode = XMLUtils::addChild(doc, parentNode, nodeName);
...
void fxVolDataToXml(ore::data::XMLDocument& doc, XMLNode* node,
    ...
    for (const auto& [key, data] : shiftdata) {
        if (data->mode == StressTestScenarioData::FXVolShiftData::AtmShiftMode::Explicit) {
            ...
            QL_REQUIRE(data->shifts.size() == 1, "Internal Error: WeightedShift should have only one shift, please "
                                                 "check construction of FxVolShiftData");
            QL_REQUIRE(data->shiftExpiries.size() == 1, "Internal Error: WeightedShift should have only one shift, "
                                                        "please check construction of FxVolShiftData");
            auto childNode = XMLUtils::addChild(doc, parentNode, nodeName);
            XMLUtils::addAttribute(doc, childNode, identifier, key);
            auto weightedShiftsNode = XMLUtils::addChild(doc, childNode, "WeightedShifts");
            ...
            QL_FAIL("internal error: unexpected FxVolShiftData, expected Explicit Shifts, Unadjusted or weighted "
                    "shifts, contact dev");
        }
    }
}
