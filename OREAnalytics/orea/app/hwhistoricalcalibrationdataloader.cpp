...
void HwHistoricalCalibrationDataLoader::cleanData() {
    ...
    /*
    // Find all missing days in the data set, use the next day's data as the replacement
    Size dateMax = 0;
    Size dateMin = QL_MAX_INTEGER;
    Size dateLength;
    string dateMaxKey;
    for (auto const& outer : fxSpots_) {
        dateLength = outer.second.size();
        if (dateLength < dateMin)
            dateMin = dateLength;
        if (dateLength > dateMax) {
            dateMax = dateLength;
            dateMaxKey = outer.first;
        }
    }
    for (auto const& outer : irCurves_) {
        dateLength = outer.second.size();
    ...
    while (dateMax != dateMin) {
        // If the missing date is before the first date, use the first date.
        // If the missing date is after the first date, use the previous available date.
        vector<Date> dateList;
        if (fxSpots_.find(dateMaxKey) == fxSpots_.end()) {
            for (auto const& dateValue : irCurves_.find(dateMaxKey)->second)
                dateList.push_back(dateValue.first);
        } else {
            for (auto const& dateValue : fxSpots_.find(dateMaxKey)->second)
                dateList.push_back(dateValue.first);
        }
        for (auto& outer : fxSpots_) {
            if (outer.first == dateMaxKey)
                continue;
            bool firstDate = true;
            for (auto const& date : dateList) {
                if (firstDate) {
                    if (outer.second.find(date) == outer.second.end()) {
                        if (date < outer.second.begin()->first) {
                            outer.second[date] = outer.second.begin()->second;
                            LOG("Add missing date " << date << " using value on " << outer.second.begin()->first
                                                    << ", Currency: " << outer.first
                                                    << ", Value: " << outer.second.begin()->second);
                        } else {
                            auto position = outer.second.upper_bound(date);
                            position--;
                            outer.second[date] = position->second;
                            LOG("Add missing date " << date << " using value on " << position->first << ", Currency: "
                                                    << outer.first << ", Value: " << position->second);
                            firstDate = false;
                        }
                    } else
                        firstDate = false;
                    ...
                    if (outer.second.find(date) == outer.second.end()) {
                        ...
                        position--;
                        outer.second[date] = position->second;
                        LOG("Add missing date " << date << " using value on " << position->first
                                                << ", Currency: " << outer.first << ", Value: " << position->second);
                    }
                }
            }
        }
            ...
            for (auto const& date : dateList) {
                if (firstDate) {
                    if (outer.second.find(date) == outer.second.end()) {
                        if (date < outer.second.begin()->first) {
                            outer.second[date] = outer.second.begin()->second;
                            auto to_string = static_cast<std::string (*)(Real)>(std::to_string);
                            LOG("Add missing date "
                                << date << " using value on " << outer.second.begin()->first
                                << ", Currency: " << outer.first << ", Value: "
                                << boost::algorithm::join(
                                       outer.second.begin()->second | boost::adaptors::transformed(to_string), ", "));
                        } else {
                            auto position = outer.second.upper_bound(date);
                            position--;
                            outer.second[date] = position->second;
                            auto to_string = static_cast<std::string (*)(Real)>(std::to_string);
                            LOG("Add missing date "
                                << date << " using value on " << position->first << ", Currency: " << outer.first
                                << ", Value: "
                                << boost::algorithm::join(position->second | boost::adaptors::transformed(to_string),
                                                          ", "));
                            firstDate = false;
                        }
                    } else
