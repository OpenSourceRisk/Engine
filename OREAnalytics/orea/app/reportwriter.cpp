...
void ReportWriter::writeCalibrationReport(ore::data::Report& report,
    ...
    report.addColumn("AssetType", string())
        ...
        .addColumn("BaseCurrency", string())
        .addColumn("Currency", string())
        .addColumn("ParameterName", string())
        .addColumn("ParameterValues", Size())
        .addColumn("ParameterTime", double(), 8)
        .addColumn("ParameterValue", double(), 8);    
	...
	for (Size i = 0; i < p.size(); ++i) {
	    ...
	    string paraType = "";
	    // IR
	    if (ext::dynamic_pointer_cast<IrLgm1fConstantParametrization>(p[i]))
	        paraType = "Lgm1fConstant";
	    else if (ext::dynamic_pointer_cast<IrLgm1fPiecewiseConstantParametrization>(p[i]))
	        paraType = "Lgm1fPiecewiseConstant"; // Hagan-HW
	    else if (ext::dynamic_pointer_cast<IrLgm1fPiecewiseLinearParametrization>(p[i]))
	        paraType = "Lgm1fPiecewiseLinear"; // Hagan-Hagan
	    else if (ext::dynamic_pointer_cast<IrLgm1fPiecewiseConstantHullWhiteAdaptor>(p[i]))
	        paraType = "Lgm1fPiecewiseConstantHullWhite"; // HW-HW
	    // FX
	    else if (ext::dynamic_pointer_cast<FxBsConstantParametrization>(p[i]))
	        paraType = "FxBsConstant";
	    else if (ext::dynamic_pointer_cast<FxBsPiecewiseConstantParametrization>(p[i]))
	        paraType = "FxBsPiecewiseConstant";
	    // EQ
	    ...
	    // CR
	    else if (ext::dynamic_pointer_cast<CrLgm1fConstantParametrization>(p[i]))
	        paraType = "CrLgm1fConstant";
	    else if (ext::dynamic_pointer_cast<CrLgm1fPiecewiseConstantParametrization>(p[i]))
	        paraType = "CrLgm1fPiecewiseConstant"; // Hagan-HW
	    else if (ext::dynamic_pointer_cast<CrLgm1fPiecewiseLinearParametrization>(p[i]))
	        paraType = "CrLgm1fPiecewiseLinear"; // Hagan-Hagan
	    else if (ext::dynamic_pointer_cast<CrLgm1fPiecewiseConstantHullWhiteAdaptor>(p[i]))
	        paraType = "CrLgm1fPiecewiseConstantHullWhiteAdaptor"; // HW-HW
	    // INF DK
	    else if (ext::dynamic_pointer_cast<InfDkConstantParametrization>(p[i]))
	        paraType = "InfDkConstant";
	    else if (ext::dynamic_pointer_cast<InfDkPiecewiseConstantParametrization>(p[i]))
	        paraType = "InfDkPiecewiseConstant"; // Hagan-HW
	    else if (ext::dynamic_pointer_cast<InfDkPiecewiseLinearParametrization>(p[i]))
	        paraType = "InfPiecewiseLinear"; // Hagan-Hagan
	    else if (ext::dynamic_pointer_cast<InfDkPiecewiseConstantHullWhiteAdaptor>(p[i]))
	        paraType = "InfDkfPiecewiseConstantHullWhiteAdaptor"; // HW-HW
	    // INF JY
	    else if (ext::dynamic_pointer_cast<InfJyParameterization>(p[i]))
	        paraType = "InfJy";
	    // COM
	    else if (ext::dynamic_pointer_cast<CommoditySchwartzParametrization>(p[i]))
	        paraType = "CommoditySchwartz"; 

	    // Reporting depends on the type of parametrization, but we may be able to use base classes
	    if (auto lgm = ext::dynamic_pointer_cast<IrLgm1fParametrization>(p[i])) {
	        QL_REQUIRE(lgm->numberOfParameters() == 2, "2 lgm1f parameters expected");
		Array alphaTimes = lgm->parameterTimes(0);
		Array alphaValues = lgm->parameterValues(0);
		QL_REQUIRE(alphaTimes.size() == alphaValues.size() - 1,
		...
		for (Size j = 0; j < alphaValues.size(); ++j) {
		    ...
		    report.add("IR")
		        ...
		        .add("Volatility")
		        .add(alphaValues.size())
		        .add(j == 0 ? 0.0 : alphaTimes[j-1])
		        .add(alphaValues[j]);
		}
		Array kappaTimes = lgm->parameterTimes(1);
		Array kappaValues = lgm->parameterValues(1);
		QL_REQUIRE(kappaTimes.size() == kappaValues.size() - 1,
		...
		Array times = fxbs->parameterTimes(0);
		Array values = fxbs->parameterValues(0);
