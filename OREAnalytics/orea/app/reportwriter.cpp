...
void ReportWriter::writeXVA(ore::data::Report& report, const string& allocationMethod,
    ...
    for (auto n : postProcess->nettingSetIds()) {
        report.next()
            ...
            .add(postProcess->ourNettingSetKVACCR(n))
            .add(postProcess->theirNettingSetKVACCR(n))
            .add(postProcess->ourNettingSetKVACVA(n))
            .add(postProcess->theirNettingSetKVACVA(n))
