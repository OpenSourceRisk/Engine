...
struct SensitivityRecord {
    ...
    */
    SensitivityRecord() : isPar(false), shift_1(0.0), shift_2(0.0), baseNpv(0.0), delta(0.0), gamma(0.0), theta(QuantLib::Null<QuantLib::Real>()) {}
    ...
    SensitivityRecord(const std::string& tradeId, bool isPar, const RiskFactorKey& key_1, const std::string& desc_1,
                      ...
                      QuantLib::Real gamma, QuantLib::Real theta = QuantLib::Null<QuantLib::Real>())
        : tradeId(tradeId), isPar(isPar), key_1(key_1), desc_1(desc_1), shift_1(shift_1), key_2(key_2), desc_2(desc_2),
          shift_2(shift_2), currency(currency), baseNpv(baseNpv), delta(delta), gamma(gamma), theta(theta) {}
