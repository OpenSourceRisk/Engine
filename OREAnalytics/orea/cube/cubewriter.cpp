...
void CubeWriter::write(const boost::shared_ptr<NPVCube>& cube, const std::map<std::string, std::string>& nettingSetMap,
    ...
    const std::set<string>& ids = cube->ids();
    ...
    for (const auto& id : ids) {
        if (nettingSetMap.find(id) != nettingSetMap.end())
            nettingSetIds[i] = nettingSetMap.at(id).c_str();
        else
            nettingSetIds[i] = "";

        fprintf(fp, fmt, id, nettingSetIds[i], static_cast<Size>(0), asofString.c_str(),
        ...
        for (Size j = 0; j < cube->numDates(); j++) {
            for (Size k = 0; k < cube->samples(); k++) {
                for (Size l = 0; l < cube->depth(); l++) {
                    fprintf(fp, fmt, id.c_str(), nettingSetIds[i], j + 1, dateStrings[j].c_str(), k + 1, l,
                            cube->get(i, j, k, l));
                }
            }
        }
