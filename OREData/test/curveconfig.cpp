...
void CurveConfigTest::testCurveConfigQuotes() {
    ...
    set<string> quotes = vars.curveConfigs.quotes();
    BOOST_REQUIRE_EQUAL(quotes.size(), vars.expectedQuotes.size());
    BOOST_CHECK_EQUAL_COLLECTIONS(quotes.begin(), quotes.end(), vars.expectedQuotes.begin(), vars.expectedQuotes.end());
}
