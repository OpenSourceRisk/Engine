...
public:
    ...
    TestMarket(Real spot, Real q, Real r, Real vol, bool withFixings = false) {
        ...
        // build discount
        yieldCurves_[make_tuple(Market::defaultConfiguration, YieldCurveType::Discount, "EUR")] = flatRateYts(r, Actual360());
        yieldCurves_[make_tuple(Market::defaultConfiguration, YieldCurveType::Discount, "JPY")] = flatRateYts(q, Actual360());
...
BOOST_AUTO_TEST_CASE(testFXAmericanOptionPrice) {
    ...
    for (auto& f : fxd) {
        ...
        OptionData optionData("Long", f.type == Option::Call ? "Call" : "Put", "American", false, vector<string>(1, maturityDate));
        Envelope env("CP1");
