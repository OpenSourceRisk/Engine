...
boost::shared_ptr<TodaysMarketParameters> marketParameters() {
    ...
    map<string, string> swapIndexMap = {{"USD-CMS-1Y", "USD-FedFunds"}, {"USD-CMS-30Y", "USD-LIBOR-3M"}, {"USD-CMS-2Y", "USD-LIBOR-3M"}, {"USD-CMS-10Y", "USD-LIBOR-3M"}};
    ...
    map<string, string> correlationMap = {{"EUR-CMS-10Y/EUR-CMS-2Y", "Correlation/EUR-CORR"}, {"USD-CMS-10Y/USD-CMS-2Y", "Correlation/USD-CORR"}};
...
boost::shared_ptr<Conventions> conventions() {
    ...
    conventions->add(boost::make_shared<CmsSpreadOptionConvention>("USD-CMS-10Y-2Y-CONVENTION", "0M", "2D", "3M", "2", "TARGET", "A360", "MF"));
...
boost::shared_ptr<CurveConfigurations> curveConfigurations() {
    ...
    configs->correlationCurveConfig("EUR-CORR") = boost::make_shared<CorrelationCurveConfig>(
        ...
        dayCounter, UnitedStates(), bdc, "EUR-CMS-10Y", "EUR-CMS-2Y", "EUR");
    configs->correlationCurveConfig("USD-CORR") = boost::make_shared<CorrelationCurveConfig>(
        ...
        CorrelationCurveConfig::QuoteType::Price, extrapolate, optionTenors3,
        Actual360(), TARGET(), ModifiedFollowing, "USD-CMS-10Y", "USD-CMS-2Y", "USD", "USD_SW_LN", "USD1D");
...
BOOST_AUTO_TEST_CASE(testCorrelationCurve) {
    ...
    // Just test that the building suceeded - the curve itself has been tested elsewhere
    Handle<QuantExt::CorrelationTermStructure> correlationCurve1 = market->correlationCurve("EUR-CMS-10Y","EUR-CMS-2Y");
    Handle<QuantExt::CorrelationTermStructure> correlationCurve2 = market->correlationCurve("USD-CMS-10Y","USD-CMS-2Y");
    ...
    Date qlEndDate1Y = calendar.advance(qlStartDate, 1 * Years,
                                       ModifiedFollowing);
    Date qlEndDate2Y = calendar.advance(qlStartDate, 2 * Years,
                                       ModifiedFollowing);
    ...
    LegData cms1YLeg(boost::make_shared<CMSSpreadLegData>("USD-CMS-10Y","USD-CMS-2Y", 2, true, std::vector<Real>(1, 0.0),
                        vector<string>(), std::vector<Real>(1, fairSpread1Y), vector<string>(), vector<double>(), 
                        vector<string>(), vector<double>(), vector<string>(), true),
                        false, "USD", cms1YSchedule, "A360", std::vector<Real>(1, 1));
    vector<LegData> legs1Y;
    legs1Y.push_back(cms1YLeg);

    LegData cms2YLeg(boost::make_shared<CMSSpreadLegData>("USD-CMS-10Y","USD-CMS-2Y", 2, true, std::vector<Real>(1, 0.0),
                        vector<string>(), std::vector<Real>(1, fairSpread2Y), vector<string>(), vector<double>(), 
                        vector<string>(), vector<double>(), vector<string>(), true),
