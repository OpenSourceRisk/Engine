...
typedef boost::bimap<string, OptionletVolatilityCurveConfig::VolatilityType> BmType;
const BmType volatilityTypeMap =
    list_of<BmType::value_type>("Normal", OptionletVolatilityCurveConfig::VolatilityType::Normal)(
        "Lognormal", OptionletVolatilityCurveConfig::VolatilityType::Lognormal)(
        "ShiftedLognormal", OptionletVolatilityCurveConfig::VolatilityType::ShiftedLognormal);
...
OptionletVolatilityCurveConfig::OptionletVolatilityCurveConfig(
...
void OptionletVolatilityCurveConfig::fromXML(XMLNode* node) {

    XMLUtils::checkNode(node, "OptionletVolatility");
...
XMLNode* OptionletVolatilityCurveConfig::toXML(XMLDocument& doc) {

    XMLNode* node = doc.allocNode("OptionletVolatility");
...
QuantExt::CapFloorTermVolSurfaceExact::InterpolationMethod OptionletVolatilityCurveConfig::interpolationMethod() const {
...
string OptionletVolatilityCurveConfig::toString(VolatilityType type) const {
...
const string& OptionletVolatilityCurveConfig::currency() const {
    string tenor;
...
void OptionletVolatilityCurveConfig::populateQuotes() {

    // Optionlet quotes are for the form:
    // OPTIONLET/(RATE_LNVOL|RATE_NVOL|RATE_SLNVOL)/<CCY>/<TENOR>/<IBOR_TENOR>/<ATM>/<RELATIVE>/<STRIKE> or
    // OPTIONLET/(RATE_LNVOL|RATE_NVOL|RATE_SLNVOL)/<CCY>/<IndexName>/<TENOR>/<IBOR_TENOR>/<ATM>/<RELATIVE>/<STRIKE>
    string ccy = currency();
    string tenor = indexTenor();

    // Volatility quote stem
    MarketDatum::QuoteType qType = quoteType();
    string stem = "OPTIONLET/" + to_string(qType) + "/" + ccy + "/";
    if(quoteIncludesIndexName())
	stem += index() + "/";

    // Optionlet matrix quotes. So, ATM flag is false i.e. 0 and RELATIVE flag is false also as strikes are absolute.
    ...
    // Optionlet shift quote depends only on the currency and ibor tenor and is of the form:
    // OPTIONLET/SHIFT/<CCY>/<IBOR_TERM> or
    // OPTIONLET/SHIFT/<CCY>/<IndexName>/<IBOR_TERM> or
    if (volatilityType_ == VolatilityType::ShiftedLognormal) {
        quotes_.push_back("OPTIONLET/SHIFT/" + ccy + "/" + (quoteIncludesIndexName() ? index() + "/" : "") + tenor);
    }
}

void OptionletVolatilityCurveConfig::configureExtrapolation(const string& extrapolation) {
...
void OptionletVolatilityCurveConfig::configureVolatilityType(const std::string& type) {
...
void OptionletVolatilityCurveConfig::configureType() {
...
void OptionletVolatilityCurveConfig::validate() const {
...
MarketDatum::QuoteType OptionletVolatilityCurveConfig::quoteType() const {
    switch (volatilityType_) {
    case OptionletVolatilityCurveConfig::VolatilityType::Lognormal:
        return MarketDatum::QuoteType::RATE_LNVOL;
    case OptionletVolatilityCurveConfig::VolatilityType::ShiftedLognormal:
        return MarketDatum::QuoteType::RATE_SLNVOL;
    case OptionletVolatilityCurveConfig::VolatilityType::Normal:
...
VolatilityType volatilityType(OptionletVolatilityCurveConfig::VolatilityType type) {
    switch (type) {
    case OptionletVolatilityCurveConfig::VolatilityType::Lognormal:
    case OptionletVolatilityCurveConfig::VolatilityType::ShiftedLognormal:
