        ...
        if (config->seasonalityBaseDate() != Null<Date>()) {
            ...
            for (Size i = 0; i < strFactorIDs.size(); i++) {
                ...
                if (marketQuote) {
                    ...
                    QuantLib::Size findex = (((Size)config->seasonalityBaseDate().month()) < sq->applyMonth())
                        ? 12 + ((Size)config->seasonalityBaseDate().month()) - sq->applyMonth()
                        : ((Size)config->seasonalityBaseDate().month()) - sq->applyMonth();
