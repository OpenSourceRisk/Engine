        ...
        try {
            ...
            QL_REQUIRE(p.second == 0 * QuantLib::Days || p.second == term,
                       "CurveID " << config.curveID()
                                  << " has a tenor suffix which does not match the term from the quotes (" << term
                                  << ")");
            std::string indexNameWithTerm =
                p.second == 0 * QuantLib::Days ? config.curveID() + "_" + to_string(term) : config.curveID();
            ...
            for (size_t i = 0; i < dps.size(); i++) {
                ...
                auto basket = QuantLib::ext::make_shared<QuantExt::Basket>(
                    basketRefDate, basketData.remainingNames, basketData.remainingWeights, pool, 0.0,
            ...
            ALOG("Failed to build base correlation from upfront for term " << term << " : " << e.what());
            QL_FAIL("Failed to build base correlation from upfront for term " << term
