...
SwaptionVolCurve::SwaptionVolCurve(Date asof, SwaptionVolatilityCurveSpec spec, const Loader& loader,
    ...
    : GenericYieldVolCurve(asof, loader, curveConfigs.swaptionVolCurveConfig(spec.curveConfigID()), requiredSwapIndices,
                           ...
                           [](const boost::shared_ptr<MarketDatum>& md, Period& term) {
                               boost::shared_ptr<SwaptionShiftQuote> q =
                                   boost::dynamic_pointer_cast<SwaptionShiftQuote>(md);
                               if (q == nullptr)
                                   return false;
                               term = q->term();
                               return true;
