...
BondYieldShiftedCurveHelperMarket::BondYieldShiftedCurveHelperMarket(
...
Handle<YieldTermStructure> BondYieldShiftedCurveHelperMarket::yieldCurve(const string& name, const string& configuration) const {
    return Handle<YieldTermStructure>(boost::make_shared<FlatForward>(0, NullCalendar(), 0.0, Actual365Fixed()));
}

Handle<Quote> BondYieldShiftedCurveHelperMarket::securitySpread(const string& securityID, const string& configuration) const {
    return Handle<Quote>(boost::make_shared<SimpleQuote>(0.0));
    }

Handle<QuantExt::CreditCurve> BondYieldShiftedCurveHelperMarket::defaultCurve(const string&,
...
Handle<Quote> BondYieldShiftedCurveHelperMarket::recoveryRate(const string&, const string& configuration) const {
        return Handle<Quote>(boost::make_shared<SimpleQuote>(0.0));
    }
