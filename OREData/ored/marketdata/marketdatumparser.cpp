...
QuantLib::ext::shared_ptr<MarketDatum> parseMarketDatum(const Date& asof, const string& datumName, const Real& value) {
    ...
    case MarketDatum::InstrumentType::BOND_FUTURE_OPTION: {
        // Quote of the form:
        // BOND_FUTURE_OPTION/<QUOTE_TYPE>/<UNDERLYING_CONTRACT_NAME>/<EXPIRY>/<STRIKE>/<EX_TYPE>/<PUT_CALL>
        // where:
        ...
        // - EX_TYPE is either A for American or E for European
        ...
        QL_REQUIRE(tokens[5] == "A" || tokens[5] == "E", "expected A for American or E for European as the "
            "penultimate element in " << datumName);
        QL_REQUIRE(tokens[6] == "C" || tokens[6] == "P", "expected C for call or P for put as the last "
            "element in " << datumName);

        const string& contractName = tokens[2];
        const string& expiry = tokens[3];
        auto strike = parseBaseStrike(tokens[4]);
        const string& exerciseType = tokens[5];
        bool isCall = tokens[6] == "C";

        return QuantLib::ext::make_shared<BondFutureOptionQuote>(value, asof, datumName, quoteType,
            contractName, expiry, strike, exerciseType, isCall);
    }
