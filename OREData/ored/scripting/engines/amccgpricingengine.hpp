...
struct TradeExposure {
    /* There are basically two types of trades
       ...
       The computation graph from the node startNodeRecombine to the node targetConditionalExpectationDerivatives
       is replayed within the exposure engine for AAD calculations. The component path values are replaced by path
       derivative values and recombined to conditional sensitivities. It is important that this part of the computation
       graph is suited to do this recombination. For the combination run, the regressor nodes and the additional
       required nodes will be populated, i.e. all nodes on which the combination depends must be listed in one of these
       members, including constants.
