...
void ScriptedInstrumentPricingEngine::calculate() const {
    ...
    if (generateAdditionalResults_) {
        ...
        for (auto const& r : additionalResults_) {
            ...
            if (s != workingContext->scalars.end()) {
                ...
                if (generateAdditionalResultsPathLevel_ && s->second.which() == ValueTypeWhich::Number) {
                    ...
                    pathLevelResult.back().time = boost::get<RandomVariable>(s->second).time();
            ...
            if (v != workingContext->arrays.end()) {
                ...
                for (auto const& d : v->second) {
                    ...
                    if (t.type() == typeid(double)) {
                        ...
                        tmpdoubleErrEst.push_back(addMcErrorEstimate(std::string(), boost::get<RandomVariable>(d)));
                        if (generateAdditionalResultsPathLevel_) {
                            pathLevelResult.push_back({});
                            pathLevelResult.back().resultId = r.first;
                            pathLevelResult.back().index = counter;
                            pathLevelResult.back().values =
                                static_cast<std::vector<double>>(boost::get<RandomVariable>(d));
                            pathLevelResult.back().time = boost::get<RandomVariable>(d).time();
                        }
                        ++counter;
                ...
                if (!tmpdouble.empty()) {
                    ...
                    if (tmpdoubleErrEst.front() != Null<Real>())
                        results_.additionalResults[r.first + "_MCErrEst"] = tmpdoubleErrEst;
                    if (generateAdditionalResultsPathLevel_) {
                        results_.additionalResults[r.first + "_pathlevel"] = pathLevelResult;
                    }
        ...
        for (Size i = 0; i < paylog->size(); ++i) {
            ...
            if (generateAdditionalResultsPathLevel_ && paylog->dates().at(i) > model_->referenceDate()) {
                ...
                pathLevelResult[i].values = static_cast<std::vector<double>>(paylog->amounts().at(i) / (fx * discount));
    ...
    if (amcEnabled_) {
        ...
        results_.additionalResults["amcCalculator"] = QuantLib::ext::static_pointer_cast<AmcCalculator>(
            QuantLib::ext::make_shared<ScriptedInstrumentAmcCalculator>(npv_, model_, ast_, context_, script_,
                                                                        interactive_, amcStickyCloseOutStates_));
    }
