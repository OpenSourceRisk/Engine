...
public:
    HestonModelBuilder(
        ...
        const std::vector<Period>& calibrationExpiries = {3 * Months, 6 * Months, 1 * Years, 2 * Years, 3 * Years,
                                                          5 * Years},
        const std::vector<Real>& calibrationMoneyness = {-2.0, -1.0, 0.0, 1.0, 2.0},
        const std::vector<Period>& calibrationVarianceTerms = {1 * Months, 3 * Months, 6 * Months, 1 * Years, 2 * Years,
                                                               3 * Years},
        ...
        const std::vector<Real>& maximumInitialValues = {0.1, 20.0, 10.0, 0.9, 0.1}, Real relaxedFellerConstraint = 1.0,
    ...
    HestonModelBuilder(
        const std::string& index, const Handle<YieldTermStructure>& curve,
        const ext::shared_ptr<GeneralizedBlackScholesProcess>& process, const std::set<Date>& simulationDates = {},
        const std::set<Date>& addDates = {}, const Size timeStepsPerYear = 1,
        const std::vector<Period>& calibrationExpiries = {3 * Months, 6 * Months, 1 * Years, 2 * Years, 3 * Years,
                                                          5 * Years},
        const std::vector<Real>& calibrationMoneyness = {-2.0, -1.0, 0.0, 1.0, 2.0},
        const std::vector<Period>& calibrationVarianceTerms = {1 * Months, 3 * Months, 6 * Months, 1 * Years, 2 * Years,
        ...
        const std::vector<bool>& fixedValues = {false, false, false, false, false},
        const std::string& calibrationMethod = "ConstantBestFit",
        const std::vector<Real>& maximumInitialValues = {0.1, 20.0, 3.0, 0.9, 0.1}, Real relaxedFellerConstraint = 1.0,
        Size maxCalibrationAttempts = 50, Real earlyExitThreshold = 0.005, Real maxAcceptableError = 0.05,
        const HestonProcess::Discretization& discretization = HestonProcess::QuadraticExponential,
        const std::string& referenceCalibrationGrid = "", const bool dontCalibrate = false,
        const Handle<YieldTermStructure>& baseCurve = {},
