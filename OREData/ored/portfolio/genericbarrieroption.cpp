...
// clang-format off
    ...
    static const std::string amcscript =
      ...
      "        NUMBER KnockedIn, KnockedOut, Active, rebate, TransatlanticActive;\n"
      "        NUMBER KnockedInAtObsTime[SIZE(BarrierMonitoringDates)], KnockedOutAtObsTime[SIZE(BarrierMonitoringDates)];\n"
      "        NUMBER Exercised, Triggered, TransatlanticTriggered;\n"
      "        NUMBER rebateAfterKI, valueAfterKI;\n"

      "        NUMBER U, i, k, d, s, j, currentNotional, levelIndex;\n"
      "\n"
      "        FOR d IN (1, SIZE(BarrierMonitoringDates), 1) DO\n"
      "\n"
      "          FOR i IN (1, SIZE(BarrierTypes), 1) DO\n"
      ...
      "                   IF KikoType == 1 OR { KikoType == 2 AND KnockedIn == 0 } OR { KikoType == 3 AND KnockedIn == 1 } THEN\n"
      "                     IF KnockedOut == 0 THEN\n"
      "                       s = DATEINDEX(BarrierMonitoringDates[d], _AMC_SimDates, GEQ);\n"
      "                       IF BarrierRebatePayTimes[i] == 0 THEN\n"
      "                         rebate = PAY( LongShort * BarrierRebates[i], BarrierMonitoringDates[d], BarrierMonitoringDates[d], BarrierRebateCurrencies[i] );\n"
      "                       ELSE\n"
      "                         rebate = PAY( LongShort * BarrierRebates[i], BarrierMonitoringDates[d], SettlementDate, BarrierRebateCurrencies[i] );\n"
      ...
      "                         IF {BarrierRebatePayTimes[i] == 0 AND AMC_SimDates[j] == BarrierMonitoringDates[d]} OR {BarrierRebatePayTimes[i] == 1 AND AMC_SimDates[j] >= BarrierMonitoringDates[d] AND AMC_SimDates[j] <= SettlementDate} THEN\n"
      "                           rebatesAMC[j] = rebatesAMC[j] + rebate;\n"
      "                         END;\n"
      "                       END;\n"
      "                     END;\n"
      "                     KnockedOut = 1;\n"
      ...
      "          END;\n"
      "\n"
      "          KnockedInAtObsTime[d] = KnockedIn;\n"    
      "          KnockedOutAtObsTime[d] = KnockedOut;\n"
      "\n"          
      "        END;\n"
      "\n"
      "        Active = 1;\n"
      ...
      "	         value = Active * TransatlanticActive * PAY( LongShort * Amount, ExpiryDate, SettlementDate, PayCurrency ) +\n"
      "                  rebate;\n"
      "          valueAfterKI = (1 - KnockedOut) * TransatlanticActive * Quantity * PAY( LongShort * Amount, ExpiryDate, SettlementDate, PayCurrency ) +\n"
      "                  rebateAfterKI;\n"
      "	       END;\n"
      "\n"
      "        IF value > 0 THEN\n"
      ...
      "          s = min(DATEINDEX(_AMC_SimDates[i], BarrierMonitoringDates, GEQ), SIZE(BarrierMonitoringDates));\n"
      "          IF s == 1 AND BarrierMonitoringDates[s] > _AMC_SimDates[i] THEN\n"
      "            _AMC_NPV[i] = max(NPVMEM(value, _AMC_SimDates[i], i), 0.0);\n"
      "          ELSE\n"
      "            IF BarrierMonitoringDates[s] > _AMC_SimDates[i] THEN\n"
      "              s = s - 1;\n"
      "            END;\n"
      "            IF KnockedOutAtObsTime[s] == 1 THEN\n"
      "              IF BarrierRebate != 0 AND _AMC_SimDates[i] < SettlementDate THEN\n"
      "                _AMC_NPV[i] = PAY( LongShort * BarrierRebate, SettlementDate, SettlementDate, BarrierRebateCurrency);\n"
      "              ELSE\n"
      "                _AMC_NPV[i] = rebates[i];\n"
      "              END;\n"
      "            ELSE\n"
      "              IF KnockedInAtObsTime[s] == 1 THEN\n"
      "                _AMC_NPV[i]  = max(NPVMEM(valueAfterKI, _AMC_SimDates[i], SIZE(_AMC_SimDates) + i,  KnockedInAtObsTime[s] == 1 AND KnockedOutAtObsTime[s] == 0 ), 0);\n"
      "              ELSE\n"
      "                _AMC_NPV[i]  = max(NPVMEM(value, _AMC_SimDates[i], 2 * SIZE(_AMC_SimDates) + i,  KnockedInAtObsTime[s] == 0 AND KnockedOutAtObsTime[s] == 0 ), 0);\n"
      "              END;\n"
      "            END;\n"
      "          END;\n"
      "        END; \n";
...
void GenericBarrierOption::build(const QuantLib::ext::shared_ptr<EngineFactory>& factory) {
    ...
    script_["AMC"] = ScriptedTradeScriptData(
        amcscript, "value",
        {{"currentNotional", "currentNotional"}, {"notionalCurrency", "PayCurrency"}}, {}, {},
        {ScriptedTradeScriptData::CalibrationData("Underlyings", {"Strike", "BarrierLevels"})});
