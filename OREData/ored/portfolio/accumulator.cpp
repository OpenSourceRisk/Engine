...
// clang-format off
    ...
    static const std::string accumulator02_script_fd =
        ...
        "\n"
        "                IF {BarrierStrictComparison == 0 AND KnockOutType == 4 AND fix >= KnockOutLevel} OR\n"
        "                   {BarrierStrictComparison == 0 AND KnockOutType == 3 AND fix <= KnockOutLevel} THEN\n"
        "                   IF KnockOutFixingAtSettlementDate == 1 THEN\n"
...
void Accumulator::build(const QuantLib::ext::shared_ptr<EngineFactory>& factory) {
    ...
    for (auto const& b : barriers_) {
        ...
        if (b.strictComparison() && strictBarrier=="0") {
            strictBarrier = b.strictComparison().value();
        }
    }
