...
void EquityOption::build(const boost::shared_ptr<EngineFactory>& engineFactory) {
    ...
    if (!strikeCurrency_.empty()) {
        QL_REQUIRE(parseCurrencyWithMinors(strikeCurrency_) == equityCurrency,
                    "Strike currency " << strikeCurrency_ << " does not match equity currency "
                                      << equityCurrency.code() << " for trade " << id() << ".");
    } else {
        DLOG("No StrikeCurrency provided, using equity currency " << equityCurrency.code() << ".");
    ...
    if (equityCurrency != parseCurrencyWithMinors(currency_)) {
        DLOG("Equity currency " << equityCurrency.code() << " and option currency " << currency_ << " for trade "
