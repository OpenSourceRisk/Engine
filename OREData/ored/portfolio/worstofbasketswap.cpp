        ...
        {"AMC",
         ScriptedTradeScriptData(
             std::string(
                 ...
                 "NUMBER lastIdx, accrualPeriodIdx, accrualFractions[SIZE(FixedTriggerLevels)], totalDays;\n"
                 "NUMBER ad, cd, dd, ed, fd;\n"
                 "NUMBER a, s, d_fl, d_fi, bwdPayoff, _AMC_NPV[SIZE(_AMC_SimDates)];\n"
                 "NUMBER nthPayoff_float[SIZE(FloatingPayDates)], nthPayoff_fixed[SIZE(FixedPayDates)];\n"
                 "NUMBER nthPayoff_equity, nthPayoff_initial;\n"
                 "\n"
                 "Option = Option + LOGPAY(LongShort * Quantity * InitialFixedRate, InitialFixedPayDate,\n"
                 "                         InitialFixedPayDate, Currency, 0, InitialFixedAmount);\n"
                 "nthPayoff_initial = PAY(LongShort * Quantity * InitialFixedRate, InitialFixedPayDate,\n"
                 "                        InitialFixedPayDate, Currency);\n"
                 "\n"
                 "n = SIZE(FloatingPeriodSchedule);\n"
                 "REQUIRE n - SIZE(FloatingPayDates) <= 1;\n"
                 "REQUIRE n - SIZE(FloatingFixingSchedule) <= 1;\n"
             ...
             std::string(
                 ...
                 "  Option = Option + LOGPAY(-1 * LongShort * Quantity * alive * fixing * floatingAccrualFraction,\n"
                 "                           FloatingFixingSchedule[d-1], FloatingPayDates[d+ad], Currency, 1, FloatingLeg);\n"
                 "  nthPayoff_float[d+ad] = nthPayoff_float[d+ad] + PAY(-1 * LongShort * Quantity * alive * fixing * floatingAccrualFraction,\n"
                 "                           FloatingFixingSchedule[d-1], FloatingPayDates[d+ad], Currency);\n"
                 "\n"
                 "  allAssetsTriggered = 1;\n"
                 "  FOR u IN (1, SIZE(Underlyings), 1) DO\n"
                 "    IF Underlyings[u](FixedDeterminationSchedule[d+cd]) < FixedTriggerLevels[d-1] * InitialPrices[u] THEN\n"
                 ...
                 "  Option = Option + LOGPAY(allAssetsTriggered * LongShort * Quantity * alive * FixedRate * couponAccumulation * fixedAccrualFraction,\n"
                 "                           FixedDeterminationSchedule[d+cd], FixedPayDates[d+fd], Currency, 2, FixedCouponLeg);\n"
                 "  nthPayoff_fixed[d+fd] = nthPayoff_fixed[d+fd] + PAY(allAssetsTriggered * LongShort * Quantity * alive * FixedRate * couponAccumulation * fixedAccrualFraction,\n"
                 "                           FixedDeterminationSchedule[d+cd], FixedPayDates[d+fd], Currency);\n"
                 "  couponAccumulation = allAssetsTriggered + (1-allAssetsTriggered) * couponAccumulation;\n"
                 "  IF AccumulatingFixedCoupons == 1 THEN\n"
                 "    couponAccumulation = couponAccumulation + (1 - allAssetsTriggered);\n"
                 "  END;\n"
                 ...
                 "      Option = Option + LOGPAY(LongShort * Quantity * alive * payoff * knockedIn, FloatingPeriodSchedule[n],\n"
                 "                               KnockInPayDate, Currency, 3, EquityAmountPayoff);\n"
                 "      nthPayoff_equity = PAY(LongShort * Quantity * alive * payoff * knockedIn, FloatingPeriodSchedule[n],\n"
                 "                             KnockInPayDate, Currency);\n"
                 "    END;\n"
                 "  END;\n"
                 "\n"
                 "  IF d != n THEN\n"
                 ...
                 "  END;\n"
                 "  d_fl = DATEINDEX(AllPayAndSimDates[a], FloatingPayDates, EQ);\n"
                 "  IF d_fl > 0 THEN\n"
                 "    bwdPayoff = bwdPayoff + nthPayoff_float[d_fl];\n"
                 "  END;\n"
                 "  IF d_fl == 1 THEN\n"
                 "    bwdPayoff = bwdPayoff + nthPayoff_initial;\n"
                 "  END;\n"
                 "  IF d_fl == SIZE(FloatingPayDates) THEN\n"
                 "    bwdPayoff = bwdPayoff + nthPayoff_equity;\n"
                 "  END;\n"
                 "  d_fi = DATEINDEX(AllPayAndSimDates[a], FixedPayDates, EQ);\n"
                 "  IF d_fi > 0 THEN\n"
                 "    bwdPayoff = bwdPayoff + nthPayoff_fixed[d_fi];\n"
                 "  END;\n"
                 "END;\n"),
             "Option",
             {{"currentNotional", "Quantity"},
             ...
             {ScriptedTradeScriptData::NewScheduleData(
                 "AllPayAndSimDates", "Join", {"_AMC_SimDates", "FloatingPayDates", "FixedPayDates"})},
             {},
             {"alive", "knockedIn"},
             {})}};
        // clang-format on
