...
FwdBondEngineBuilder::Curves FwdBondEngineBuilder::getCurves(const string& id, const Currency& ccy,
    ...
    curves.numeraireCcyCurve_ = market_->discountCurve(ccy.code(), configuration(MarketContext::pricing));
...
QuantLib::ext::shared_ptr<PricingEngine> CamAmcFwdBondEngineBuilder::buildMcEngine(
    ...
    const Handle<YieldTermStructure>& referenceCurve, const Handle<YieldTermStructure>& numeraireCcyCurve, const Handle<Quote>& conversionFactor) {
    ...
    return QuantLib::ext::make_shared<QuantExt::McLgmFwdBondEngine>(
        ...
        contractCurve, referenceCurve, numeraireCcyCurve, conversionFactor, simulationDates, stickyCloseOutDates_, externalModelIndices,
...
CamAmcFwdBondEngineBuilder::engineImpl(const string& id, const Currency& ccy, const string& discountCurveName,
    ...
    return buildMcEngine(model->lgm(0), curves.spreadedReferenceCurve_, simulationDates_, externalModelIndices,
                         curves.incomeCurve_, curves.discountCurve_, curves.referenceCurve_, curves.numeraireCcyCurve_, curves.conversionFactor_);
}
