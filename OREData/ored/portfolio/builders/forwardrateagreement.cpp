...
boost::shared_ptr<PricingEngine> LgmAmcFraEngineBuilder::buildMcEngine(const boost::shared_ptr<LGM>& lgm,
                                                                       const Handle<YieldTermStructure>& discountCurve,
                                                                       const std::vector<Date>& simulationDates,
                                                                       const std::vector<Size>& externalModelIndices) {

    return boost::make_shared<QuantExt::McLgmFraEngine>(
...
boost::shared_ptr<PricingEngine> LgmAmcFraEngineBuilder::engineImpl(const Currency& ccy) {
    DLOG("Building AMC Fra engine for ccy " << ccy << " (from externally given CAM)");

    QL_REQUIRE(cam_ != nullptr, "CamAmcFraEngineBuilder::engineImpl: cam is null");
