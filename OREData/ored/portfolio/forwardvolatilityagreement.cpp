...
void ForwardVolatilityAgreement::build(const QuantLib::ext::shared_ptr<EngineFactory>& factory) {
    ...
    if (isPut) {
        ...
        numbers_.emplace_back("Number", "Strike", underlyingStrikeProvided_ ? underlyingStrike_ : "0.0");
    }
    ...
    static const std::string putScript =
        ...
        "\n"
        "D1 = ln(S1 / Strike) + (FixedRate - DividendYield + 0.5 * ImpliedVolStrike * ImpliedVolStrike) * Tau;\n"
        "D1 = D1 / (ImpliedVolStrike * sqrt(Tau));\n"
        "D2 = ln(S1 / Strike) + (FixedRate - DividendYield - 0.5 * ImpliedVolStrike * ImpliedVolStrike) * Tau;\n"
        "D2 = D2 / (ImpliedVolStrike * sqrt(Tau));\n"
        "\n"
        "P1 = (Strike * normalCdf(-D2) - normalCdf(-D1) * S1 * exp((FixedRate - DividendYield) * Tau)) *\n"
        "     exp(-DividendYield * Tau);\n"
        "PremiumLeg = -1 * LongShort * Quantity * LOGPAY(P1, FvaDate, PremiumDate, PayCcy);\n"
        "\n"
        "Payoff = max(Strike - Underlying(OptionExpiry), 0);\n"
...
void ForwardVolatilityAgreement::fromXML(XMLNode* node) {
    ...
    underlyingStrikeProvided_ = !underlyingStrike_.empty();
    ...
    payoffType_ = XMLUtils::getChildValue(tradeDataNode, "PayoffStyle", false, "Straddle");
...
XMLNode* ForwardVolatilityAgreement::toXML(XMLDocument& doc) const {
    ...
    if (!payoffType_.empty() && payoffType_ != "Straddle")
        XMLUtils::addChild(doc, tradeNode, "PayoffStyle", payoffType_);
