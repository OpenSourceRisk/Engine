 ...
 */
 ...
 std::map<AssetClass, std::set<std::string>> EquityAsianOption::underlyingIndices(const boost::shared_ptr<ReferenceDataManager>& referenceDataManager) const {
 ...
 void EquityAsianOption::fromXML(XMLNode* node) {
     ...
     XMLNode* scheduleDataNode = XMLUtils::getChildNode(eqNode, "ObservationDates");
 ...
 XMLNode* EquityAsianOption::toXML(XMLDocument& doc) {
     ...
     XMLUtils::appendNode(eqNode, tmp);
