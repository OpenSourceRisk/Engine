...
public:
    ...
    LegAdditionalData(const LegType& legType, bool isSimmPlainVanillaIrLeg)
        : legType_(legType), legNodeName_(to_string(legType) + "LegData"), isSimmPlainVanillaIrLeg_(isSimmPlainVanillaIrLeg) {}
...
public:
    ...
    ZeroCouponFixedLegData(const vector<double>& rates, const vector<string>& rateDates = vector<string>(),
        ...
        : LegAdditionalData(LegType::ZeroCouponFixed, false), rates_(rates), rateDates_(rateDates), compounding_(compounding),
...
public:
    ...
    FloatingLegData(const string& index, QuantLib::Size fixingDays, bool isInArrears, const vector<double>& spreads,
        ...
        : LegAdditionalData(LegType::Floating, true), index_(ore::data::internalIndexName(index)), fixingDays_(fixingDays),
...
public:
    ...
    YoYLegData(string index, string observationLag, Size fixingDays,
        ...
        : LegAdditionalData(LegType::YY, false), index_(index), observationLag_(observationLag), fixingDays_(fixingDays),
...
public:
    //! Default constructor
    CMSLegData() : LegAdditionalData(LegType::CMS, false), fixingDays_(Null<Size>()), isInArrears_(true), nakedOption_(false) {}
    //! Constructor
    CMSLegData(const string& swapIndex, Size fixingDays, bool isInArrears, const vector<double>& spreads,
        ...
        : LegAdditionalData(LegType::CMS, false), swapIndex_(swapIndex), fixingDays_(fixingDays), isInArrears_(isInArrears),
...
public:
    //! Default constructor
    CMSSpreadLegData()
        : LegAdditionalData(LegType::CMSSpread, false), fixingDays_(Null<Size>()), isInArrears_(false), nakedOption_(false) {}
    //! Constructor
    CMSSpreadLegData(const string& swapIndex1, const string& swapIndex2, Size fixingDays, bool isInArrears,
                     ...
                     const vector<double>& gearings = vector<double>(),
                     const vector<string>& gearingDates = vector<string>(), bool nakedOption = false)
        : LegAdditionalData(LegType::CMSSpread, false), swapIndex1_(swapIndex1), swapIndex2_(swapIndex2), fixingDays_(fixingDays),
...
public:
    //! Default constructor
    CMBLegData() : LegAdditionalData(LegType::CMB, false), fixingDays_(Null<Size>()), isInArrears_(true), nakedOption_(false) {}
    //! Constructor
    CMBLegData(const string& genericBond, bool hasCreditRisk, Size fixingDays, bool isInArrears, const vector<double>& spreads,
               ...
               const vector<string>& floorDates = vector<string>(), const vector<double>& gearings = vector<double>(),
               const vector<string>& gearingDates = vector<string>(), bool nakedOption = false)
        : LegAdditionalData(LegType::CMB, false), genericBond_(genericBond), hasCreditRisk_(hasCreditRisk), fixingDays_(fixingDays), isInArrears_(isInArrears),
          spreads_(spreads), spreadDates_(spreadDates), caps_(caps), capDates_(capDates), floors_(floors),
          floorDates_(floorDates), gearings_(gearings), gearingDates_(gearingDates), nakedOption_(nakedOption) {
        //indices_.insert(swapIndex_);
    }
